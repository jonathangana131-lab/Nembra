...
/// Visual currentness only. The canonical adapter below derives this from the
/// package-owned Energy Rail projection; this enum never grants telemetry authority.
...
///
/// `acceptedWatts` is semantic measurement truth. `displayWatts`, `railFraction`,
/// and `peakMarkerFraction` are display-clock values/geometry and must never become
/// telemetry, persistence, ride evidence, protocol evidence, or physical claims.
/// `acceptedTargetFraction` is stable package-derived presentation geometry used by
/// Reduce Motion so the app never reconstructs a power scale from watts.
struct NembraEnergyRailVisualState: Equatable {
    ...
    /// The only production app adapter. It consumes the sealed NembraCore projection
    /// as-is and cannot accept loose watts/geometry/currentness primitives.
    init(projection: PropulsionEnergyRailAppProjection) {
        let visualCurrentness: NembraEnergyRailVisualCurrentness
        switch projection.currentness {
        case .live:
            visualCurrentness = .live
        case .retained:
            visualCurrentness = .retained
        case .unavailable:
            visualCurrentness = .unavailable
        }

        self.init(
            currentness: visualCurrentness,
            acceptedWatts: projection.acceptedWatts,
...
/// owned by the enclosing Energy Rail and announces only accepted semantic watts.
private struct NembraRollingPowerValueView: View {
    ...
    /// Fallback rendering capacity only, never a physical motor/controller maximum.
...
///
/// The caller owns the display clock. This view adds no second smoothing algorithm.
/// Under Reduce Motion it snaps to accepted semantic watts and the canonical accepted
/// target geometry instead of consuming spatial interpolation.
