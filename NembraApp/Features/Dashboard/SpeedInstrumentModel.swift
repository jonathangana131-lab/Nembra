...
///
/// Speed and propulsion share a localized display clock, while accepted source truth
/// remains outside that clock. Propulsion may enter only through the Store's sealed
/// Simulator power projection and NembraCore's source-receipt runtime.
@MainActor
struct DashboardSpeedInstrumentView: View {
    ...
    var body: some View {
        ...
        .onChange(of: reduceMotion) { _, _ in
            // Reduce Motion stops continuous display frames, but package-owned
            // one-shot deadlines still wake semantic freshness transitions.
            refreshEnergyRailPresentationSchedule()
        }
        .onDisappear {
            model.stop()
            // SwiftUI view lifetime is not propulsion source lifetime. Do not mark
            // evidence unavailable merely because this subtree disappeared.
    ...
    /// Mechanical Store -> package mapping. No aggregate watts, speed receipt,
    /// mode callback, view timestamp, or display clock can mint power authority.
    ...
    private func waitForEnergyRailPresentationTransition() async {
        guard let deadline = energyRailNextTransitionUptimeNanoseconds else { return }
        let now = DispatchTime.now().uptimeNanoseconds
        if deadline > now {
            do {
        ...
        // Recompose presentation only. Accepted source receipt chronology is unchanged.
    ...
    private func energyRailVisualState(
        ...
        guard sourceCapability,
              let energyRailRuntime else { return nil }
    ...
    private func instrumentContent(
        ...
        VStack(spacing: 0) {
            ...
            // VoiceOver consumes field-specific accepted speed truth, never a 60 Hz
            // speed midpoint or the Energy Rail display clock.
