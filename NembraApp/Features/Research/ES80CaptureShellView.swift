...
struct ES80CaptureShellView: View {
    ...
    private var passiveSafetyPanel: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                Text("Nembra follows one continuous capture from repeated Bluetooth matching through passive observation and sealing. It never sends scooter control commands, and it will not choose a target from its name, signal strength, or service hints alone.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private func primaryContent(
        ...
        case .physicalProcedureLocked:
            statePanel(
                ...
                title: "Capture is locked on this build",
                message: "This exact build is still locked for real scooter capture. OFF / ON steps, connection, observation, and sealing stay unavailable until the build is cleared.",
        ...
        case let .correlationStarting(window):
            statePanel(
                ...
                message: "Nembra is waiting for Bluetooth and scanning to become active for this window. Timing starts only after the window is actually ready.",
        ...
        case let .correlationObserving(window):
            ...
            guidanceFootnote("This countdown is guidance only. Nembra checks the recorded window before accepting it, so tapping early cannot create evidence.")
        ...
        case .correlatedTarget:
            statePanel(
                ...
                message: "One Bluetooth target matched both ON windows and stayed absent from both OFF windows. Treat it only as a correlated signal until later physical evidence proves more.",
        ...
        case .rediscoveringTarget:
            statePanel(
                ...
                message: "Nembra is looking again for the exact signal that matched both OFF / ON cycles. Keep the scooter ON as it was in the final window.",
                symbol: "scope"
            )
            ProgressView()
                .tint(.white)
        ...
        case .targetReacquired:
            statePanel(
                ...
                message: "The same signal reappeared in a new scan after confirmation. This is still local Bluetooth correlation, not permanent scooter authentication.",
                symbol: "checkmark.circle"
            )
            primaryButton(
        ...
        case .connecting:
            statePanel(
                ...
                message: "Nembra is opening only the confirmed correlated signal for passive observation. This workflow does not send scooter control commands.",
                symbol: "link"
            )
            ProgressView()
                .tint(.white)
        ...
        case .acquiring:
            statePanel(
                ...
                title: "Checking readable data",
                message: "Nembra is passively checking what the connected target exposes for reading and notifications. Observation starts only after this setup finishes cleanly.",
        ...
        case .observing:
            ...
            statePanel(
                ...
                title: remaining == 0 ? "Waiting for final seal check" : "Hold observation — \(remaining)s",
                message: "Passive setup is complete. Keep Nembra foregrounded and the scooter stationary while the required observation time finishes. The displayed timer is guidance only.",
                symbol: "timer"
            )
            .accessibilityElement(children: .ignore)
            ...
            .accessibilityValue(
                remaining == 0
                    ? "Display guidance complete; waiting for final seal check"
                    : "\(remaining) seconds of display guidance remaining"
            )
            ...
            .accessibilityValue(
                canFinalize
                    ? "Ready"
                    : "Unavailable; waiting for final seal check"
            )
        ...
        case .readyToSeal:
            statePanel(
                ...
                message: "The required observation period is complete. Sealing now freezes this capture at one exact end point.",
        ...
        case .finalizing:
            statePanel(
                ...
                message: "Nembra is finishing the exact cutoff, checking the capture, and preparing the final shareable file. Keep the app open until this finishes.",
                symbol: "lock.doc"
            )
            ProgressView()
                .tint(.white)
    ...
    private func correlationObservingPanel(
        ...
        return VStack(alignment: .leading, spacing: 16) {
            ...
            Text("Nembra is recording nearby Bluetooth signals for this exact window. Keep the phone nearby and Nembra in the foreground; do not open the stock scooter app during this series.")
                .font(.body)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
        }
    ...
    private func observationHealthStrip(
        ...
        return HStack(spacing: 12) {
            healthItem("SIGNAL", value: connection == .connected ? "BOUND" : "WAIT")
            Divider().frame(height: 28).overlay(.white.opacity(0.12))
            healthItem("SETUP", value: observationReady ? "READY" : "WAIT")
            Divider().frame(height: 28).overlay(.white.opacity(0.12))
            healthItem("SEAL", value: horizonReady ? "READY" : "WAIT")
        }
        .padding(14)
        .background(.white.opacity(0.045), in: RoundedRectangle(cornerRadius: 14, style: .continuous))
        .accessibilityElement(children: .combine)
        .accessibilityLabel(
            "Capture health. Signal \(connection == .connected ? "bound" : "waiting"). Passive setup \(observationReady ? "ready" : "waiting"). Seal \(horizonReady ? "ready" : "waiting")."
        )
    }
...
#endif
                Text("The exact \(report.finalShareByteCount.formatted())-byte final Share artifact passed the final Share integrity checks. No protocol field meaning is claimed yet.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            } else if let artifact = coordinator.finalizedArtifact {
                Text("\(artifact.captureJSON.count.formatted()) capture bytes are sealed for this run. Analysis is not ready until Nembra verifies the exact final Share file and its contained evidence.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
