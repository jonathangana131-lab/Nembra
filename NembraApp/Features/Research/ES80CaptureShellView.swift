...
struct ES80CaptureShellView: View {
    ...
    private var passiveSafetyPanel: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                Text("One capture, start to finish")
                    .font(.headline)
                    .foregroundStyle(.white)

                Text("Nembra keeps one Capture session from the four OFF / ON checks through passive observation and the final seal. It never sends scooter-control writes and never guesses the target from a name, signal strength, or service hint.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private func primaryContent(
        ...
        case .physicalProcedureLocked:
            statePanel(
                ...
                message: "This build is locked for field use. No OFF / ON check, connection, observation, or seal can start.",
        ...
        case let .correlationStarting(window):
            statePanel(
                ...
                message: "Nembra is waiting for Bluetooth scanning to become active for this window. Capture timing starts only after scanning is actually running.",
        ...
        case let .correlationObserving(window):
            ...
            .accessibilityHint("This timer is guidance only. Capture verifies the required observation time before accepting the window.")

            guidanceFootnote("This countdown is guidance only. Nembra verifies the required observation interval before accepting this window, so finishing early cannot create a valid result.")
        ...
        case .noRepeatableTarget:
            statePanel(
                ...
                message: "No single Bluetooth signal disappeared in both OFF windows and returned in both ON windows. Nembra will not guess from name, signal strength, service hints, or short IDs.",
        ...
        case .correlatedTarget:
            statePanel(
                ...
                message: "One Bluetooth signal appeared in both ON windows and disappeared in both OFF windows. Treat it only as a correlated Bluetooth target for this Capture session.",
        ...
        case .rediscoveringTarget:
            statePanel(
                ...
                message: "Nembra is checking for the same Bluetooth signal again before passive observation. Keep the scooter ON after the final window.",
                symbol: "scope"
            )
            ProgressView()
                .tint(.white)
        ...
        case .targetReacquired:
            statePanel(
                ...
                message: "The same Bluetooth signal reappeared in a fresh check. This confirms the local match for this Capture session; it does not permanently authenticate the scooter.",
                symbol: "checkmark.circle"
            )
            primaryButton(
        ...
        case .connecting:
            statePanel(
                ...
                message: "Nembra is connecting only to the matched Bluetooth signal for passive observation. This workflow sends no scooter-control writes.",
                symbol: "link"
            )
            ProgressView()
                .tint(.white)
        ...
        case .acquiring:
            statePanel(
                ...
                message: "Nembra is passively learning what the scooter exposes over Bluetooth. Capture will show Ready only after this discovery step is complete.",
        ...
        case .observing:
            ...
            statePanel(
                ...
                title: remaining == 0 ? "Waiting for final seal checks" : "Hold observation — \(remaining)s",
                message: "Bluetooth discovery is ready. Keep Nembra foregrounded and the scooter stationary while the required observation time completes. The displayed timer is guidance only.",
                symbol: "timer"
            )
            .accessibilityElement(children: .ignore)
            ...
            .accessibilityValue(
                remaining == 0
                    ? "Display guidance complete; waiting for final seal checks"
                    : "\(remaining) seconds of display guidance remaining"
            )
            ...
            .accessibilityValue(
                canFinalize
                    ? "Ready"
                    : "Unavailable; waiting for the required observation checks"
            )
            .accessibilityHint("Available only after Nembra verifies the required observation duration.")

        case .readyToSeal:
            statePanel(
                eyebrow: "HORIZON READY",
                title: "Capture can be sealed",
                message: "The required observation checks are complete. Seal Capture now to freeze this session into its final shareable evidence.",
        ...
        case .finalizing:
            statePanel(
                ...
                message: "Nembra is finishing the last accepted records, sealing the Capture, and preparing the final evidence file. Keep the app open until this finishes.",
                symbol: "lock.doc"
            )
            ProgressView()
                .tint(.white)
    ...
    private func correlationObservingPanel(
        ...
        return VStack(alignment: .leading, spacing: 16) {
            ...
            Text("Nembra is recording nearby Bluetooth signals for this window. Keep the phone nearby and the app foregrounded; do not open the stock scooter app during this series.")
                .font(.body)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
        }
...
#endif
                Text("The exact \(report.finalShareByteCount.formatted())-byte final Share artifact passed all required file-integrity checks. No protocol field meaning is claimed yet.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            } else if let artifact = coordinator.finalizedArtifact {
                Text("\(artifact.captureJSON.count.formatted()) capture bytes are sealed from this Capture session. Analysis readiness is not earned until Nembra verifies the exact final Share file and its nested evidence.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
