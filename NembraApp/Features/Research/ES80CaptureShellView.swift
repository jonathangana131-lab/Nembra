...
struct ES80CaptureShellView: View {
    ...
    private var passiveSafetyPanel: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                Text("Nembra keeps one continuous session from the four OFF / ON checks through passive observation and sealing. It writes no scooter values and never treats a name, signal strength, or service hint as proof of the target.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private func primaryContent(
        ...
        case .physicalProcedureLocked:
            statePanel(
                eyebrow: "PHYSICAL PROCEDURE",
                title: "This build is not authorized",
                message: "This build has not been cleared for the physical procedure. OFF / ON checks, connection, observation, and sealing stay locked.",
        ...
        case let .correlationStarting(window):
            statePanel(
                ...
                message: "Nembra is waiting for Bluetooth to become ready and scanning for this window. Observation has not started yet.",
        ...
        case let .correlationObserving(window):
            ...
            .accessibilityHint("This timer cannot create evidence; Nembra decides when the window has enough observations.")

            guidanceFootnote("This countdown is guidance only. Nembra decides when enough Bluetooth observations have been recorded; tapping early cannot complete the window.")
        ...
        case .noRepeatableTarget:
            statePanel(
                ...
                message: "No Bluetooth target was absent in both OFF windows and repeated in both ON windows. Nembra will not guess from name, signal strength, services, or a short ID.",
        ...
        case let .ambiguousTargets(count):
            statePanel(
                ...
                message: "More than one Bluetooth target followed the OFF / ON pattern. Nembra refuses to guess from display name, signal strength, services, or a short ID.",
        ...
        case .correlatedTarget:
            statePanel(
                ...
                message: "One Bluetooth target was present in both ON windows and absent from both OFF windows. Treat this only as a correlated signal, not verified scooter identity.",
        ...
        case .rediscoveringTarget:
            statePanel(
                ...
                message: "A fresh scan is looking again for the exact signal that passed both OFF / ON cycles. Keep the scooter ON from the final window.",
                symbol: "scope"
            )
            ProgressView()
                .tint(.white)
        ...
        case .targetReacquired:
            statePanel(
                ...
                message: "The same Bluetooth target reappeared in a fresh scan after confirmation. This is still local correlation evidence, not permanent hardware authentication.",
                symbol: "checkmark.circle"
            )
            primaryButton(
        ...
        case .connecting:
            statePanel(
                ...
                message: "Nembra is connecting only to the signal matched by this run. This workflow stays passive and performs no scooter value writes.",
                symbol: "link"
            )
            ProgressView()
                .tint(.white)
        ...
        case .acquiring:
            statePanel(
                ...
                title: "Checking the readable Bluetooth surface",
                message: "Nembra is passively discovering the readable Bluetooth surface for this run. Observation starts only after required discovery is complete.",
        ...
        case .observing:
            ...
            .accessibilityValue(
                canFinalize
                    ? "Ready"
                    : "Unavailable; waiting for seal readiness"
            )
        ...
        case .readyToSeal:
            statePanel(
                ...
                message: "Required discovery and observation are complete. Finishing now asks Nembra to seal this capture once from the same continuous session.",
        ...
        case .finalizing:
            statePanel(
                ...
                message: "Nembra is freezing the final cutoff, checking integrity, and preparing the sealed capture file. Keep the app open while this finishes.",
                symbol: "lock.doc"
            )
            ProgressView()
                .tint(.white)
    ...
    private func correlationReadyPanel(_ window: PassiveBluetoothPowerCycleObservationPhase) -> some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text(window.operatorExpectedPowerOn
                 ? "Set the scooter to ON, keep the stock app closed, then begin this Bluetooth observation window."
                 : "Set the scooter fully OFF, keep the stock app closed, then begin this Bluetooth observation window.")
                .font(.body)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
        }
    ...
    private func correlationObservingPanel(
        ...
        return VStack(alignment: .leading, spacing: 16) {
            ...
            Text("Nembra is recording nearby Bluetooth signals for this exact window. Keep the phone nearby and the app foregrounded; do not open the stock scooter app during this series.")
                .font(.body)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
        }
    ...
    private func observationHealthStrip(
        ...
        .accessibilityElement(children: .combine)
        .accessibilityLabel(
            "Capture health. Target \(connection == .connected ? "bound" : "waiting"). Passive discovery \(observationReady ? "ready" : "waiting"). Seal readiness \(horizonReady ? "ready" : "waiting")."
        )
    }
...
#endif
                Text("The exact \(report.finalShareByteCount.formatted())-byte final Share file passed all required Capture integrity checks. No protocol field meaning is claimed yet.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
    ...
    private func heroTitle(for phase: Phase) -> String {
        ...
        case .readyToSeal, .observing: return "Hold the observation."
        case .acquiring, .connecting, .rediscoveringTarget, .targetReacquired: return "Stay with the matched signal."
    ...
    private func statusTitle(for phase: Phase) -> String {
        switch phase {
        case .physicalProcedureLocked: return "Physical procedure locked"
        ...
        case .ambiguousTargets: return "More than one target"
        case .correlatedTarget: return "Scooter signal found"
        case .rediscoveringTarget: return "Finding signal again"
        case .targetReacquired: return "Signal reacquired"
