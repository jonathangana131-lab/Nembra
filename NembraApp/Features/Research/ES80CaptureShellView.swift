...
final class ES80GuidedBluetoothCapture: NSObject, ObservableObject {
    ...
    private var pendingReadOrigins: [ObjectIdentifier: String] = [:]
    private var readableCharacteristicByID: [ObjectIdentifier: CBCharacteristic] = [:]
    ...
    var scenarioProgressFraction: Double { guard !Self.scenarios.isEmpty else { return 1 }; return min(1, Double(currentScenarioIndex) / Double(Self.scenarios.count)) }

    func beginBaselineScan() {
        guard central.state == .poweredOn, !isConnected, !isConnecting else { lastMessage = "Bluetooth must be on and no scooter can already be connected."; return }
        stopScanningIfNeeded(recordStop: false); baselinePeripheralIDs.removeAll(); candidateByID.removeAll(); candidates.removeAll(); peripheralByID.removeAll(); exportData = nil; discoveryStage = .baselineScanning; record(kind: "discovery_baseline_started")
        central.scanForPeripherals(withServices: nil, options: [CBCentralManagerScanOptionAllowDuplicatesKey: true]); isScanning = true; lastMessage = "Scooter OFF baseline is running. Leave the scooter OFF for about 8–10 seconds."
    }

    func finishBaseline() {
        guard discoveryStage == .baselineScanning else { return }; stopScanningIfNeeded(); baselinePeripheralIDs = Set(candidateByID.keys); discoveryStage = .waitingForPowerOn; record(kind: "discovery_baseline_finished", details: ["baselinePeripheralCount": String(baselinePeripheralIDs.count)]); lastMessage = "Baseline saved. Now turn the scooter ON, then tap Scan after power-on."
    }

    func beginPoweredOnScan() {
        guard central.state == .poweredOn, !isConnected, !isConnecting else { return }; stopScanningIfNeeded(recordStop: false); candidateByID.removeAll(); candidates.removeAll(); peripheralByID.removeAll(); discoveryStage = .poweredOnScanning; record(kind: "discovery_power_on_scan_started", details: ["baselinePeripheralCount": String(baselinePeripheralIDs.count)]); central.scanForPeripherals(withServices: nil, options: [CBCentralManagerScanOptionAllowDuplicatesKey: true]); isScanning = true; lastMessage = "Scanning after power-on. Devices marked NEW were not present in the OFF baseline."
    }

    func beginQuickScan() {
        guard central.state == .poweredOn, !isConnected, !isConnecting else { return }; stopScanningIfNeeded(recordStop: false); baselinePeripheralIDs.removeAll(); candidateByID.removeAll(); candidates.removeAll(); peripheralByID.removeAll(); discoveryStage = .quickScanning; record(kind: "discovery_quick_scan_started"); central.scanForPeripherals(withServices: nil, options: [CBCentralManagerScanOptionAllowDuplicatesKey: true]); isScanning = true; lastMessage = "Quick scanning. Pick the scooter only if you are confident which Bluetooth device it is."
    }

    func stopScan() { stopScanningIfNeeded(); if discoveryStage == .baselineScanning { baselinePeripheralIDs = Set(candidateByID.keys); discoveryStage = .waitingForPowerOn }; lastMessage = "Scan stopped." }
    ...
    func connect(to candidate: Candidate) {
        ...
        stopScanningIfNeeded(); exportData = nil; selectedPeripheralID = candidate.id; selectedPeripheralName = candidate.name; activePeripheral = peripheral; peripheral.delegate = self; isConnecting = true; discoveryStage = .connecting; lastMessage = "Connecting to \(candidate.displayName)…"; record(kind: "connect_requested", peripheral: peripheral, details: ["displayName": candidate.displayName, "appearedAfterPowerOn": candidate.appearedAfterPowerOn ? "true" : "false", "advertisementCountBeforeConnect": String(candidate.advertisementCount)]); central.connect(peripheral)
    }

    func requestReferenceLocationIfNeeded() { refreshLocationAuthorizationText(); switch locationManager.authorizationStatus { case .notDetermined: locationManager.requestWhenInUseAuthorization(); case .authorizedAlways, .authorizedWhenInUse: startReferenceLocation(); default: record(kind: "location_reference_unavailable", details: ["authorization": locationAuthorizationText]) } }

    func beginScenario(_ scenario: ScenarioDefinition) {
        guard !isFrozen else { lastMessage = "This dataset is already frozen. Reset only if you intentionally want to start over."; return }
        ...
        let runID = UUID(); activeScenarioRunID = runID; activeScenarioID = scenario.id; activeScenarioStartedAt = Date(); activeScenarioStartedUptimeNanoseconds = DispatchTime.now().uptimeNanoseconds
        record(kind: "scenario_started", peripheral: activePeripheral, scenario: scenario, details: ["phase": scenario.phase, "recommendedSeconds": String(scenario.recommendedSeconds), "optional": scenario.optional ? "true" : "false", "motionClass": scenario.motionClass.rawValue]); activeScenarioStartEventID = nextEventID - 1; pollReadableValues(origin: "scenario_start_snapshot"); lastMessage = scenario.motionClass == .moving ? "Recording \(scenario.title). Do not touch the phone until you are fully stopped again." : "Recording \(scenario.title)."
    }
    ...
    func completeActiveScenario(note: String? = nil) {
        ...
        let finishedAt = Date(); let finishedUptime = DispatchTime.now().uptimeNanoseconds; record(kind: "scenario_completed", peripheral: activePeripheral, scenario: scenario, details: ["durationSeconds": String(format: "%.3f", Double(finishedUptime - startedUptime) / 1_000_000_000), "operatorNote": normalizedOptional(note) ?? ""]); let endEventID = nextEventID - 1
        scenarioRuns.append(ScenarioRun(id: runID, scenarioID: scenario.id, scenarioTitle: scenario.title, phase: scenario.phase, startedAt: startedAt, finishedAt: finishedAt, startedAtMonotonicNanoseconds: startedUptime, finishedAtMonotonicNanoseconds: finishedUptime, state: .completed, note: normalizedOptional(note), startEventID: startEventID, endEventID: endEventID)); clearActiveScenario(); advancePastScenario(scenario.id); lastMessage = "Saved \(scenario.title). Continue to the next step when ready."
    }

    func skipScenario(_ scenario: ScenarioDefinition, reason: String? = nil) {
        guard activeScenarioID == nil else { return }; let now = Date(); let uptime = DispatchTime.now().uptimeNanoseconds; record(kind: "scenario_skipped", peripheral: activePeripheral, scenario: scenario, details: ["reason": normalizedOptional(reason) ?? "not available / intentionally skipped"]); let eventID = nextEventID - 1; scenarioRuns.append(ScenarioRun(id: UUID(), scenarioID: scenario.id, scenarioTitle: scenario.title, phase: scenario.phase, startedAt: now, finishedAt: now, startedAtMonotonicNanoseconds: uptime, finishedAtMonotonicNanoseconds: uptime, state: .skipped, note: normalizedOptional(reason), startEventID: eventID, endEventID: eventID)); advancePastScenario(scenario.id); lastMessage = "Skipped \(scenario.title)."
    }

    func recordCustomAction(_ label: String) { guard let normalized = normalizedOptional(label) else { return }; record(kind: "operator_custom_action", peripheral: activePeripheral, details: ["label": normalized]); lastMessage = "Marked: \(normalized)" }
    func recordReference(label: String, value: String) { guard let normalizedLabel = normalizedOptional(label), let normalizedValue = normalizedOptional(value) else { return }; record(kind: "operator_reference_value", peripheral: activePeripheral, details: ["label": normalizedLabel, "value": normalizedValue]); lastMessage = "Reference saved: \(normalizedLabel) = \(normalizedValue)" }
    ...
    func prepareJSON() {
        ...
        let envelope = ExportEnvelope(schemaVersion: 2, purpose: "Guided scooter Bluetooth learning capture for Nembra protocol/capability calibration", captureID: captureID.uuidString, createdAt: createdAt, finishedAt: Date(), selectedPeripheralID: selectedPeripheralID?.uuidString, selectedPeripheralName: selectedPeripheralName, eventCount: events.count, buildIdentifier: normalizedBundleString("NembraCaptureBuildIdentifier"), buildSourceCommit: normalizedBundleString("NembraCaptureBuildCommitSHA"), locationReferenceAuthorization: locationAuthorizationText, summary: summary, scenarioRuns: scenarioRuns, limitations: ["This is a CoreBluetooth application-level capture, not an over-the-air BLE packet sniffer.", "It records advertisements exposed by iOS, GATT topology, initial and 1 Hz resampled readable characteristic values, notify/indicate callbacks, descriptor values exposed by CoreBluetooth, RSSI, lifecycle/errors, operator scenario markers, and optional iPhone location speed reference.", "This utility never issues CoreBluetooth application-characteristic value writes.", "Notification subscription may update the standard GATT Client Characteristic Configuration descriptor as required by CoreBluetooth; that is subscription transport state, not a scooter command.", "Observed bytes are raw evidence. Battery, speed, power, mode, brake, throttle, light, lock, cruise, and other semantics must be inferred only from repeatable scenario correlations.", "Another scooter app may not be able to stay connected at the same time. The guided flow is designed around scooter-local controls while Nembra owns the Bluetooth connection.", "Moving scenarios are started and completed only while fully stopped; the phone must not be handled while the scooter is moving."], events: events)
        do { let encoder = JSONEncoder(); encoder.outputFormatting = [.prettyPrinted, .sortedKeys, .withoutEscapingSlashes]; encoder.dateEncodingStrategy = .iso8601; exportData = try encoder.encode(envelope); exportFilename = "Nembra-Scooter-Learning-\(captureID.uuidString.prefix(8)).json"; lastMessage = "Dataset ready. Share this exact JSON back into ChatGPT so the Bluetooth values can be mapped into Nembra." } catch { lastMessage = "JSON encoding failed: \(error.localizedDescription)" }
    }

    func finishCaptureAndPrepareJSON() {
        guard activeScenarioID == nil else { lastMessage = "Complete the active calibration step while fully stopped before freezing the dataset."; return }
        if isScanning { stopScanningIfNeeded() }; locationManager.stopUpdatingLocation(); rssiTask?.cancel(); rssiTask = nil; readPollTask?.cancel(); readPollTask = nil; record(kind: "operator_capture_finished", peripheral: activePeripheral); acceptingEvents = false; isFrozen = true; if let activePeripheral, activePeripheral.state != .disconnected { central.cancelPeripheralConnection(activePeripheral) }; prepareJSON()
    }

    func reset() {
        stopScanningIfNeeded(recordStop: false); rssiTask?.cancel(); rssiTask = nil; readPollTask?.cancel(); readPollTask = nil; locationManager.stopUpdatingLocation(); if let activePeripheral, activePeripheral.state != .disconnected { central.cancelPeripheralConnection(activePeripheral) }; activePeripheral = nil; isConnecting = false; isConnected = false; connectedAt = nil; selectedPeripheralID = nil; selectedPeripheralName = nil; discoveryStage = .idle; peripheralByID.removeAll(); candidateByID.removeAll(); baselinePeripheralIDs.removeAll(); candidates.removeAll(); pendingReadOrigins.removeAll(); readableCharacteristicByID.removeAll(); subscribeAfterRead.removeAll(); events.removeAll(keepingCapacity: true); nextEventID = 1; eventCount = 0; exportData = nil; activeScenarioID = nil; activeScenarioStartedAt = nil; activeScenarioStartedUptimeNanoseconds = nil; activeScenarioRunID = nil; activeScenarioStartEventID = nil; currentScenarioIndex = 0; scenarioRuns.removeAll(); latestReferenceSpeedMPH = nil; latestReferenceHorizontalAccuracy = nil; observedServices.removeAll(); observedCharacteristics.removeAll(); readableCharacteristics.removeAll(); notifyingCharacteristics.removeAll(); observedDescriptors.removeAll(); advertisementCount = 0; characteristicValueEventCount = 0; descriptorValueEventCount = 0; locationReferenceSampleCount = 0; droppedEventCount = 0; acceptingEvents = true; isFrozen = false; lastMessage = "Reset complete. Start with the scooter OFF for the cleanest device identification."
    }

    func elapsedSecondsForActiveScenario(nowUptimeNanoseconds: UInt64 = DispatchTime.now().uptimeNanoseconds) -> Double { guard let start = activeScenarioStartedUptimeNanoseconds, nowUptimeNanoseconds >= start else { return 0 }; return Double(nowUptimeNanoseconds - start) / 1_000_000_000 }
    ...
    private func startRSSIPolling() { rssiTask?.cancel(); rssiTask = Task { @MainActor [weak self] in while !Task.isCancelled { guard let self, self.isConnected, let peripheral = self.activePeripheral else { return }; peripheral.readRSSI(); try? await Task.sleep(nanoseconds: 2_000_000_000) } } }
    private func startReadableValuePolling() { readPollTask?.cancel(); readPollTask = Task { @MainActor [weak self] in while !Task.isCancelled { try? await Task.sleep(nanoseconds: 1_000_000_000); guard let self, self.isConnected else { return }; self.pollReadableValues(origin: "periodic_read_1hz") } } }
    private func pollReadableValues(origin: String) { guard isConnected, let peripheral = activePeripheral else { return }; for (key, characteristic) in readableCharacteristicByID { guard pendingReadOrigins[key] == nil else { continue }; pendingReadOrigins[key] = origin; record(kind: "characteristic_read_requested", peripheral: peripheral, characteristic: characteristic, origin: origin); peripheral.readValue(for: characteristic) } }

    private func startReferenceLocation() { guard CLLocationManager.locationServicesEnabled() else { locationAuthorizationText = "Location Services disabled"; record(kind: "location_reference_unavailable", details: ["reason": "services_disabled"]); return }; switch locationManager.authorizationStatus { case .authorizedAlways, .authorizedWhenInUse: locationManager.startUpdatingLocation(); record(kind: "location_reference_started"); default: break } }
    private func refreshLocationAuthorizationText() { switch locationManager.authorizationStatus { case .notDetermined: locationAuthorizationText = "Not requested"; case .restricted: locationAuthorizationText = "Restricted"; case .denied: locationAuthorizationText = "Denied"; case .authorizedAlways: locationAuthorizationText = "Authorized always"; case .authorizedWhenInUse: locationAuthorizationText = "Authorized while using app"; @unknown default: locationAuthorizationText = "Future authorization state" } }
    ...
    private static func describeDescriptorValue(_ value: Any?) -> [String: String] { guard let value else { return ["valueType": "nil"] }; if let data = value as? Data { return ["valueType": "Data", "valueBase64": data.base64EncodedString(), "valueHex": hex(data)] }; if let string = value as? String { return ["valueType": "String", "value": string] }; if let number = value as? NSNumber { return ["valueType": "NSNumber", "value": number.stringValue] }; return ["valueType": String(describing: type(of: value)), "value": String(describing: value)] }

    private static func advertisementDetails(_ advertisementData: [String: Any], rssi: NSNumber) -> [String: String] {
        var details: [String: String] = [:]; if let name = advertisementData[CBAdvertisementDataLocalNameKey] as? String { details["localName"] = name }; if let connectable = advertisementData[CBAdvertisementDataIsConnectable] as? NSNumber { details["isConnectable"] = connectable.boolValue ? "true" : "false" }; if let tx = advertisementData[CBAdvertisementDataTxPowerLevelKey] as? NSNumber { details["txPower"] = tx.stringValue }; if let manufacturer = advertisementData[CBAdvertisementDataManufacturerDataKey] as? Data { details["manufacturerDataBase64"] = manufacturer.base64EncodedString(); details["manufacturerDataHex"] = hex(manufacturer) }; if let uuids = advertisementData[CBAdvertisementDataServiceUUIDsKey] as? [CBUUID] { details["serviceUUIDs"] = uuids.map { $0.uuidString.uppercased() }.joined(separator: ",") }; if let uuids = advertisementData[CBAdvertisementDataOverflowServiceUUIDsKey] as? [CBUUID] { details["overflowServiceUUIDs"] = uuids.map { $0.uuidString.uppercased() }.joined(separator: ",") }; if let uuids = advertisementData[CBAdvertisementDataSolicitedServiceUUIDsKey] as? [CBUUID] { details["solicitedServiceUUIDs"] = uuids.map { $0.uuidString.uppercased() }.joined(separator: ",") }; if let serviceData = advertisementData[CBAdvertisementDataServiceDataKey] as? [CBUUID: Data] { for (uuid, data) in serviceData { let key = uuid.uuidString.uppercased(); details["serviceData.\(key).base64"] = data.base64EncodedString(); details["serviceData.\(key).hex"] = hex(data) } }; details["rssiRaw"] = rssi.stringValue; details["advertisementKeys"] = advertisementData.keys.sorted().joined(separator: ","); for key in advertisementData.keys.sorted() where details["raw.\(key)"] == nil { let value = advertisementData[key]; if let data = value as? Data { details["raw.\(key).base64"] = data.base64EncodedString(); details["raw.\(key).hex"] = hex(data) } else { details["raw.\(key)"] = String(describing: value ?? "nil") } }; return details
    }
...
extension ES80GuidedBluetoothCapture: @preconcurrency CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String: Any], rssi RSSI: NSNumber) { let normalizedRSSI = RSSI.intValue == 127 ? nil : RSSI.intValue; let localName = (advertisementData[CBAdvertisementDataLocalNameKey] as? String) ?? peripheral.name; let connectable = (advertisementData[CBAdvertisementDataIsConnectable] as? NSNumber)?.boolValue; peripheralByID[peripheral.identifier] = peripheral; if discoveryStage == .baselineScanning { baselinePeripheralIDs.insert(peripheral.identifier) }; let existing = candidateByID[peripheral.identifier]; let candidate = Candidate(id: peripheral.identifier, name: localName ?? existing?.name, rssi: normalizedRSSI, connectable: connectable ?? existing?.connectable, firstSeen: existing?.firstSeen ?? Date(), lastSeen: Date(), advertisementCount: (existing?.advertisementCount ?? 0) + 1, appearedAfterPowerOn: discoveryStage == .poweredOnScanning && !baselinePeripheralIDs.contains(peripheral.identifier)); candidateByID[peripheral.identifier] = candidate; candidates = candidateByID.values.sorted { if $0.appearedAfterPowerOn != $1.appearedAfterPowerOn { return $0.appearedAfterPowerOn && !$1.appearedAfterPowerOn }; switch ($0.rssi, $1.rssi) { case let (.some(left), .some(right)) where left != right: return left > right; case (.some, .none): return true; case (.none, .some): return false; default: return $0.id.uuidString < $1.id.uuidString } }; advertisementCount += 1; record(kind: "advertisement", peripheral: peripheral, rssi: normalizedRSSI, details: Self.advertisementDetails(advertisementData, rssi: RSSI).merging(["appearedAfterPowerOn": candidate.appearedAfterPowerOn ? "true" : "false"], uniquingKeysWith: { current, _ in current })) }
    func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) { isConnecting = false; isConnected = true; discoveryStage = .connected; connectedAt = Date(); activePeripheral = peripheral; peripheral.delegate = self; selectedPeripheralName = peripheral.name ?? selectedPeripheralName; record(kind: "connected", peripheral: peripheral, details: ["peripheralName": peripheral.name ?? ""]); lastMessage = "Connected. Nembra is dumping all discoverable/readable/notifying GATT data. Start the guided calibration steps."; peripheral.discoverServices(nil); peripheral.readRSSI(); startRSSIPolling(); startReadableValuePolling(); requestReferenceLocationIfNeeded() }
    ...
    func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) { rssiTask?.cancel(); rssiTask = nil; readPollTask?.cancel(); readPollTask = nil; pendingReadOrigins.removeAll(); readableCharacteristicByID.removeAll(); locationManager.stopUpdatingLocation(); isConnecting = false; isConnected = false; record(kind: "disconnected", peripheral: peripheral, error: error); if exportData == nil { lastMessage = "Disconnected. The data already captured is still available; reconnect or export it." } }
}
...
extension ES80GuidedBluetoothCapture: @preconcurrency CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) { record(kind: "services_discovered", peripheral: peripheral, error: error); guard error == nil, let services = peripheral.services else { return }; for service in services { let uuid = service.uuid.uuidString.uppercased(); observedServices.insert(uuid); record(kind: "service", peripheral: peripheral, service: service, details: ["isPrimary": service.isPrimary ? "true" : "false"]); peripheral.discoverIncludedServices(nil, for: service); peripheral.discoverCharacteristics(nil, for: service) } }
    func peripheral(_ peripheral: CBPeripheral, didDiscoverIncludedServicesFor service: CBService, error: Error?) { record(kind: "included_services_discovered", peripheral: peripheral, service: service, error: error); guard error == nil else { return }; for included in service.includedServices ?? [] { let includedUUID = included.uuid.uuidString.uppercased(); observedServices.insert(includedUUID); record(kind: "included_service", peripheral: peripheral, service: service, details: ["includedServiceUUID": includedUUID, "includedServiceIsPrimary": included.isPrimary ? "true" : "false"]); peripheral.discoverCharacteristics(nil, for: included) } }
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) { record(kind: "characteristics_discovered", peripheral: peripheral, service: service, error: error); guard error == nil else { return }; for characteristic in service.characteristics ?? [] { let characteristicUUID = characteristic.uuid.uuidString.uppercased(); observedCharacteristics.insert(characteristicUUID); let properties = Self.characteristicProperties(characteristic.properties); let canRead = characteristic.properties.contains(.read); let canNotify = characteristic.properties.contains(.notify) || characteristic.properties.contains(.indicate); if canRead { readableCharacteristics.insert(characteristicUUID) }; if canNotify { notifyingCharacteristics.insert(characteristicUUID) }; record(kind: "characteristic", peripheral: peripheral, characteristic: characteristic, details: ["properties": properties, "isNotifyingAtDiscovery": characteristic.isNotifying ? "true" : "false"]); peripheral.discoverDescriptors(for: characteristic); let key = ObjectIdentifier(characteristic); if canRead { readableCharacteristicByID[key] = characteristic; pendingReadOrigins[key] = "initial_read"; if canNotify { subscribeAfterRead.insert(key) }; record(kind: "characteristic_read_requested", peripheral: peripheral, characteristic: characteristic, origin: "initial_read"); peripheral.readValue(for: characteristic) } else if canNotify { record(kind: "subscription_requested", peripheral: peripheral, characteristic: characteristic); peripheral.setNotifyValue(true, for: characteristic) } } }
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) { let key = ObjectIdentifier(characteristic); let pendingReadOrigin = pendingReadOrigins.removeValue(forKey: key); let wasPendingRead = pendingReadOrigin != nil; let shouldSubscribeNow = wasPendingRead && subscribeAfterRead.remove(key) != nil; let origin = pendingReadOrigin ?? (characteristic.isNotifying ? "notify_or_indicate" : "unsolicited_callback"); if let error { record(kind: "characteristic_value_error", peripheral: peripheral, characteristic: characteristic, origin: origin, error: error) } else { characteristicValueEventCount += 1; record(kind: "characteristic_value", peripheral: peripheral, characteristic: characteristic, origin: origin, value: characteristic.value, details: ["byteCount": String(characteristic.value?.count ?? 0), "isNotifying": characteristic.isNotifying ? "true" : "false"]) }; if shouldSubscribeNow { record(kind: "subscription_requested_after_read", peripheral: peripheral, characteristic: characteristic); peripheral.setNotifyValue(true, for: characteristic) } }
...
private struct GuidedCaptureTransfer: Transferable { let data: Data; let filename: String; static var transferRepresentation: some TransferRepresentation { DataRepresentation(exportedContentType: .json) { transfer in transfer.data }.suggestedFileName { transfer in transfer.filename } } }

@MainActor
struct ES80OneTimeBluetoothDumpView: View {
    @StateObject private var capture = ES80GuidedBluetoothCapture(); @State private var customAction = ""; @State private var referenceLabel = ""; @State private var referenceValue = ""; @State private var scenarioNote = ""; @State private var showingAdvanced = false
    var body: some View { NavigationStack { TimelineView(.periodic(from: .now, by: 0.5)) { _ in ScrollView { VStack(alignment: .leading, spacing: 18) { header; statusCard; if !capture.isConnected { discoveryCard } else { connectedScooterCard; calibrationCard; referenceCard; captureHealthCard; finishCard }; if let data = capture.exportData { shareCard(data: data) }; advancedDisclosure }.frame(maxWidth: 760).padding(.horizontal, 18).padding(.vertical, 20).frame(maxWidth: .infinity) }.background(Color.black.ignoresSafeArea()) }.navigationTitle("Nembra Capture").navigationBarTitleDisplayMode(.inline).preferredColorScheme(.dark).onAppear { UIApplication.shared.isIdleTimerDisabled = true }.onDisappear { UIApplication.shared.isIdleTimerDisabled = false } } }
    ...
    private var discoveryCard: some View { VStack(alignment: .leading, spacing: 14) { Label("1 · FIND THE SCOOTER", systemImage: "scope").font(.headline).foregroundStyle(.white); switch capture.discoveryStage { case .idle: Text("Best method: leave the scooter OFF, scan the room for 8–10 seconds, save that baseline, then power the scooter ON. Nembra will mark Bluetooth devices that newly appeared.").font(.subheadline).foregroundStyle(.secondary); Button("Start scooter-OFF baseline") { capture.beginBaselineScan() }.buttonStyle(.borderedProminent).disabled(capture.bluetoothState != .poweredOn); Button("I already know the device · quick scan") { capture.beginQuickScan() }.buttonStyle(.bordered).disabled(capture.bluetoothState != .poweredOn); case .baselineScanning: Text("Keep the scooter OFF. Let this run for about 8–10 seconds.").foregroundStyle(.secondary); Button("Baseline done") { capture.finishBaseline() }.buttonStyle(.borderedProminent); case .waitingForPowerOn: Text("Now turn the scooter ON. Wait a moment, then start the second scan.").foregroundStyle(.secondary); Button("Scan after power-on") { capture.beginPoweredOnScan() }.buttonStyle(.borderedProminent); case .poweredOnScanning, .quickScanning: Text(capture.discoveryStage == .poweredOnScanning ? "Tap the likely scooter. NEW means the device was not present while the scooter was OFF." : "Tap the scooter only when you are confident which device it is.").foregroundStyle(.secondary); Button("Stop scan") { capture.stopScan() }.buttonStyle(.bordered); case .connecting: ProgressView("Connecting…").tint(.white); case .connected: EmptyView() }; if !capture.candidates.isEmpty { VStack(spacing: 10) { ForEach(capture.candidates) { candidate in Button { capture.connect(to: candidate) } label: { HStack(spacing: 12) { VStack(alignment: .leading, spacing: 3) { HStack(spacing: 7) { Text(candidate.displayName).font(.headline).foregroundStyle(.white); if candidate.appearedAfterPowerOn { Text("NEW").font(.caption2.monospaced().weight(.bold)).foregroundStyle(.black).padding(.horizontal, 7).padding(.vertical, 3).background(.green, in: Capsule()) } }; Text(candidate.id.uuidString).font(.caption2.monospaced()).foregroundStyle(.secondary).lineLimit(1); Text("\(candidate.rssiText) · \(candidate.advertisementCount) ads").font(.caption).foregroundStyle(.secondary) }; Spacer(); Image(systemName: "chevron.right").foregroundStyle(.secondary) }.padding(14).background(candidate.appearedAfterPowerOn ? .green.opacity(0.10) : .white.opacity(0.05), in: RoundedRectangle(cornerRadius: 16, style: .continuous)) }.buttonStyle(.plain).disabled(candidate.connectable == false || capture.isConnecting) } } } }.padding(18).background(.white.opacity(0.06), in: RoundedRectangle(cornerRadius: 22, style: .continuous)) }
    ...
    private var calibrationCard: some View { VStack(alignment: .leading, spacing: 14) { HStack { Label("3 · GUIDED CALIBRATION", systemImage: "list.bullet.clipboard").font(.headline).foregroundStyle(.white); Spacer(); Text("\(min(capture.currentScenarioIndex, ES80GuidedBluetoothCapture.scenarios.count))/\(ES80GuidedBluetoothCapture.scenarios.count)").font(.caption.monospacedDigit().weight(.semibold)).foregroundStyle(.secondary) }; ProgressView(value: capture.scenarioProgressFraction).tint(.green); if let scenario = capture.currentScenario { scenarioCard(scenario) } else { Label("Guided sequence complete", systemImage: "checkmark.seal.fill").font(.title3.weight(.semibold)).foregroundStyle(.green); Text("You can still add custom capability/reference markers below, then prepare the final JSON.").foregroundStyle(.secondary) }; if capture.completedScenarioCount > 0 || capture.skippedScenarioCount > 0 { Text("\(capture.completedScenarioCount) completed · \(capture.skippedScenarioCount) skipped").font(.caption.monospacedDigit()).foregroundStyle(.secondary) } }.padding(18).background(.white.opacity(0.06), in: RoundedRectangle(cornerRadius: 22, style: .continuous)) }
    private func scenarioCard(_ scenario: ES80GuidedBluetoothCapture.ScenarioDefinition) -> some View { let isActive = capture.activeScenarioID == scenario.id; let elapsed = isActive ? capture.elapsedSecondsForActiveScenario() : 0; return VStack(alignment: .leading, spacing: 12) { HStack(alignment: .firstTextBaseline) { VStack(alignment: .leading, spacing: 4) { Text(scenario.phase.uppercased()).font(.caption2.monospaced().weight(.bold)).foregroundStyle(.secondary); Text(scenario.title).font(.title3.weight(.semibold)).foregroundStyle(.white) }; Spacer(); if scenario.optional { Text("OPTIONAL").font(.caption2.monospaced().weight(.bold)).foregroundStyle(.secondary) } }; Text(scenario.instruction).font(.body).foregroundStyle(.white).fixedSize(horizontal: false, vertical: true); if let caution = scenario.caution { Label(caution, systemImage: "exclamationmark.triangle.fill").font(.footnote.weight(.semibold)).foregroundStyle(.orange).fixedSize(horizontal: false, vertical: true) }; Text(scenario.completionHint).font(.footnote).foregroundStyle(.secondary); if isActive { HStack { Label("RECORDING", systemImage: "record.circle.fill").font(.caption.monospaced().weight(.bold)).foregroundStyle(.red); Spacer(); Text(timeText(elapsed)).font(.title3.monospacedDigit().weight(.semibold)).foregroundStyle(.white) }; if scenario.recommendedSeconds > 0 { ProgressView(value: min(1, elapsed / Double(scenario.recommendedSeconds))).tint(elapsed >= Double(scenario.recommendedSeconds) ? .green : .white) }; TextField("Optional note about what happened", text: $scenarioNote, axis: .vertical).textFieldStyle(.roundedBorder).lineLimit(2...4); Button(scenario.motionClass == .moving ? "I am fully stopped · complete step" : "Complete step") { capture.completeActiveScenario(note: scenarioNote); scenarioNote = "" }.buttonStyle(.borderedProminent) } else { Button(startButtonTitle(for: scenario)) { scenarioNote = ""; capture.beginScenario(scenario) }.buttonStyle(.borderedProminent); if scenario.optional { Button("Skip — scooter doesn't have this / not safe here") { capture.skipScenario(scenario) }.buttonStyle(.bordered) } } }.padding(16).background(.black.opacity(0.32), in: RoundedRectangle(cornerRadius: 18, style: .continuous)) }
    private var referenceCard: some View { VStack(alignment: .leading, spacing: 14) { Label("EXTRA CAPABILITIES + REFERENCE VALUES", systemImage: "plus.circle").font(.headline).foregroundStyle(.white); Text("Use these while stopped for anything the guided list does not cover. Examples: horn, cruise, lock state, units, zero-start, speed limit, a dashboard battery %, or a display reading.").font(.footnote).foregroundStyle(.secondary); TextField("Action you just triggered (example: horn pressed)", text: $customAction).textFieldStyle(.roundedBorder); Button("Mark custom action now") { capture.recordCustomAction(customAction); customAction = "" }.buttonStyle(.bordered).disabled(customAction.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty); Divider().overlay(.white.opacity(0.12)); TextField("Reference label (example: dashboard battery)", text: $referenceLabel).textFieldStyle(.roundedBorder); TextField("Reference value (example: 73%)", text: $referenceValue).textFieldStyle(.roundedBorder); Button("Record reference value") { capture.recordReference(label: referenceLabel, value: referenceValue); referenceLabel = ""; referenceValue = "" }.buttonStyle(.bordered).disabled(referenceLabel.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || referenceValue.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty) }.padding(18).background(.white.opacity(0.06), in: RoundedRectangle(cornerRadius: 22, style: .continuous)) }
    private var captureHealthCard: some View { VStack(alignment: .leading, spacing: 10) { Label("CAPTURE HEALTH", systemImage: "waveform.path.ecg").font(.headline).foregroundStyle(.white); LabeledContent("Raw events", value: "\(capture.eventCount)"); LabeledContent("Completed steps", value: "\(capture.completedScenarioCount)"); LabeledContent("Skipped steps", value: "\(capture.skippedScenarioCount)"); LabeledContent("GPS reference", value: referenceSpeedText); LabeledContent("Dataset state", value: capture.isFrozen ? "Frozen" : "Recording"); if capture.droppedEventCount > 0 { Label("\(capture.droppedEventCount) events exceeded the in-memory safety ceiling", systemImage: "exclamationmark.triangle.fill").font(.footnote).foregroundStyle(.orange) }; Text("Keep the stock scooter app closed while Nembra owns the Bluetooth connection. If there is an extra scooter-local control we did not list, mark it above while stopped.").font(.footnote).foregroundStyle(.secondary) }.padding(18).background(.white.opacity(0.06), in: RoundedRectangle(cornerRadius: 22, style: .continuous)) }
    private var finishCard: some View { VStack(alignment: .leading, spacing: 12) { Label("4 · FINISH + SHARE", systemImage: "square.and.arrow.up").font(.headline).foregroundStyle(.white); Text("When you have finished the guided steps, prepare one JSON. It contains the complete event timeline plus scenario boundaries so we can map the real scooter protocol.").font(.subheadline).foregroundStyle(.secondary); Button("Finish capture & prepare JSON") { capture.finishCaptureAndPrepareJSON() }.buttonStyle(.borderedProminent).disabled(capture.activeScenarioID != nil || capture.isFrozen); if capture.activeScenarioID != nil { Text("Finish is locked while a calibration step is active. Stop safely and complete that step first.").font(.footnote).foregroundStyle(.orange) }; Button("Prepare a snapshot without ending capture") { capture.prepareJSON() }.buttonStyle(.bordered) }.padding(18).background(.white.opacity(0.06), in: RoundedRectangle(cornerRadius: 22, style: .continuous)) }
