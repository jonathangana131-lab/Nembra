...
struct ES80CaptureShellView: View {
    ...
    private var passiveSafetyPanel: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                Text("Nembra keeps one continuous read-only session from scooter matching through final sealing. It never sends scooter commands and never chooses a target from its name, signal strength, or service hints.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private func primaryContent(
        ...
        case .physicalProcedureLocked:
            statePanel(
                ...
                message: "This build is locked for field use. OFF / ON matching, connection, capture, and sealing stay unavailable.",
        ...
        case let .correlationStarting(window):
            statePanel(
                ...
                message: "Nembra is waiting for Bluetooth to become ready and start scanning. This window has not started recording yet.",
        ...
        case let .correlationObserving(window):
            ...
            .accessibilityHint("This timer is guidance only; Nembra decides when enough capture evidence exists.")

            guidanceFootnote("This countdown is guidance only. Nembra records the window from its own capture timing; tapping early cannot create evidence.")
        ...
        case .noRepeatableTarget:
            statePanel(
                ...
                message: "No signal stayed absent in both OFF windows and appeared in both ON windows. Nembra will not guess from name, signal strength, service hints, or a shortened identifier.",
        ...
        case let .ambiguousTargets(count):
            statePanel(
                ...
                message: "More than one signal followed the same OFF / ON pattern. Nembra refuses to break the tie using its name, signal strength, services, or a shortened identifier.",
        ...
        case .correlatedTarget:
            statePanel(
                ...
                message: "One Bluetooth signal appeared in both ON windows and stayed absent from both OFF windows. Treat it only as a correlated target for this capture.",
        ...
        case .rediscoveringTarget:
            statePanel(
                ...
                message: "Nembra is looking again for the exact signal matched by both OFF / ON cycles. Keep the scooter ON from the final window.",
                symbol: "scope"
            )
            ProgressView()
                .tint(.white)
        ...
        case .targetReacquired:
            statePanel(
                ...
                message: "The same Bluetooth signal reappeared in a fresh scan after confirmation. This is still local correlation evidence, not permanent hardware authentication.",
                symbol: "checkmark.circle"
            )
            primaryButton(
        ...
        case .connecting:
            statePanel(
                ...
                message: "Nembra is connecting only to the signal you just confirmed. This workflow remains read-only and sends no scooter commands.",
                symbol: "link"
            )
            ProgressView()
                .tint(.white)
        ...
        case .acquiring:
            statePanel(
                ...
                message: "Nembra is mapping the scooter's readable Bluetooth surface without sending commands. Ready appears only after this discovery stage is complete.",
        ...
        case .observing:
            ...
            statePanel(
                ...
                message: "Bluetooth discovery is complete. Keep Nembra open and the scooter stationary while the required observation time completes. The displayed timer is guidance only.",
                symbol: "timer"
            )
            .accessibilityElement(children: .ignore)
            ...
            .accessibilityValue(
                canFinalize
                    ? "Ready"
                    : "Unavailable; required observation time is still being verified"
            )
        ...
        case .readyToSeal:
            statePanel(
                ...
                message: "The required observation time is complete. Sealing now freezes this capture into one final read-only artifact.",
        ...
        case .finalizing:
            statePanel(
                ...
                message: "Nembra is finishing queued capture events, checking integrity, and creating the final shareable capture. Keep the app open until sealing completes.",
                symbol: "lock.doc"
            )
            ProgressView()
                .tint(.white)
    ...
    private func observationHealthStrip(
        ...
        .accessibilityElement(children: .combine)
        .accessibilityLabel(
            "Capture health. Target \(connection == .connected ? "bound" : "waiting"). Discovery \(observationReady ? "ready" : "waiting"). Observation time \(horizonReady ? "ready" : "waiting")."
        )
    }
...
#endif
                Text("The exact \(report.finalShareByteCount.formatted())-byte Share Capture file passed every required integrity check. No protocol field meaning is claimed yet.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            } else if let artifact = coordinator.finalizedArtifact {
                Text("\(artifact.captureJSON.count.formatted()) capture bytes are sealed. Ready for analysis appears only after Nembra verifies the exact final Share file and its nested evidence.")
                    .font(.subheadline)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
