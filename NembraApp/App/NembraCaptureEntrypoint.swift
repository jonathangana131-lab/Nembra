...
#if canImport(ThingSmartHomeKit) && canImport(ThingSmartDeviceKit)
...
private struct CaptureP0Root: View {
        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    Text("No ride calibration is available here. The next physical run is stationary and only proves supported Tuya authentication, >45 s continuity, and a genuine Tuya application report on that same SDK-owned session.").foregroundStyle(.secondary)
                    safety
                    account
    ...
    private var safety: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("The existing QR account link is metadata/ownership discovery only. Nembra does not synthesize, fuzz, or guess FD50 authentication frames.").foregroundStyle(.secondary)
            Text("No unbind, reset, lock, speed, light, mode, throttle, brake, firmware, DP query, or other DP command is sent.").font(.footnote.bold()).foregroundStyle(.green)
        }.card()
    }
    ...
    private var devices: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("The secure-link controller uses the selected Tuya device identity. CoreBluetooth is used only to correlate the nearby physical scooter; it is never used as post-authentication evidence.").font(.footnote).foregroundStyle(.secondary)
        }.card()
    }
}
...
private final class SecureLinkController: NSObject, ObservableObject {
    ...
    struct Event: Codable {
        ...
        /// Exact JSON-safe representation of the Tuya SDK callback dictionary when present.
        /// This is SDK-decoded application data, NOT raw FD50 transport bytes.
        let applicationPayloadJSON: String?
    }
    ...
    struct Export: Codable {
        ...
        let applicationReportCount: Int
        let payloadAuthority: String
    ...
    @Published private(set) var applicationReportCount = 0
    ...
    var age: Double? {
        guard let a = authUptime else { return nil }
        let n = DispatchTime.now().uptimeNanoseconds
        return n >= a ? Double(n - a) / 1e9 : nil
    }
    var passed: Bool { secure && applicationReportCount > 0 && (age ?? 0) > 45 }
    ...
    func stopScan() {
        central.stopScan()
        if selectedID == nil, let c = candidates.first, c.likely { choose(c) }
        if selectedID == nil { message = "No candidate has enough scooter/Tuya evidence yet." }
        log("scan_stopped")
    }

    func choose(_ c: Candidate) {
        central.stopScan()
        selectedID = c.id
        phase = .selected
        message = c.likely ? "Likely scooter selected automatically from evidence." : "Evidence is not strong enough; re-scan instead of guessing."
        log("candidate_selected", ["id": c.id.uuidString, "score": String(c.score), "evidence": c.evidence.joined(separator: ",")])
    }

    func authenticate() {
        guard let c = selected, c.likely else { fail("A strongly matched scooter candidate is required.", "candidate_not_confident"); return }
        guard !deviceID.isEmpty, !tuyaUUID.isEmpty, !productID.isEmpty else { fail("Tuya device identity is incomplete.", "tuya_identity_incomplete"); return }
        ...
        message = "Tuya is establishing its supported secure BLE session. Nembra sends no DP query or control command."
        log("official_connect_requested", ["correlatedCoreBluetoothID": c.id.uuidString, "tuyaDeviceID": deviceID, "tuyaUUID": tuyaUUID, "productID": productID])

        d.connect(
            deviceID: deviceID,
            uuid: tuyaUUID,
            productID: productID,
            applicationReport: { [weak self] dps in
                Task { @MainActor in self?.receivedApplicationReport(dps) }
            },
            success: { [weak self] in
                Task { @MainActor in self?.authenticated() }
            },
            failure: { [weak self] m in
                Task { @MainActor in self?.fail(m, "official_connect_failed") }
            }
        )
    }
    ...
    private func authenticated() {
        ...
        phase = .observing
        message = "Secure scooter link established. Waiting for a Tuya SDK application report on this same session…"
        log("official_session_ready", ["evidenceSource": "ThingSmartBLEManager + ThingSmartDeviceDelegate"])
        startWatchdog()
    }

    private func receivedApplicationReport(_ dps: [AnyHashable: Any]) {
        guard secure, phase == .observing || phase == .accepted else { return }
        guard !dps.isEmpty else { return }
        applicationReportCount += 1
        let payload = Self.canonicalApplicationJSON(dps)
        log("tuya_sdk_dps_update", ["dpCount": String(dps.count)], applicationPayloadJSON: payload)
        if passed {
            phase = .accepted
            message = "Receiving scooter application data. Secure link passed the >45-second gate."
        } else {
            message = "Receiving scooter application data · \(applicationReportCount) report(s). Keep it stationary until >45 s."
        }
    }
    ...
    private func startWatchdog() {
        ...
        watchdog = Task { @MainActor [weak self] in
            while !Task.isCancelled {
                ...
                if self.passed {
                    self.phase = .accepted
                    self.message = "Receiving scooter application data. Secure link passed the >45-second gate."
                    self.log("acceptance_passed", ["applicationReports": String(self.applicationReportCount)])
                    return
                }
                if (self.age ?? 0) > 60, self.applicationReportCount == 0 {
                    self.fail("The official secure session survived, but Tuya delivered no application DP report within 60 seconds. Export diagnostics; do not repeat the ride capture.", "no_application_reports")
                    return
                }
    ...
    func prepareExport() {
        let x = Export(
            ...
            applicationReportCount: applicationReportCount,
            payloadAuthority: "tuya-sdk-decoded-dps-update",
        ...
        do {
            ...
            exportData = try e.encode(x)
            exportName = "Nembra-Secure-Link-\(deviceID.prefix(8))-Diagnostics.json"
            message = "Sanitized diagnostics ready. SDK DP reports are preserved exactly as JSON-safe values; passwords, tokens, local_key and AppSecret are excluded. Raw FD50 bytes are not claimed."
        } catch {
    ...
    private func fail(_ m: String, _ kind: String) {
        watchdog?.cancel()
        watchdog = nil
        phase = .failed
        message = m
        log(kind, ["message": sanitize(m)])
    }

    private func log(_ kind: String, _ details: [String: String] = [:], applicationPayloadJSON: String? = nil) {
        events.append(Event(at: Date(), kind: kind, details: details.mapValues(sanitize), applicationPayloadJSON: applicationPayloadJSON.map(sanitize)))
    }

    private func sanitize(_ s: String) -> String {
        var r = s
        for k in ["NEMBRA_TUYA_APP_KEY", "NEMBRA_TUYA_APP_SECRET"] {
            if let v = ProcessInfo.processInfo.environment[k], !v.isEmpty { r = r.replacingOccurrences(of: v, with: "<redacted>") }
        }
        return r
    }

    private static func canonicalApplicationJSON(_ dps: [AnyHashable: Any]) -> String {
        let normalized = Dictionary(uniqueKeysWithValues: dps.map { (String(describing: $0.key), jsonSafe($0.value)) })
    ...
    private static func jsonSafe(_ value: Any) -> Any {
        ...
        case let v as Data: return ["encoding": "base64", "value": v.base64EncodedString()]
        case let v as [Any]: return v.map(jsonSafe)
        case let v as [AnyHashable: Any]: return Dictionary(uniqueKeysWithValues: v.map { (String(describing: $0.key), jsonSafe($0.value)) })
        default: return String(describing: value)
        }
    }

    private static func tuyaCompany(_ d: Data?) -> Bool {
        guard let d, d.count >= 2 else { return false }
        return (UInt16(d[d.startIndex]) | UInt16(d[d.index(after: d.startIndex)]) << 8) == 0x07D0
    }
    ...
    private func update(_ p: CBPeripheral, _ ad: [String: Any], _ n: NSNumber) {
        ...
        let old = byID[id]
        let name = (ad[CBAdvertisementDataLocalNameKey] as? String) ?? p.name ?? old?.name
        let r = n.intValue == 127 ? old?.rssi : n.intValue
        ...
        let sd = ad[CBAdvertisementDataServiceDataKey] as? [CBUUID: Data]
        let fd = uuids.contains(Self.fd50) || sd?.keys.contains(Self.fd50) == true || old?.fd50 == true
        let co = Self.tuyaCompany(ad[CBAdvertisementDataManufacturerDataKey] as? Data) || old?.tuyaCompany == true
        let kn = id == Self.known
        let nw = (phase == .scanning && !baseline.contains(id)) || old?.newAfterPowerOn == true
        ...
        var score = 0
        var ev: [String] = []
        if kn { score += 1000; ev.append("known previous UUID") }
        if fd { score += 500; ev.append("FD50") }
        if co { score += 350; ev.append("Tuya company 0x07D0") }
        if nw { score += 180; ev.append("appeared after power-on") }
        if nm { score += 100; ev.append("expected name") }
        if let r {
            if r >= -50 { score += 80; ev.append("very close RSSI") }
            else if r >= -65 { score += 50; ev.append("nearby RSSI") }
...
private protocol OfficialTuyaDriver: AnyObject {
    func connect(
        ...
        uuid: String,
        productID: String,
        applicationReport: @escaping ([AnyHashable: Any]) -> Void,
...
private final class SmartLifeDriver: NSObject, OfficialTuyaDriver, ThingSmartDeviceDelegate {
    private var observedDevice: ThingSmartDevice?
    private var onApplicationReport: (([AnyHashable: Any]) -> Void)?
    ...
    func connect(
        ...
        guard let key = env["NEMBRA_TUYA_APP_KEY"], !key.isEmpty,
              let secret = env["NEMBRA_TUYA_APP_SECRET"], !secret.isEmpty else {
        ...
        onApplicationReport = applicationReport
...
private struct SecureLinkView: View {
        ...
        TimelineView(.periodic(from: .now, by: 0.5)) { _ in
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    sdk
                    discovery
                    if let c = test.selected { selected(c) }
                    acceptance
                    export
                }.frame(maxWidth: 760).padding(18).frame(maxWidth: .infinity)
    ...
    private var status: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                Spacer()
                Text("\(test.applicationReportCount)").monospacedDigit()
            }
            Text(test.message).font(.footnote).foregroundStyle(.secondary)
            if let a = test.age {
                LabeledContent("Secure-session age", value: String(format: "%.1f s", a))
                ProgressView(value: min(a / 45, 1))
            }
            LabeledContent("Tuya application reports", value: String(test.applicationReportCount))
        }.card()
    }
    ...
    private var discovery: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            case .powerOn:
                ...
                Button("Scan after power-on") { test.scanOn() }.buttonStyle(.borderedProminent)
            case .scanning:
                Button("Stop scan / use best evidence") { test.stopScan() }.buttonStyle(.bordered)
            default:
                EmptyView()
            }
            ForEach(test.candidates.prefix(8)) { c in
                Button { test.choose(c) } label: {
                    VStack(alignment: .leading, spacing: 4) {
                        HStack {
                            Text(c.title).bold()
                            if c.likely { Text("LIKELY SCOOTER").font(.caption2.bold()).padding(.horizontal, 6).padding(.vertical, 2).background(.green, in: Capsule()).foregroundStyle(.black) }
                            Spacer()
                            Text("\(c.score)").monospacedDigit()
                        }
                        Text("\(c.rssi.map { String($0) + " dBm" } ?? "RSSI ?") · \(c.id.uuidString)").font(.caption2).foregroundStyle(.secondary).lineLimit(1)
                        Text(c.evidence.joined(separator: " · ")).font(.caption).foregroundStyle(.secondary)
                    }
                }.buttonStyle(.plain)
            }
        }.card()
    ...
    private func selected(_ c: SecureLinkController.Candidate) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Label("Authentication gate", systemImage: "key.horizontal").font(.headline)
            Text(c.evidence.joined(separator: " · ")).font(.footnote).foregroundStyle(.secondary)
            Button("Start secure read-only test") { test.authenticate() }.buttonStyle(.borderedProminent).disabled(!c.likely || !test.sdkCompiled || !test.privateConfig || !test.sdkAccountAuthorized || [.authenticating, .observing, .accepted].contains(test.phase))
        }.card()
    }
    ...
    private var acceptance: some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            Text("Pass only when Tuya's official session succeeds, it survives >45 seconds, and the Tuya SDK delivers at least one genuine application DP report on that same SDK-owned device session. CoreBluetooth discovery bytes cannot satisfy this gate.").font(.footnote).foregroundStyle(.secondary)
            Text("Raw FD50 transport bytes are not captured by this authority path and are never claimed as evidence.").font(.footnote.bold()).foregroundStyle(.secondary)
    ...
    private var export: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let d = test.exportData {
                ShareLink(item: SecureTransfer(data: d, name: test.exportName), preview: SharePreview(test.exportName)) {
                    Label("Share diagnostic JSON", systemImage: "square.and.arrow.up")
                }.buttonStyle(.borderedProminent)
            }
            Text("Export includes correlation evidence, monotonic session timing, failures, and exact JSON-safe Tuya SDK DP callback payloads. It excludes passwords, tokens, local_key, and AppSecret and explicitly records rawFD50BytesCaptured=false.").font(.footnote).foregroundStyle(.secondary)
        }.card()
    }
}
