        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    Text("The next physical run is stationary. It proves supported Tuya authentication, >45 s observed local-BLE survival, and real application updates through Tuya's own SDK. The old 17-step ride sequence stays disabled until this passes.").foregroundStyle(.secondary)
...
private final class SecureLinkController: NSObject, ObservableObject {
    ...
    static let acceptedObservationNanoseconds: UInt64 = 45_000_000_000
    ...
    private var localBLEObservedAtUptime: UInt64?
    ...
    var secureSessionAgeSeconds: Double? {
        guard let start = localBLEObservedAtUptime else { return nil }
    ...
    var passed: Bool {
        guard (phase == .observing || phase == .accepted),
              ...
              let start = localBLEObservedAtUptime else { return false }
        let now = DispatchTime.now().uptimeNanoseconds
        return now >= start && now - start >= Self.acceptedObservationNanoseconds
    }
    ...
    func scanAfterPowerOn() {
        guard central.state == .poweredOn else { fail("Bluetooth is not ready.", "bluetooth_unavailable"); return }
        phase = .scanning
        message = "Looking for the prior physical UUID or corroborating FD50 + Tuya company evidence. Name/RSSI remain descriptive only."
    ...
    func choose(_ candidate: Candidate) {
        guard candidate.likely else {
            message = "That candidate does not have enough deterministic target evidence. Name or RSSI alone cannot authorize it."
            return
        }
        central.stopScan()
    ...
    func authenticate() {
        ...
        newDriver.connect(
            ...
            success: { [weak self] in Task { @MainActor in self?.connectCallbackSucceeded() } },
            failure: { [weak self] error in Task { @MainActor in self?.fail(error, "official_connect_failed") } }
        )
    }

    private func connectCallbackSucceeded() {
        ...
        localBLEObservedAtUptime = nil
        sdkLocalBLEOnline = false
        applicationUpdateCount = 0
        phase = .observing
        message = "Official connect callback returned. Waiting for Tuya's local-BLE status to become online before starting the observation clock."
        log("official_connect_success_callback")
        refreshLocalBLEObservation()
        startWatchdog()
    }

    private func refreshLocalBLEObservation() {
        ...
        if online && !sdkLocalBLEOnline {
            ...
            localBLEObservedAtUptime = DispatchTime.now().uptimeNanoseconds
            applicationUpdateCount = 0
            log("sdk_local_ble_online_observed")
            message = "Tuya local BLE is online. The 45-second observed-survival window starts now; waiting for a current-window application update."
    ...
    private func receivedApplicationUpdate(_ update: [String: String]) {
        ...
        guard sdkLocalBLEOnline, localBLEObservedAtUptime != nil else {
            log("tuya_application_update_before_local_ble_gate", update)
            return
        }
        applicationUpdateCount += 1
        log("tuya_current_window_application_update", update)
        if passed {
            acceptIfReady()
        } else {
            message = "Receiving current-window scooter application data · \(applicationUpdateCount) update(s). Keep it stationary until the 45-second observed-survival gate passes."
        }
    }
    ...
    private func startWatchdog() {
        ...
        watchdog = Task { @MainActor [weak self] in
            while !Task.isCancelled {
                ...
                self.refreshLocalBLEObservation()
                ...
                let now = DispatchTime.now().uptimeNanoseconds
                if self.localBLEObservedAtUptime == nil,
                ...
                if let localStart = self.localBLEObservedAtUptime,
                   now >= localStart,
                   now - localStart >= 60_000_000_000,
                   self.applicationUpdateCount == 0 {
                    self.fail("Tuya local BLE remained observed online through the 60-second window, but no current-window application update arrived.", "no_current_window_application_updates")
                    return
                }
    ...
    private func acceptIfReady() {
        ...
        phase = .accepted
        message = "Secure scooter link passed. Tuya remained observed locally online beyond the old ~30-second rejection window and delivered current-window application data."
    ...
    func prepareExport() {
        ...
        do {
            ...
            message = "Sanitized diagnostics ready. Passwords, verification codes, account tokens, local_key and AppSecret are excluded."
        } catch {
...
private final class OfficialTuyaAccountAuthorizer: ObservableObject {
    ...
    private func finishLoginSuccess() {
        ...
        verificationCode = ""
        authorized = true
        status = "Official Tuya SDK account authorized. The secure read-only scooter test can now proceed."
    }
                ...
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        if let age = test.secureSessionAgeSeconds {
                            LabeledContent("Observed local-BLE age", value: String(format: "%.1f s", age))
                            ProgressView(value: min(age / 45, 1))
                        }
                        LabeledContent("Tuya local BLE", value: test.sdkLocalBLEOnline ? "Online" : "Not proven")
                        LabeledContent("Current-window application updates", value: String(test.applicationUpdateCount))
                    }.card()
                    ...
                    VStack(alignment: .leading, spacing: 7) {
                        ...
                        Text("Pass only when Tuya's official SDK is observed locally online for at least 45 monotonic seconds without an observed offline sample, and at least one application update arrives after that local-BLE window begins. This proves survival beyond the prior ~30-second rejection pattern; Nembra still assigns no DP meaning here.").font(.footnote).foregroundStyle(.secondary)
                        if test.passed { Text("Secure scooter link established\nReceiving current-window scooter data").font(.title3.bold()).foregroundStyle(.green) }
                    }.card()
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Text("Export includes deterministic target evidence, observed local-BLE window state, failures and opaque application-update values. It excludes passwords, verification codes, account tokens, local_key and AppSecret.").font(.footnote).foregroundStyle(.secondary)
                    }.card()
                }
                .frame(maxWidth: 760)
                .padding(18)
