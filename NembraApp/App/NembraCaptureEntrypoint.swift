...
private final class SecureLinkController: NSObject, ObservableObject {
    ...
    func consumeCorrelationAsyncInvalidation() {
        ...
        failLocally(
            "Bluetooth correlation ended before this window could be sealed. Restart from OFF1; completed windows from this attempt are not reused.",
...
private struct SecureLinkView: View {
    ...
    var body: some View {
        TimelineView(.periodic(from: .now, by: 0.5)) { _ in
            ...
            .background(Color.black.ignoresSafeArea())
            .onChange(of: test.correlationProgress?.isSeriesInvalidated) { _, _ in
