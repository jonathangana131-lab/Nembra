    ...
    private var account: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if tuya.phase == .failed {
                Button("Reset account link") { tuya.resetLink() }.buttonStyle(.bordered)
            }
        }
        .card()
    }
...
private final class SecureLinkController: NSObject, ObservableObject {
    ...
    struct Export: Codable {
        ...
        let authenticatedContinuityActive: Bool
    ...
    private var authEndUptime: UInt64?
    ...
    var age: Double? {
        guard let start = authUptime else { return nil }
        let end = authEndUptime ?? DispatchTime.now().uptimeNanoseconds
        guard end >= start else { return nil }
        return Double(end - start) / 1e9
    }

    private var acceptanceEligible: Bool {
        phase == .observing && secure && continuityActive && packetCount > 0 && (age ?? 0) >= 45
    }
    ...
    func choose(_ candidate: Candidate) {
        ...
        phase = .selected
        message = candidate.likely ? "Likely scooter selected automatically from evidence." : "Evidence is not strong enough; re-scan instead of guessing."
    ...
    func authenticate() {
        guard let peripheral = selectedPeripheral, let candidate = selected, candidate.likely else {
        ...
        provider.connect(
            uuid: tuyaUUID,
            productID: productID,
            success: { [weak self] in Task { @MainActor in self?.authenticated() } },
            failure: { [weak self] value in Task { @MainActor in self?.fail(value, "official_connect_failed") } }
        )
    }
    ...
    private func startWatchdog() {
        ...
        watchdog = Task { @MainActor [weak self] in
            while !Task.isCancelled {
                guard let self, self.phase == .observing, self.continuityActive else { return }
                if self.acceptanceEligible {
                    self.acceptCurrentHorizon()
                    return
                }
                if (self.age ?? 0) > 60 && self.packetCount == 0 {
                    self.fail("Secure session survived, but no post-auth FD50 notification arrived within 60 seconds.", "no_notifications")
                    return
                }
    ...
    private func acceptCurrentHorizon() {
        guard acceptanceEligible else { return }
        let acceptedPackets = packetCount
        freezeContinuity()
        phase = .accepted
        message = "Receiving scooter data. Secure link passed the 45-second gate."
        log("acceptance_passed", [
            "packets": String(acceptedPackets),
            "durationSeconds": String(format: "%.3f", age ?? 0)
        ])
    }
    ...
    func prepareExport() {
        let export = Export(
            ...
            authenticatedContinuityActive: continuityActive,
    ...
    private func freezeContinuity() {
        if continuityActive, authEndUptime == nil {
            authEndUptime = DispatchTime.now().uptimeNanoseconds
        }
    ...
    private func update(_ peripheral: CBPeripheral, _ advertisement: [String: Any], _ number: NSNumber) {
        ...
        let hasTuyaCompany = Self.tuyaCompany(advertisement[CBAdvertisementDataManufacturerDataKey] as? Data) || old?.tuyaCompany == true
...
extension SecureLinkController: @preconcurrency CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        guard acceptRaw, continuityActive, selectedID == peripheral.identifier else { return }
    ...
    func centralManager(_ central: CBCentralManager, didFailToConnect peripheral: CBPeripheral, error: Error?) {
        guard selectedID == peripheral.identifier, continuityActive else { return }
        fail("Passive observer could not attach: \(error?.localizedDescription ?? "unknown")", "observer_connect_failed")
    }
    ...
    func centralManager(_ central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        ...
        if continuityActive || secure {
            fail("Secure link disconnected before acceptance. Export diagnostics; do not repeat the ride capture.", "disconnect_before_acceptance")
        }
    }
}
...
extension SecureLinkController: @preconcurrency CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateNotificationStateFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if characteristic.isNotifying, continuityActive {
            message = "Secure scooter link established. Waiting for post-auth FD50 bytes…"
        }
    }

    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        guard acceptRaw, continuityActive, phase == .observing, characteristic.uuid == Self.notify, characteristic.isNotifying else { return }
        if let error {
...
private struct SecureLinkView: View {
    ...
    private var status: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                Spacer()
                Text("\(test.packetCount)").monospacedDigit()
            }
    ...
    private var export: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Export includes candidate evidence, frozen authenticated timing, failures, and accepted post-auth raw bytes; it excludes passwords, tokens, local_key, and AppSecret.")
                .font(.footnote)
                .foregroundStyle(.secondary)
        }
        .card()
