...
private final class SecureLinkController: NSObject, ObservableObject {
    ...
    var secureSessionEstablished: Bool {
        if case .authenticated = preflightSnapshot.authenticationState { return true }
        return false
    }
    ...
    var secureSessionAgeSeconds: Double? {
        guard let authenticatedAt = preflightSnapshot.authenticatedAtUptimeNanoseconds,
              ...
              latest >= authenticatedAt else { return nil }
        return Double(latest - authenticatedAt) / 1_000_000_000
    }

    private var elapsedSinceAuthenticationNow: Double? {
        guard let authenticatedAt = preflightSnapshot.authenticatedAtUptimeNanoseconds else { return nil }
        let now = DispatchTime.now().uptimeNanoseconds
        guard now >= authenticatedAt else { return nil }
        return Double(now - authenticatedAt) / 1_000_000_000
    }
    ...
    func authenticate() {
        ...
        guard sdkAccountAuthorized else {
            stopWithFailure("The official Tuya SDK has no authorized account session. The metadata QR session cannot substitute for SDK login.", "sdk_account_not_authorized")
            return
        }
        ...
        Task { @MainActor [weak self] in
            ...
            do {
                ...
                stopWithFailure("Could not start the sealed authentication attempt.", "authority_begin_failed")
            }
        }
    }
    ...
    private func authenticationFailed(_ text: String, token: TuyaReadOnlyConnectionToken) async {
        ...
        do {
            try await authority.markAuthenticationFailed(for: token)
        } catch {
            log("auth_failure_authority_rejected", ["error": String(describing: error)])
        }
        currentToken = nil
        await refreshPreflight()
    ...
    private func receivedApplicationUpdate(
        ...
        if passed {
            phase = .accepted
            message = "Secure scooter link passed. Canonical authority accepted current-generation Tuya application data after the full stability window."
        } else {
        ...
        watchdog = Task { @MainActor [weak self] in
            while !Task.isCancelled {
                ...
                if !self.sdkLocalBLEOnline {
                    if (self.elapsedSinceAuthenticationNow ?? 0) > 2 {
    ...
    private func invalidateCurrentAttempt(
        ...
        do {
            try await authority.invalidateAttempt(for: token)
        } catch {
            log("attempt_invalidation_authority_rejected", ["error": String(describing: error)])
        }
        currentToken = nil
        await refreshPreflight()
    ...
    func prepareExport() {
        ...
        case .readyForStationaryMapping:
            verdictName = "readyForStationaryMapping"
            blockReason = nil
        ...
        let envelope = Export(
            ...
            canonicalVerdict: verdictName,
            canonicalBlockReason: blockReason,
...
private struct SecureLinkView: View {
    ...
    private var acceptanceCard: some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            Text("PASS comes only from Nembra's canonical authenticated-session authority: current generation, accepted SmartLife provenance, a genuine non-empty decoded application update, valid monotonic chronology, and at least 45 seconds of SDK-local continuity. No DP meaning is inferred here.")
                .font(.footnote)
                .foregroundStyle(.secondary)
            if test.passed {
    ...
    private var exportCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Export includes candidate evidence, canonical generation/verdict/chronology, SDK-local status, failures and opaque decoded application values. It excludes passwords, verification codes, account tokens, local_key and AppSecret.")
                .font(.footnote)
                .foregroundStyle(.secondary)
        }
        .card()
