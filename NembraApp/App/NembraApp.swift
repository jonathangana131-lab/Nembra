                ...
                NavigationStack {
                    if PassiveBluetoothExperimentOneFieldExecutionGate.permitsPhysicalProcedure {
                        if let researchCoordinator {
                            // The shell still consumes the coordinator-owned controller directly at
                            // this checkpoint. Correlation/session migration remains a separate app
                            // rung, but launch authority can no longer bypass the package field gate
                            // or mint an unrelated generic ES80 controller.
                            ES80CaptureShellView(controller: researchCoordinator.controller)
                        } else {
                            ContentUnavailableView(
                                "Capture unavailable",
                                systemImage: "antenna.radiowaves.left.and.right.slash",
                                description: Text("The authorized Experiment One coordinator could not be created.")
                            )
