...
Fresh workers must inspect live GitHub before trusting any mutable head. At this revision, the active P0 convergence subject is **PR #3868**, branch `capture/auth-gate-current-main-convergence-sol`, targeting `main`.

PR #3868 was reconstructed on current-main lineage after PR #3852 restored the strict C7D09A22 physical-truth boundary. Its head is intentionally **not pinned here** because any follow-up commit invalidates older exact-head acceptance. Read the live PR head and its workflows directly.
...
PR #3868 must first reach terminal exact-head software acceptance on its **unchanged live head**. At minimum, treat the PR-triggered Capture standalone and Xcode 27 exact-head gates as evidence only when they are terminal green for that exact head. Any head movement makes earlier runs stale.
