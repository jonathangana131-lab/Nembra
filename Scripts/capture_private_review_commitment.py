...
def create_commitment(*, witness: Path, key_path: Path) -> str:
    _, witness_bytes = _require_private_regular(
        witness,
        exact_size=None,
        max_size=MAX_WITNESS_BYTES,
    )
    key = _write_fresh_key(key_path)
    # Re-read both accepted local subjects after key publication. The returned
    # public tag is authoritative only for one stable post-publication pair.
    _, stable_witness = _require_private_regular(
        witness,
        exact_size=None,
        max_size=MAX_WITNESS_BYTES,
    )
    _, stable_key = _require_private_regular(
        key_path,
...
def verify_commitment(*, witness: Path, key_path: Path, expected_tag: str) -> str:
    ...
    _, key = _require_private_regular(
        key_path,
        exact_size=KEY_BYTES,
        max_size=KEY_BYTES,
    )
    ...
    # Re-read after comparison so a same-UID replacement at the verification
    # boundary cannot survive as an accepted stable generation.
    _, final_witness = _require_private_regular(
        witness,
        exact_size=None,
        max_size=MAX_WITNESS_BYTES,
    )
    _, final_key = _require_private_regular(
        key_path,
        exact_size=KEY_BYTES,
        max_size=KEY_BYTES,
    )
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    try:
        if arguments.mode == "create":
            ...
            value = verify_commitment(
                witness=arguments.witness,
                key_path=arguments.key,
                expected_tag=arguments.expected,
            )
