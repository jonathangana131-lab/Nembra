...
# Bind the checkout root immediately after canonical resolution, before any
# mutable path beneath that root (including the writer) is read. Otherwise a
# same-UID process could swap the checkout after trusted writer capture but
# before root admission and make the replacement tree become the accepted
# credential destination. The writer later opens the root descriptor itself and
# must prove it still has this exact pre-credential device/inode identity.
[[ -x /usr/bin/python3 ]] || {
  builtin printf '%s\n' 'ERROR: system Python 3 is required for checkout-root custody.' >&2
  exit 4
}
...
WRITER_SHA256="868d9ee4393b24b2194927fb83971ea862502adf8a800241a40f64abd30a82c2"
...
[[ -x /usr/bin/shasum && -x /usr/bin/awk ]] || {
  unset ROOT_DEVICE ROOT_INODE
  builtin printf '%s\n' 'ERROR: SHA-256 tooling is required for private identity publication.' >&2
  exit 4
}
...
[[ "$CAPTURED_WRITER_SHA256" == "$WRITER_SHA256" ]] || {
  unset WRITER_SOURCE CAPTURED_WRITER_SHA256 ROOT_DEVICE ROOT_INODE
...
The checkout root is admitted before writer path use; writer bytes are SHA-256 pinned before credential input; filesystem publication is descriptor-bound and no-follow under that exact admitted root.
