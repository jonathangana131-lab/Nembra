...
# Direct privileged Bash suppresses BASH_ENV/imported startup functions. Close
# xtrace and executable lookup before reading or locating private values.
set +x
PATH="/usr/bin:/bin:/usr/sbin:/sbin"
export PATH
unset BASH_ENV ENV CDPATH GLOBIGNORE SHELLOPTS || true
ROOT="$(cd "$(/usr/bin/dirname "${BASH_SOURCE[0]}")/.." && /bin/pwd -P)"
LOCAL_SECRETS="$ROOT/LocalSecrets"
# NEMBRA_TUYA_RUNTIME_DIR is intentionally ignored. Credential output is fixed
# beneath this checkout's ignored LocalSecrets tree.
...
Private Tuya app identity is ready in this checkout's ignored LocalSecrets tree.
No plaintext credential was written to Git, shell history, process arguments, or output.
