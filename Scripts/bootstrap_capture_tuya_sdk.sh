...
COCOAPODS_SUBJECT_HELPER="$SCRIPT_DIR/capture_cocoapods_build_subject.py"
REVIEW_ONLY=0
...
if [[ ! -f "$COCOAPODS_SUBJECT_HELPER" ]]; then
...
if [[ ! -d Pods ]]; then
  echo "ERROR: CocoaPods did not create Pods; exact generated build-subject provenance is unavailable." >&2
  exit 9
fi
...
COCOAPODS_BUILD_SHA256="$(/usr/bin/python3 -I "$COCOAPODS_SUBJECT_HELPER" --pods "$REPO_ROOT/Pods" --workspace "$REPO_ROOT/NembraCapture.xcworkspace")" || {
  echo "ERROR: could not compute the CocoaPods generated build-subject SHA-256 fingerprint." >&2
  exit 13
}
[[ "$COCOAPODS_BUILD_SHA256" =~ ^[0-9a-f]{64}$ ]] || {
  echo "ERROR: CocoaPods generated build-subject helper returned a malformed fingerprint." >&2
  exit 13
}
...
normal bootstrap/installer with those accepted digests supplied as
...
[[ "$ACCEPTED_COCOAPODS_BUILD_SHA256" =~ ^[0-9a-f]{64}$ ]] || {
  echo "ERROR: NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_BUILD_SHA256 must be the preaccepted 64-hex generated build-subject SHA-256. Stop before xcodebuild/install." >&2
  exit 17
}
[[ "$COCOAPODS_BUILD_SHA256" == "$ACCEPTED_COCOAPODS_BUILD_SHA256" ]] || {
  echo "ERROR: CocoaPods generated build inputs do not match the preaccepted generated build subject. Stop before xcodebuild/install and review the changed generated graph/toolchain output." >&2
  exit 18
}
...
unset ACCEPTED_LOCK_SHA256 ACCEPTED_COCOAPODS_BUILD_SHA256 \
...
NEXT BUILD RULE:
  ...
  Preserve the exact reviewed CocoaPods generated build subject through xcodebuild.
  Do not run 'pod update', replace ThingSmartCryption, regenerate the private
  identity, or change CocoaPods-generated build bytes before an accepted physical
  capture; any input change is a new reviewed field-build candidate and must earn
  a new exact-head acceptance.
