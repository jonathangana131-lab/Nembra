...
GENERATED_MANIFEST_SHA256="$(
  /usr/bin/python3 -I "$ACCEPTED_BUILD_INPUT_HELPER" manifest     --root "$REPO_ROOT"     --source-sha "$SOURCE_SHA" |     /usr/bin/shasum -a 256 | /usr/bin/awk '{print $1}' | /usr/bin/tr '[:upper:]' '[:lower:]'
)" || {
