...
GENERATED_SUBJECT_HELPER="$SCRIPT_DIR/capture_cocoapods_generated_build_subject.py"
REVIEW_ONLY=0
...
if [[ "$REVIEW_ONLY" == "0" ]]; then
  : "${NEMBRA_CAPTURE_ACCEPTED_TUYA_LOCK_SHA256:?Set NEMBRA_CAPTURE_ACCEPTED_TUYA_LOCK_SHA256 to the preaccepted 64-hex Podfile.lock SHA-256 before field bootstrap. To create review candidates without build authority, use --resolve-lock-for-review.}"
  : "${NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_BUILD_SHA256:?Set NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_BUILD_SHA256 to the preaccepted 64-hex generated CocoaPods build-subject SHA-256 before field bootstrap. To create review candidates without build authority, use --resolve-lock-for-review.}"
  ...
  ACCEPTED_COCOAPODS_BUILD_SHA256="$(printf '%s' "$NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_BUILD_SHA256" | tr '[:upper:]' '[:lower:]')"
else
  unset NEMBRA_CAPTURE_ACCEPTED_TUYA_LOCK_SHA256 NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_BUILD_SHA256 || true
  ACCEPTED_LOCK_SHA256=""
  ACCEPTED_COCOAPODS_BUILD_SHA256=""
fi
...
if [[ ! -d Pods ]]; then
  echo "ERROR: CocoaPods did not create Pods; exact generated build-subject authority is unavailable." >&2
  exit 9
fi
...
# Snapshot every private ignored input that can materially change the field
...
if ! COCOAPODS_BUILD_SHA256="$(/usr/bin/python3 -I "$GENERATED_SUBJECT_HELPER" digest \
  --lockfile "$REPO_ROOT/Podfile.lock" \
  --pods "$REPO_ROOT/Pods" \
...
[[ "$COCOAPODS_BUILD_SHA256" =~ ^[0-9a-f]{64}$ ]] || {
  echo "ERROR: generated CocoaPods build-subject helper returned a malformed digest." >&2
  exit 13
}
...
DEPENDENCY + GENERATED BUILD CANDIDATE ONLY — NOT FIELD BUILD AUTHORITY
  ...
  CocoaPods generated build-subject SHA-256: $COCOAPODS_BUILD_SHA256
...
[[ "$COCOAPODS_BUILD_SHA256" == "$ACCEPTED_COCOAPODS_BUILD_SHA256" ]] || {
  echo "ERROR: CocoaPods generated build inputs do not match the preaccepted build-subject SHA-256. Stop before xcodebuild/install; the reviewed lock alone cannot authorize changed generated build bytes." >&2
  exit 17
}
printf 'Preaccepted Tuya dependency lock matched: %s\n' "$LOCK_SHA256"
printf 'Preaccepted CocoaPods generated build subject matched: %s\n' "$COCOAPODS_BUILD_SHA256"
unset ACCEPTED_LOCK_SHA256 ACCEPTED_COCOAPODS_BUILD_SHA256 || true
...
NEXT BUILD RULE:
  ...
  Preserve the accepted generated-build digest through the xcodebuild custody
  window; any generated workspace/Pods mutation requires a new reviewed subject.
