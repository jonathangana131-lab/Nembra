...
if [[ "$REVIEW_ONLY" == "0" ]]; then
  ...
  : "${NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_SUBJECT_SHA256:?Set NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_SUBJECT_SHA256 to the preaccepted 64-hex generated CocoaPods build-subject SHA-256 before field bootstrap. To create a candidate dependency subject without build authority, use --resolve-lock-for-review.}"
  ...
  [[ "$NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_SUBJECT_SHA256" =~ ^[0-9A-Fa-f]{64}$ ]] || {
    echo "ERROR: NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_SUBJECT_SHA256 must be exactly 64 hex characters." >&2
    exit 1
  }
  ACCEPTED_LOCK_SHA256="$(printf '%s' "$NEMBRA_CAPTURE_ACCEPTED_TUYA_LOCK_SHA256" | tr '[:upper:]' '[:lower:]')"
  ACCEPTED_COCOAPODS_SUBJECT_SHA256="$(printf '%s' "$NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_SUBJECT_SHA256" | tr '[:upper:]' '[:lower:]')"
else
  unset NEMBRA_CAPTURE_ACCEPTED_TUYA_LOCK_SHA256 || true
  unset NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_SUBJECT_SHA256 || true
  ACCEPTED_LOCK_SHA256=""
  ACCEPTED_COCOAPODS_SUBJECT_SHA256=""
fi
...
if [[ ! -d Pods ]]; then
  echo "ERROR: CocoaPods did not create Pods; generated build-subject custody is unavailable." >&2
  exit 9
fi
...
if ! COCOAPODS_SUBJECT_SHA256="$(/usr/bin/python3 -I "$GENERATED_SUBJECT_HELPER" fingerprint \
  --repository-root "$REPO_ROOT" \
  --pods "$REPO_ROOT/Pods" \
  --workspace "$REPO_ROOT/NembraCapture.xcworkspace")"
then
  echo "ERROR: generated CocoaPods build subject could not be fingerprinted safely." >&2
  exit 17
fi
[[ "$COCOAPODS_SUBJECT_SHA256" =~ ^[0-9a-f]{64}$ ]] || {
  echo "ERROR: generated CocoaPods build-subject helper did not produce one lowercase SHA-256 digest." >&2
  exit 17
}
...
DEPENDENCY + GENERATED BUILD SUBJECT CANDIDATE ONLY — NOT FIELD BUILD AUTHORITY
  ...
  CocoaPods generated-subject SHA-256: $COCOAPODS_SUBJECT_SHA256
...
normal bootstrap/installer with both accepted digests supplied as
NEMBRA_CAPTURE_ACCEPTED_TUYA_LOCK_SHA256 and
NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_SUBJECT_SHA256. This review-only mode never
...
[[ "$COCOAPODS_SUBJECT_SHA256" == "$ACCEPTED_COCOAPODS_SUBJECT_SHA256" ]] || {
  echo "ERROR: generated CocoaPods build subject does not match the preaccepted generated-subject SHA-256. Stop before xcodebuild/install and review the exact generated build inputs." >&2
  exit 17
}
printf 'Preaccepted Tuya dependency lock matched: %s\n' "$LOCK_SHA256"
printf 'Preaccepted CocoaPods generated build subject matched: %s\n' "$COCOAPODS_SUBJECT_SHA256"
unset ACCEPTED_LOCK_SHA256 NEMBRA_CAPTURE_ACCEPTED_TUYA_LOCK_SHA256 || true
unset ACCEPTED_COCOAPODS_SUBJECT_SHA256 NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_SUBJECT_SHA256 || true
...
NEXT BUILD RULE:
  ...
  Preserve this exact private-input fingerprint record and generated-subject
  digest with the field workspace. Do not run 'pod update', replace
  ThingSmartCryption, regenerate the private identity, or admit regenerated
  CocoaPods build bytes before an accepted physical capture; any input change is
  a new reviewed field-build candidate and must earn a new exact-head acceptance.
