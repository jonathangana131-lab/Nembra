...
if [[ "$REVIEW_ONLY" == "0" ]]; then
  ...
  : "${NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_SUBJECT_SHA256:?Set NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_SUBJECT_SHA256 to the preaccepted 64-hex generated CocoaPods build-subject SHA-256 before field bootstrap.}"
  ...
  ACCEPTED_GENERATED_SUBJECT_SHA256="$(printf '%s' "$NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_SUBJECT_SHA256" | tr '[:upper:]' '[:lower:]')"
else
...
if [[ ! -f "$PROVENANCE_HELPER" || ! -f "$GENERATED_SUBJECT_HELPER" ]]; then
  echo "ERROR: accepted Capture provenance helpers are missing from source." >&2
  exit 6
fi
...
if [[ ! -d Pods ]]; then
  echo "ERROR: CocoaPods did not create Pods/; generated field build subject is unavailable." >&2
  exit 10
fi
...
GENERATED_SUBJECT_SHA256="$(/usr/bin/python3 -I "$GENERATED_SUBJECT_HELPER" \
  --pods "$REPO_ROOT/Pods" \
  --workspace "$REPO_ROOT/NembraCapture.xcworkspace")" || {
  echo "ERROR: generated CocoaPods field build subject could not be fingerprinted." >&2
  exit 16
}
[[ "$GENERATED_SUBJECT_SHA256" =~ ^[0-9a-f]{64}$ ]] || {
  echo "ERROR: generated CocoaPods field build subject digest is invalid." >&2
  exit 16
}
...
DEPENDENCY BUILD SUBJECT CANDIDATE ONLY — NOT FIELD BUILD AUTHORITY
  ...
  Generated CocoaPods subject SHA-256: $GENERATED_SUBJECT_SHA256
...
the normal bootstrap/installer with those accepted digests. This review-only
mode never invokes xcodebuild, installs Nembra, scans Bluetooth, or authorizes
a physical attempt.
EOF
  exit 0
fi
...
[[ "$GENERATED_SUBJECT_SHA256" == "$ACCEPTED_GENERATED_SUBJECT_SHA256" ]] || {
  echo "ERROR: generated CocoaPods build bytes do not match the preaccepted generated-subject SHA-256. Stop before xcodebuild/install and review the new build subject." >&2
  exit 18
}
...
unset ACCEPTED_GENERATED_SUBJECT_SHA256 NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_SUBJECT_SHA256 || true
...
NEXT BUILD RULE:
  ...
  identity, or regenerate Pods/workspace before an accepted physical capture;
  any input or generated-subject change is a new reviewed field-build candidate
  and must earn a new exact-head acceptance.
