...
GENERATED_BUILD_HELPER="$SCRIPT_DIR/capture_cocoapods_generated_build_subject.py"
REVIEW_ONLY=0
...
if [[ "$REVIEW_ONLY" == "0" ]]; then
  ...
  : "${NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_BUILD_SHA256:?Set NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_BUILD_SHA256 to the preaccepted 64-hex CocoaPods generated-build SHA-256 before field bootstrap. To create a candidate dependency subject without build authority, use --resolve-lock-for-review.}"
  ...
  [[ "$NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_BUILD_SHA256" =~ ^[0-9A-Fa-f]{64}$ ]] || {
    echo "ERROR: NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_BUILD_SHA256 must be exactly 64 hex characters." >&2
    exit 1
  }
  ACCEPTED_LOCK_SHA256="$(printf '%s' "$NEMBRA_CAPTURE_ACCEPTED_TUYA_LOCK_SHA256" | tr '[:upper:]' '[:lower:]')"
  ACCEPTED_GENERATED_BUILD_SHA256="$(printf '%s' "$NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_BUILD_SHA256" | tr '[:upper:]' '[:lower:]')"
else
  unset NEMBRA_CAPTURE_ACCEPTED_TUYA_LOCK_SHA256 NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_BUILD_SHA256 || true
  ACCEPTED_LOCK_SHA256=""
  ACCEPTED_GENERATED_BUILD_SHA256=""
fi
...
if [[ ! -f "$GENERATED_BUILD_HELPER" ]]; then
  echo "ERROR: CocoaPods generated-build authority helper is missing from the accepted source." >&2
  exit 6
fi
...
# build. This record intentionally remains separate from generated CocoaPods
# authority: private source trees and generated build files have different
# custody boundaries and are both required below.
...
# The lock describes dependency resolution, but it does not authenticate the
# actual ignored workspace/support files that xcodebuild consumes. Fingerprint
# those exact generated bytes after pod install. Review-only emits this digest;
# a real field bootstrap must reproduce the exact same generated subject.
if ! GENERATED_BUILD_SHA256="$(/usr/bin/python3 -I "$GENERATED_BUILD_HELPER" fingerprint \
  --pods "$REPO_ROOT/Pods" \
...
GENERATED_BUILD_SHA256="$(printf '%s' "$GENERATED_BUILD_SHA256" | tr '[:upper:]' '[:lower:]')"
[[ "$GENERATED_BUILD_SHA256" =~ ^[0-9a-f]{64}$ ]] || {
  echo "ERROR: CocoaPods generated build subject did not produce one canonical SHA-256." >&2
  exit 14
}
...
DEPENDENCY + GENERATED BUILD CANDIDATE ONLY — NOT FIELD BUILD AUTHORITY
  ...
  CocoaPods generated build SHA-256: $GENERATED_BUILD_SHA256
...
NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_BUILD_SHA256. This review-only mode never
invokes xcodebuild, installs Nembra, scans Bluetooth, or authorizes a physical
attempt.
EOF
  exit 0
fi
...
[[ "$GENERATED_BUILD_SHA256" == "$ACCEPTED_GENERATED_BUILD_SHA256" ]] || {
  echo "ERROR: CocoaPods generated build subject does not match the preaccepted generated-build SHA-256. Stop before xcodebuild/install; the ignored workspace/Pods build bytes changed under the reviewed dependency lock." >&2
  exit 18
}
printf 'Preaccepted Tuya dependency lock matched: %s\n' "$LOCK_SHA256"
printf 'Preaccepted CocoaPods generated build subject matched: %s\n' "$GENERATED_BUILD_SHA256"
unset ACCEPTED_LOCK_SHA256 ACCEPTED_GENERATED_BUILD_SHA256 \
  NEMBRA_CAPTURE_ACCEPTED_TUYA_LOCK_SHA256 NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_BUILD_SHA256 || true
...
NEXT BUILD RULE:
  ...
  Preserve this exact private-input fingerprint record and generated-build
  subject with the field workspace. Do not run 'pod update', replace
  ThingSmartCryption, regenerate the private identity, or regenerate CocoaPods
  build files before an accepted physical capture; any input change is a new
