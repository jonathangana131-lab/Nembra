...
if [[ "$REVIEW_ONLY" == "0" ]]; then
  ...
  : "${NEMBRA_CAPTURE_ACCEPTED_TUYA_GENERATED_BUILD_SHA256:?Set NEMBRA_CAPTURE_ACCEPTED_TUYA_GENERATED_BUILD_SHA256 to the preaccepted 64-hex CocoaPods generated-build SHA-256 before field bootstrap. To create a candidate dependency subject without build authority, use --resolve-lock-for-review.}"
  ...
  [[ "$NEMBRA_CAPTURE_ACCEPTED_TUYA_GENERATED_BUILD_SHA256" =~ ^[0-9A-Fa-f]{64}$ ]] || {
    echo "ERROR: NEMBRA_CAPTURE_ACCEPTED_TUYA_GENERATED_BUILD_SHA256 must be exactly 64 hex characters." >&2
    exit 1
  }
  ACCEPTED_LOCK_SHA256="$(printf '%s' "$NEMBRA_CAPTURE_ACCEPTED_TUYA_LOCK_SHA256" | tr '[:upper:]' '[:lower:]')"
  ACCEPTED_GENERATED_BUILD_SHA256="$(printf '%s' "$NEMBRA_CAPTURE_ACCEPTED_TUYA_GENERATED_BUILD_SHA256" | tr '[:upper:]' '[:lower:]')"
else
  unset NEMBRA_CAPTURE_ACCEPTED_TUYA_LOCK_SHA256 NEMBRA_CAPTURE_ACCEPTED_TUYA_GENERATED_BUILD_SHA256 || true
...
# Start from a finite generated subject. Keeping stale ignored workspace/Pods
# bytes would allow an old or hostile generator residue to survive `pod install`
# and silently participate in the signed field build.
rm -rf -- "$REPO_ROOT/Pods" "$REPO_ROOT/NembraCapture.xcworkspace"
...
# build. The helper writes only fingerprints + public reviewed versions.
...
if ! GENERATED_BUILD_SHA256="$(/usr/bin/python3 -I "$GENERATED_BUILD_HELPER" \
  --pods "$REPO_ROOT/Pods" \
...
the normal bootstrap/installer with both accepted digests supplied as
NEMBRA_CAPTURE_ACCEPTED_TUYA_LOCK_SHA256 and
NEMBRA_CAPTURE_ACCEPTED_TUYA_GENERATED_BUILD_SHA256. This review-only mode never
...
[[ "$GENERATED_BUILD_SHA256" == "$ACCEPTED_GENERATED_BUILD_SHA256" ]] || {
  echo "ERROR: CocoaPods-generated build subject does not match the preaccepted generated-build SHA-256. Stop before xcodebuild/install; the generator produced different signed-build inputs under the reviewed lock." >&2
  exit 18
}
...
unset ACCEPTED_LOCK_SHA256 ACCEPTED_GENERATED_BUILD_SHA256 || true
...
NEXT BUILD RULE:
  ...
  Preserve this exact private-input fingerprint record and generated-build digest.
  The installer must keep the accepted generated subject under build-window
  custody across xcodebuild. Do not run 'pod update', replace ThingSmartCryption,
  regenerate the private identity, or regenerate Pods/workspace after review;
  any input change is a new reviewed field-build candidate.
