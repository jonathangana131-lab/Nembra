...
EOF
  ...
  # Mutable dependency generation is permitted only while creating a candidate
  # for explicit review. The normal field path below is verification-only and
  # must consume the exact already-reviewed lock + Pods/workspace bytes.
...
# In field mode the reviewed lock is the first mutable-workspace authority gate.
# Fail before any provenance write or generated-subject admission if it drifted.
