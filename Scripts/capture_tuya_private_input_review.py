...
def create_review(*, record_path: Path, key_path: Path, current: dict[str, str]) -> str:
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    try:
        if arguments.mode == "review":
            ...
            current = provenance.build_record(**paths)
            print(create_review(record_path=record_path, key_path=key_path, current=current))
        else:
