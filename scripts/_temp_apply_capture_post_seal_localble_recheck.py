...
recheck = watchdog.index("driver.isLocallyConnected(uuid: self.tuyaUUID)", seal)
post_seal = watchdog[seal:freeze]
for token in ["self.sdkLocalBLEOnline = driver.isLocallyConnected(uuid: self.tuyaUUID)", "guard self.sdkLocalBLEOnline else", "self.phase = .failed", "return"]:
...
if "await " in watchdog[seal + len("try await sessionLedger.sealAcceptedObservation(for: token)"):recheck]:
