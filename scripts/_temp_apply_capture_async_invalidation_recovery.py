...
progress_old = "    var correlationProgress: PassiveBluetoothPowerCycleObservationProgress? { correlationSession?.progress }\n    var correlationWindowIsScanning: Bool { correlationProgress?.isScanning == true }"
progress_new = "    var correlationProgress: PassiveBluetoothPowerCycleObservationProgress? { correlationSession?.progress }\n    var correlationSeriesIsInvalidated: Bool { correlationProgress?.isSeriesInvalidated == true }\n    var correlationWindowIsScanning: Bool { correlationProgress?.isScanning == true }"
if app.count(progress_old) != 1:
    raise SystemExit(f"correlation progress insertion count was {app.count(progress_old)}; refusing stale edit")
app = app.replace(progress_old, progress_new, 1)

lifecycle_marker = "\n    private func startCurrentCorrelationWindow() {"
if app.count(lifecycle_marker) != 1:
    raise SystemExit(f"correlation lifecycle insertion count was {app.count(lifecycle_marker)}; refusing stale edit")
recovery = r'''
...
'''
app = app.replace(lifecycle_marker, recovery + lifecycle_marker, 1)
...
if app.count(view_old) != 1:
    raise SystemExit(f"TimelineView insertion count was {app.count(view_old)}; refusing stale edit")
app = app.replace(view_old, view_new, 1)
