...
if [[ "$(uname -s)" != "Darwin" ]]; then echo "Guided Capture visual evidence requires macOS/CoreSimulator." >&2; exit 2; fi
if [[ ! -d "$APP_PATH" ]]; then echo "Standalone Capture app was not found at: $APP_PATH" >&2; exit 3; fi
if [[ ! -x /usr/bin/plutil ]] || ! command -v xcrun >/dev/null 2>&1; then echo "plutil and xcrun/CoreSimulator are required." >&2; exit 4; fi
for sha in "$EXPECTED_VALIDATION_SHA" "$EXPECTED_PRODUCT_PARENT_SHA"; do
  if [[ ! "$sha" =~ ^[0-9a-f]{40}$ ]]; then echo "Expected identities must be lowercase 40-hex SHAs." >&2; exit 5; fi
done

INFO_PLIST="$APP_PATH/Info.plist"
if [[ ! -f "$INFO_PLIST" ]]; then echo "Standalone Capture Info.plist is missing." >&2; exit 6; fi
...
if [[ "$BUNDLE_ID" != "$EXPECTED_BUNDLE_ID" ]]; then echo "Unexpected bundle identifier: $BUNDLE_ID" >&2; exit 7; fi
if [[ "$SOURCE_SHA" != "$EXPECTED_VALIDATION_SHA" ]]; then echo "Built app source identity does not match the exact validation head." >&2; exit 8; fi
EXPECTED_BUILD_IDENTIFIER="capture-v14-${SOURCE_SHA:0:12}"
if [[ "$BUILD_IDENTIFIER" != "$EXPECTED_BUILD_IDENTIFIER" ]]; then echo "Build identifier does not rendezvous with source SHA." >&2; exit 9; fi
if [[ -n "$TUYA_DEPENDENCY_LOCK_SHA256" ]]; then echo "Public visual evidence must not carry private Tuya dependency provenance or a shape-only substitute." >&2; exit 10; fi
if [[ "$PROCEDURE_SOURCE_CONTRACT_ID" != "ES80-AUTHENTICATED-STATIONARY-v1" ]]; then echo "Visual evidence procedure source contract is not canonical." >&2; exit 11; fi

if [[ -e "$ARTIFACTS_DIR" || -L "$ARTIFACTS_DIR" ]]; then echo "Refusing to overwrite prior visual evidence: $ARTIFACTS_DIR" >&2; exit 12; fi
...
'; } 2>/dev/null)" || { echo "No available iOS 27 Simulator runtime was found." >&2; exit 13; }
DEVICE_TYPE="$({ xcrun simctl list devicetypes -j | /usr/bin/python3 -c '
import json,sys
for item in json.load(sys.stdin)["devicetypes"]:
    if item.get("name") == "iPhone 12": print(item["identifier"]); raise SystemExit(0)
raise SystemExit(1)
'; } 2>/dev/null)" || { echo "V14 visual acceptance requires the iPhone 12 Simulator exactly." >&2; exit 14; }

SIM_NAME="Nembra Capture Current Visual ${GITHUB_RUN_ID:-local}-${GITHUB_RUN_ATTEMPT:-0}"
...
while IFS= read -r variable_name; do
  case "$variable_name" in
    SIMCTL_CHILD_*|NEMBRA_SIMULATION_*) echo "Refusing inherited synthetic authority: $variable_name" >&2; exit 15 ;;
  esac
done < <(compgen -v)

launch_output="$(xcrun simctl launch "$UDID" "$BUNDLE_ID" | tee "$ARTIFACTS_DIR/logs/launch.log")"
pid="${launch_output##*: }"
if [[ ! "$pid" =~ ^[0-9]+$ ]]; then echo "Could not parse standalone Capture process ID." >&2; exit 16; fi
sleep 2
if ! kill -0 "$pid" >/dev/null 2>&1; then echo "Standalone Nembra Capture exited before visual evidence." >&2; exit 17; fi
...
if [[ ! -s "$STANDARD_SCREENSHOT" ]]; then echo "Standard Capture screenshot was not created." >&2; exit 18; fi
...
if [[ ! -s "$AX_SCREENSHOT" ]]; then echo "Accessibility XXXL screenshot was not created." >&2; exit 19; fi
...
for digest in "$STANDARD_SHA256" "$AX_SHA256" "$INFO_PLIST_SHA256"; do [[ "$digest" =~ ^[0-9a-f]{64}$ ]] || exit 20; done

/usr/bin/python3 - "$ARTIFACTS_DIR/NembraCaptureGuidedCurrentVisualEvidence.json" "$BUILD_IDENTIFIER" "$SOURCE_SHA" "$EXPECTED_PRODUCT_PARENT_SHA" "$PROCEDURE_SOURCE_CONTRACT_ID" "$BUNDLE_ID" "$RUNTIME_ID" "$DEVICE_TYPE" "$STANDARD_SHA256" "$AX_SHA256" "$INFO_PLIST_SHA256" <<'PY'
import json,sys
(output_path, build_identifier, validation_sha, product_parent_sha, procedure_source_contract_id, bundle_id, runtime_id, device_type, standard_sha256, ax_sha256, info_plist_sha256) = sys.argv[1:]
record = {
 "schemaVersion": 3,
 ...
 "screenshots": [
   {"state":"unprovisioned-dark-standard","relativePath":"screenshots/standalone-unprovisioned-dark.png","sha256":standard_sha256},
   {"state":"unprovisioned-dark-accessibility-xxxl","relativePath":"screenshots/standalone-unprovisioned-dark-accessibility-xxxl.png","sha256":ax_sha256}
 ],
...
printf '%s\n' "Current guided Capture visual evidence captured." "Validation source: $SOURCE_SHA" "Product parent: $EXPECTED_PRODUCT_PARENT_SHA" "Human review remains required; no field, protocol, or physical authority is created."
