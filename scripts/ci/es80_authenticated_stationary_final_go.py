...
def review(pr:int,review_id:int,source:str,v:dict[str,Any],get=api):
    ...
    keys={"schemaVersion","authority","sourceCommitSHA","visualRunID","visualArtifactID","standardScreenshotSHA256","accessibilityScreenshotSHA256","tuyaDependencyLockSHA256","cocoaPodsBuildSubjectSHA256","verdict"}; lock=b.get("tuyaDependencyLockSHA256"); build_subject=b.get("cocoaPodsBuildSubjectSHA256")
    user=r.get("user",{})
    if set(b)!=keys or b.get("schemaVersion")!=3 or b.get("authority")!="nembra-capture-human-review-github-v3" or canon(b.get("sourceCommitSHA"),"candidate review source")!=source or b.get("visualRunID")!=v["runID"] or b.get("visualArtifactID")!=v["artifactID"] or not isinstance(lock,str) or not HEX64.fullmatch(lock) or lock!=lock.lower() or not isinstance(build_subject,str) or not HEX64.fullmatch(build_subject) or build_subject!=build_subject.lower() or b.get("verdict")!="accepted": raise GoError("GitHub candidate review authority mismatch")
    ...
    return {"authority":b["authority"],"reviewID":review_id,"reviewNodeID":r.get("node_id"),"reviewBodySHA256":sha(body.encode()),"reviewedAtUTC":stamp,"reviewer":OWNER,"state":r["state"],"verdict":"accepted","standardScreenshotSHA256":std,"accessibilityScreenshotSHA256":ax,"tuyaDependencyLockSHA256":lock,"cocoaPodsBuildSubjectSHA256":build_subject}
...
def candidate(repo:Path,source:str):
    ...
    ins=paths["installer"].read_text(); rb=paths["runbook"].read_text(); ident=paths["buildIdentity"].read_text(); bootstrap=paths["bootstrap"].read_text(); build_subject_helper=paths["cocoaPodsBuildSubjectHelper"].read_text()
    ...
    if "NEMBRA_CAPTURE_ACCEPTED_COCOAPODS_BUILD_SUBJECT_SHA256" not in bootstrap or "nembra-cocoapods-build-subject-v1" not in build_subject_helper: raise GoError("candidate lacks reviewed generated CocoaPods build-subject authority")
...
def installer_environment(device:Path,device_digest:str,accepted_lock_sha256:str,accepted_build_subject_sha256:str)->dict[str,str]:
...
def build(*,authority_repo:Path,authority_pr:int,authority_run:int,candidate_repo:Path,source:str,pr:int,runs:dict[str,int],artifact_id:int,review_id:int,archive:Path,device_file:Path,retained_ipa:Path,get=api,control_authority=control_plane,run_installer=installer,inspect_signed_artifact=retained_signed_artifact,reinspect_signed_artifact=retained_signed_artifact_reinspect,now=None):
    ...
    got=run_installer(candidate_repo,source,device_file,dh,rv["tuyaDependencyLockSHA256"],rv["cocoaPodsBuildSubjectSHA256"]); expected={"authority":"accepted-candidate-private-installer-execution-v1","result":"success","sourceCommitSHA":source,"buildIdentifier":f"capture-v14-{source[:12]}","bundleIdentifier":BUNDLE,"procedureIdentifier":PROC,"baselineDevice":DEVICE,"baselineProductType":PRODUCT,"baselineOS":"iOS 27"}
