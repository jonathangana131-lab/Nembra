...
def installer(repo:Path,source:str,device:Path,accepted_lock_sha:str):
    ...
    root=repo.expanduser().resolve(strict=True); device_path=canonical_private_path(device,"private intended-device identifier"); env=os.environ.copy(); env["NEMBRA_INTENDED_FIELD_DEVICE_UDID_FILE"]=str(device_path); env["NEMBRA_CAPTURE_ACCEPTED_TUYA_LOCK_SHA256"]=accepted_lock_sha
