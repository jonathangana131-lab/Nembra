...
def control_plane(authority_repo:Path,pr:int,run_id:int,get=api):
    ...
    if run.get("event")=="pull_request":
        bound=(isinstance(pulls,list) and any(isinstance(x,dict) and x.get("number")==pr for x in pulls)) or (pulls==[] and r.get("head_branch")==branch)
...
def review(pr:int,review_id:int,source:str,v:dict[str,Any],get=api):
    review_id=pos(review_id,"visual review ID"); _,r=get(f"/pulls/{pr}/reviews/{review_id}")
    body=r.get("body")
