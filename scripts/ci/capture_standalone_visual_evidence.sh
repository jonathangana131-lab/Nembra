...
for digest in "$STANDARD_SCREENSHOT_SHA256" "$AX5_SCREENSHOT_SHA256" "$INFO_PLIST_SHA256"; do
  [[ "$digest" =~ ^[0-9a-f]{64}$ ]] || { echo "Could not derive stable SHA-256 evidence digests." >&2; exit 17; }
done
...
import json, sys
(output_path, build_identifier, source_sha, procedure_identifier, bundle_id, runtime_id, device_type, standard_screenshot_sha256, ax5_screenshot_sha256, info_plist_sha256) = sys.argv[1:]
record = {
    "schemaVersion": 6,
...
printf '%s\n' "Standalone Nembra Capture visual evidence captured on exact V14 baseline." "Build: $BUILD_IDENTIFIER" "Source: $SOURCE_SHA" "Procedure read back from built app: $PROCEDURE_IDENTIFIER" "Tuya dependency authority: deliberately absent in public CI" "Baseline: $EXPECTED_DEVICE_NAME / iOS 27 Simulator" "Standard screenshot: $STANDARD_SCREENSHOT" "Accessibility XXXL screenshot: $AX5_SCREENSHOT" "Visual review is still required; this artifact creates no physical/protocol authority."
