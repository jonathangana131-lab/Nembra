...
[[ -d "$APP_PATH" ]] || { echo "Standalone Capture app was not found at: $APP_PATH" >&2; exit 3; }
[[ -x /usr/bin/plutil ]] || { echo "plutil is required to verify the standalone Capture build." >&2; exit 4; }
...
# Public visual QA intentionally carries no private dependency authority.
...
[[ "$PROCEDURE_IDENTIFIER" == "$EXPECTED_PROCEDURE_IDENTIFIER" ]] || { echo "Procedure mismatch: $PROCEDURE_IDENTIFIER" >&2; exit 23; }
...
[[ "$SOURCE_SHA" == "$CHECKOUT_SHA" ]] || { echo "App/source checkout mismatch." >&2; exit 20; }

if [[ -e "$ARTIFACTS_DIR" || -L "$ARTIFACTS_DIR" ]]; then
  echo "Refusing to mix prior visual evidence: $ARTIFACTS_DIR" >&2
  exit 11
fi
...
'; } 2>/dev/null)" || { echo "No iOS 27 Simulator runtime is available." >&2; exit 12; }
...
raise SystemExit(1)
'; } 2>/dev/null)" || { echo "Required iPhone 12 Simulator type unavailable." >&2; exit 13; }
[[ "$DEVICE_TYPE" == "$EXPECTED_DEVICE_TYPE" ]] || { echo "Unexpected iPhone 12 device type: $DEVICE_TYPE" >&2; exit 21; }
...
# Refuse inherited synthetic authority before launching the unfixtureized app.
while IFS= read -r variable_name; do
  case "$variable_name" in
    SIMCTL_CHILD_*|NEMBRA_SIMULATION_*) echo "Refusing inherited synthetic authority: $variable_name" >&2; exit 18 ;;
  esac
done < <(compgen -v)

launch_output="$(xcrun simctl launch "$UDID" "$BUNDLE_ID" | tee "$ARTIFACTS_DIR/logs/launch.log")"
pid="${launch_output##*: }"
[[ "$pid" =~ ^[0-9]+$ ]] || { echo "Could not parse standalone Capture PID." >&2; exit 14; }
sleep 2
kill -0 "$pid" >/dev/null 2>&1 || { echo "Standalone Nembra Capture exited before capture." >&2; exit 15; }
...
[[ -s "$AX5_SCREENSHOT" ]] || { echo "Accessibility screenshot missing." >&2; exit 22; }
...
for digest in "$STANDARD_SCREENSHOT_SHA256" "$AX5_SCREENSHOT_SHA256" "$INFO_PLIST_SHA256"; do
  [[ "$digest" =~ ^[0-9a-f]{64}$ ]] || { echo "Could not derive stable evidence digest." >&2; exit 17; }
done
