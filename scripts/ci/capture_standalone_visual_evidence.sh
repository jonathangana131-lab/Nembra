...
if [[ ! -f "$IDENTITY_SOURCE" ]] || ! grep -Fq "static let requiredFieldProcedureIdentifier = \"$EXPECTED_PROCEDURE_IDENTIFIER\"" "$IDENTITY_SOURCE"; then
  echo "Standalone Capture source does not declare the required stationary procedure." >&2
  exit 19
fi
if ! grep -Fq 'static var fieldProcedureIdentifier: String' "$IDENTITY_SOURCE" || ! grep -Fq 'current.procedureIdentifier' "$IDENTITY_SOURCE"; then
  echo "Standalone Capture source does not expose actually built procedure provenance." >&2
  exit 24
fi
...
# Public Simulator evidence intentionally carries no reviewed private dependency lock. A real
# source/procedure rendezvous is useful for visual review without promoting this app to field authority.
...
if [[ "$PROCEDURE_IDENTIFIER" != "$EXPECTED_PROCEDURE_IDENTIFIER" ]]; then
  echo "Standalone Capture actually built procedure does not rendezvous with the required stationary procedure: $PROCEDURE_IDENTIFIER" >&2
  exit 23
fi
...
# `simctl launch` forwards host variables prefixed with SIMCTL_CHILD_ into the app. Reject inherited
# simulation/authority fixtures so this remains one real public-unprovisioned product launch.
...
record = {
    ...
    "procedureBuildRendezvousVerified": procedure_identifier == "ES80-AUTHENTICATED-STATIONARY-v1",
    "bundleIdentifier": bundle_id,
    "baselineDevice": "iPhone 12",
    "baselineOS": "iOS 27",
    "simulatorRuntime": runtime_id,
    "simulatorDeviceType": device_type,
    "launchContext": "real standalone bundle; actual built procedure read back; private dependency provenance absent; inherited SIMCTL_CHILD/NEMBRA_SIMULATION authority rejected before launch",
