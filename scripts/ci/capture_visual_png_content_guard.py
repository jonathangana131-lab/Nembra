...
CONTENT_FRACTION_DENOMINATOR = 500  # >= 0.2% of app-content pixels.
...
def inspect_rendered_content(path: Path) -> dict[str, object]:
    ...
    app_y_start = max(1, height // 8)  # Deliberately excludes status-bar-only pixels.
...
def main() -> int:
    ...
    if not result["ready"]:
        print(
            f"{args.label}: app-content region is not visibly rendered; refusing blank/partial visual evidence",
            file=sys.stderr,
        )
        return 1
    return 0
