...
'''
if s.count(old) != 1:
    raise SystemExit('unexpected Payload declaration')
...
'''
if s.count(old) != 1:
    raise SystemExit('unexpected consume declaration')
...
if method.count(old) != 1:
    raise SystemExit('unexpected Experiment One staging block')
...
assert m.index('previewForControllerStaging()') < m.index('peripheralByIdentifier[') < m.index('admission.consume()')
...
assert m.index('receivedAtUptimeNanoseconds > preview.issuedAtUptimeNanoseconds') < m.index('admission.consume()')
assert m.index('admission.consume()') < m.index('payload.admissionIdentity == preview.admissionIdentity') < m.index('recorder = payload.recorder')
assert m.count('admission.consume()') == 1
assert '>=' not in m[m.index('latestAdvertisementByIdentifier['):m.index('admission.consume()')]
r = run.read_text(); assert 'struct StagingPreview: Equatable, Sendable' in r; assert 'func previewForControllerStaging() throws -> StagingPreview' in r
