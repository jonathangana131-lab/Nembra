...
class SignedFieldCandidateIntendedDeviceSourceTests(unittest.TestCase):
    def test_intended_device_is_required_through_private_path_only_input(self) -> None:
        ...
        # The verification value is read only inside the runner after every caller-supplied path
        # component has been opened without following symlinks. It must never become an OS-visible
        # child-process argument/environment value, and an accepted path cannot itself contain it.
        ...
        self.assertIn('--canonical-inspector-fd', runner)
    ...
    def test_relative_private_input_path_fails_closed(self) -> None:
        ...
        with self.assertRaisesRegex(
            ...
            runner.read_private_identifier(Path("device-id"))
