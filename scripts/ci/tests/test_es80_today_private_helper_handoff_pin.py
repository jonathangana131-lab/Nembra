...
class PrivateHelperHandoffPinTests(unittest.TestCase):
    ...
    TEST_BLOB = "e87c15948f7a6c934b600e4fc3bb525653847f5d"
    QA_RUN = "31350148402"
    QA_JOB = "93339277106"
    ...
    def test_both_operator_handoffs_cross_bind_current_descriptor_only_lineage(self):
        ...
        for document in (production, custody):
            with self.subTest(document=document[:40]):
                ...
                self.assertIn(self.TEST_BLOB, document)
                self.assertIn(self.QA_RUN, document)
                self.assertIn(self.QA_JOB, document)

    def test_active_materialization_assignments_use_current_helper_not_audit_history(self):
        production, custody = self.documents()
    ...
    def test_current_handoffs_describe_pathname_nondestructive_failure_cleanup(self):
        production, custody = self.documents()
        for document in (production, custody):
            with self.subTest(document=document[:40]):
                self.assertIn("zero-length", document)
                self.assertIn("replacement", document)
                self.assertIn("path", document.lower())

        self.assertIn("no mutable pathname is unlinked", production)
        self.assertIn("never unlinks a mutable pathname", custody)
