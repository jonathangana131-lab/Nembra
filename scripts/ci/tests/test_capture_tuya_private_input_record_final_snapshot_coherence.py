...
class CaptureTuyaPrivateInputFinalSnapshotCoherenceTests(unittest.TestCase):
    ...
    def test_final_generation_snapshot_revalidates_earlier_authorities_after_later_tree_walk(self) -> None:
        ...
        def tree_snapshot_with_final_cross_input_mutation(path: Path):
            ...
            # build_record performs two whole-record generation snapshots. Each snapshot
            # walks security_build, then identity_sources. Call four is therefore the
            # final snapshot's last tree, after that same snapshot already captured the
            # Podfile.lock identity. Mutate and restore the earlier authority here.
