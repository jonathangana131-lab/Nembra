...
review = load("capture_private_review_closure_2757", REVIEW_HELPER)
guard = load("capture_private_guard_closure_2757", GUARD_HELPER)
...
class PrivateInputReviewAuthorityClosureTests(unittest.TestCase):
    ...
    def test_private_authority_layer_preserves_checkout_ancestry_contract(self) -> None:
        source = GUARD_HELPER.read_text(encoding="utf-8")
        self.assertIn("def _lexical_absolute", source)
        self.assertIn("def _require_real_checkout_ancestry", source)
        self.assertIn("Path(os.path.abspath", source)
        parse_region = source[source.index("def _parse_args"):source.index("def main")]
        self.assertNotIn(".resolve()", parse_region)
        self.assertIn("path ancestry must not contain symlinks", source)
