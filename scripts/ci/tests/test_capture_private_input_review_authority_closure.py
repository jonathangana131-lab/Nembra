...
review = load("capture_private_review_closure", REVIEW_HELPER)
guard = load("capture_private_guard_closure", GUARD_HELPER)
...
class PrivateInputReviewAuthorityClosureTests(unittest.TestCase):
    ...
    def test_reviewed_generation_round_trips_with_private_local_key(self) -> None:
        ...
        # External authority is one opaque token. It is not any direct private
        # record field/fingerprint that could be published as a credential oracle.
    ...
    def test_attacker_cannot_self_authorize_by_rewriting_record_and_key(self) -> None:
        ...
        # Even using the legitimate local review helper to create a fully stable
        # B record+key produces new authority, not the externally accepted A token.
    ...
    def test_key_alias_or_broad_permissions_fail_closed(self) -> None:
        ...
        # Restore then make a hard-link alias; either condition must make the key
        # unusable as private review authority.
    ...
    def test_build_guard_requires_external_private_authority(self) -> None:
        ...
        with mock.patch.dict(os.environ, {AUTHORITY_ENV: ""}, clear=False):
            with self.assertRaisesRegex(guard.BuildGuardError, "must carry the exact reviewed"):
                inputs.authority_bound_generation_snapshot()

    def test_build_guard_rejects_change_during_authority_rebind(self) -> None:
        inputs = guard.PrivateInputs(
            lockfile=Path("/tmp/lock"),
            security_podspec=Path("/tmp/security.podspec"),
            security_build=Path("/tmp/security-build"),
        ...
        with mock.patch.dict(os.environ, {AUTHORITY_ENV: accepted}, clear=False), mock.patch.object(
            ...
            guard.private_review,
            "verify_review_paths",
            return_value=accepted,
        ):
            with self.assertRaisesRegex(guard.BuildGuardError, "changed while private review authority was rebound"):
                inputs.authority_bound_generation_snapshot()

    def test_run_guarded_build_uses_authority_bound_snapshot_as_baseline(self) -> None:
        source = GUARD_HELPER.read_text(encoding="utf-8")
        authority_line = "initial_snapshot = inputs.authority_bound_generation_snapshot()"
