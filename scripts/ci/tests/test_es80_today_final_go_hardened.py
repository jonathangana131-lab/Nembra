...
class HardenedFinalGoCompositionTests(unittest.TestCase):
    ...
    def fake_foundation(self, **kwargs):
        ...
        operator = hardened.foundation._operator_attestation(
            kwargs["operator_attestation"], candidate, kwargs["now_utc"]
        )
        return {
    ...
    def test_composition_replaces_both_authority_seams_and_restores_them(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.object(
                hardened.foundation, "_operator_attestation", local_operator
            ), mock.patch.object(
                hardened.foundation,
            ...
            self.assertEqual(verify_xcode.call_args.kwargs["source_commit_sha"], self.SOURCE)
            self.assertEqual(verify_xcode.call_args.kwargs["expected_pr_number"], 833)
    ...
    def test_trusted_xcode_failure_becomes_final_go_error_and_restores_seams(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            original_xcode = hardened.foundation._trusted_xcode_subject
            original_operator = hardened.foundation._operator_attestation
    ...
    def test_trusted_operator_failure_becomes_final_go_error_and_restores_seams(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            original_xcode = hardened.foundation._trusted_xcode_subject
            original_operator = hardened.foundation._operator_attestation
