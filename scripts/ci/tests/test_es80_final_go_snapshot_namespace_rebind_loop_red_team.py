...
Current #3038 keeps accepted regular-file inodes open across candidate custody
and correctly rejects persistent replacement plus finite content rehash attacks.
...
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("could not load current #3038 Final-GO snapshot subject")
...
class FinalGoSnapshotNamespaceRebindLoopRedTeamTests(unittest.TestCase):
    ...
    def test_final_audit_rebind_can_sample_accepted_inode_then_return_divergent_path(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="nembra-finalgo-snapshot-rebind-audit-") as temporary:
            ...
            def diverge_after_second_exact_parent_audit(current_root: Path, current_source: str):
                nonlocal attack_active, audit_calls
                result = original_audit(current_root, current_source)
                audit_calls += 1
                self.assertEqual(result, accepted_entries)
                # Current #3038 validates once before snapshot admission, then
                # runs the exact parent again while the held snapshot is live.
                # Diverge only after the in-snapshot parent audit so production
                # reaches its final live-namespace rebind with attacker bytes.
                if audit_calls == 2:
                    self._install_attacker_and_hold_accepted(
                        tracked, accepted_holder, attacker_source
                    )
                    attack_active = True
                return result
            ...
            MODULE._ORIGINAL_AUDIT = diverge_after_second_exact_parent_audit
            ...
            self.assertGreaterEqual(audit_calls, 2, "current #3038 two-stage parent audit was not exercised")
