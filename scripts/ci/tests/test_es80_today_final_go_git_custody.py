#!/usr/bin/env python3
"""Expected-red regression for Final GO trusted-workflow Git custody.

The hardened Final GO entrypoint must not let caller PATH/Git semantics manufacture the Git blob
identity used to trust the default-branch Capture Xcode workflow. This test is validation-only: the
parent implementation is expected to fail until its Git lookup is isolated behind producer-owned
absolute tooling and a closed Git environment.
"""
