...
class CaptureNativeACLBaselineCoherenceRedTeamTests(unittest.TestCase):
    def test_seeded_fd_baseline_can_diverge_from_path_baseline_and_grant_still_succeeds(self) -> None:
        ...
        # Exact attacked shape: descriptor baseline capture exists, but production has
        # no native canonical-path ACL acquisition/canonicalization before +a.
        ...
        with tempfile.TemporaryDirectory(prefix="nembra-native-acl-coherence-redteam-") as temporary:
            ...
            # Model the dangerous state directly: the held-FD capture returns a
            # retained seeded rollback ACL while the canonical path pre-grant view
            # is empty. Current production carries only the opaque retained handle,
            # so there is no native semantic comparison that can reject divergence.
            path_listings = iter(
                (
                    "",
            ...
            with (
                ...
                mock.patch.object(
                    helper, "_capture_fd_acl_baseline", return_value=0xBEEF
                ) as capture,
                mock.patch.object(
                    helper,
                    "_path_acl_listing",
