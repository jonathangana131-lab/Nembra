...
class CaptureFieldInstallerBuildWindowGuardSourceTests(unittest.TestCase):
    def test_installer_runs_xcodebuild_inside_private_input_guard(self) -> None:
        ...
        start = source.index('say "Building SDK-integrated Nembra Capture for the intended iPhone"')
        end = source.index('APP="$DERIVED/Build/Products/Debug-iphoneos/Nembra Capture.app"', start)
        ...
        self.assertEqual(build.count('xcodebuild'), 2)  # comment + guarded command only

    def test_guard_keeps_existing_post_build_crypto_verify(self) -> None:
        source = INSTALLER.read_text(encoding="utf-8")
        build = source.index('say "Building SDK-integrated Nembra Capture for the intended iPhone"')
        command = source.index('/usr/bin/python3 -I "$TUYA_BUILD_WINDOW_GUARD"', build)
        post = source.index('verify_private_tuya_inputs', command)
        app = source.index('APP="$DERIVED/Build/Products/Debug-iphoneos/Nembra Capture.app"', command)
        self.assertLess(command, post)
