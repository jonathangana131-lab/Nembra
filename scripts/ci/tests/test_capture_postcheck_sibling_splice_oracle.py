#!/usr/bin/env python3
"""Red-team oracle for the cached-parent validation-to-child-open race.

Success is exploit evidence: the exact composed parent can admit SDK-A + Runtime-B
when TuyaRuntime changes after cached-parent validation but before child open.
"""
...
def load():
    spec = importlib.util.spec_from_file_location("nembra_postcheck_sibling_splice", HELPER)
...
def install_postcheck_runtime_splice(helper, root: Path, generation_b: Path):
    original_open_directory = helper._open_directory_at
    state = {"swapped": False}

    def splice(parent_fd: int, name: str, relative: Path) -> int:
    ...
    return state, mock.patch.object(helper, "_open_directory_at", side_effect=splice)


class CapturePostcheckSiblingSpliceOracleTests(unittest.TestCase):
    def test_manifest_reproduces_sdk_a_runtime_b_after_parent_precheck(self) -> None:
        helper = load()
        with tempfile.TemporaryDirectory(prefix="nembra-postcheck-splice-manifest-") as raw:
            ...
            state, patcher = install_postcheck_runtime_splice(helper, root, generation_b)
            with patcher:
                encoded = helper.canonical_generated_manifest(root, SOURCE_SHA)
            self.assertTrue(state["swapped"])
            payload = json.loads(encoded)
            self.assertEqual(entry_sha(payload, "LocalSecrets/TuyaSDK/sdk.bin"), hashlib.sha256(b"SDK-A\n").hexdigest())
            self.assertEqual(entry_sha(payload, "LocalSecrets/TuyaRuntime/identity.bin"), hashlib.sha256(b"RUNTIME-B\n").hexdigest())

    def test_copy_reproduces_sdk_a_runtime_b_after_parent_precheck(self) -> None:
        helper = load()
        with tempfile.TemporaryDirectory(prefix="nembra-postcheck-splice-copy-") as raw:
            ...
            self.assertEqual((destination / "LocalSecrets/TuyaRuntime/identity.bin").read_bytes(), b"RUNTIME-B\n")
