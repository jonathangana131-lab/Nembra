...
class PrivateInputReviewWitnessTests(unittest.TestCase):
    ...
    def run_bootstrap(self, *, review_only: bool, accepted_lock: str | None = None, accepted_generated: str | None = None) -> subprocess.CompletedProcess[str]:
        ...
        return subprocess.run(command, cwd=self.root, env=environment, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=False)
    ...
    def test_normal_field_mode_rejects_substitution_before_cocoapods_and_preserves_witness(self) -> None:
        ...
        self.assertEqual(
            self.pod_counter.read_text(encoding="utf-8").splitlines(), ["pod"],
            "unreviewed private inputs reached CocoaPods before review-witness rejection",
        )
        self.assertEqual(self.record.read_bytes(), reviewed_record, "normal field bootstrap rewrote the review witness")
