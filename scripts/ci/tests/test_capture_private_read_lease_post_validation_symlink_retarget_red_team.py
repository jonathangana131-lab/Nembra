...
class CapturePrivateReadLeasePostValidationSymlinkRetargetRedTeamTests(unittest.TestCase):
    def test_grant_can_accept_symlink_retargeted_after_descriptor_bound_validation(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            ...
            def validate_safe_then_retarget(
                ...
                # The authoritative held-directory validation has just accepted the
                # internal symlink. Replace only the symlink object before the real
                # descriptor-plan coherence/grant path continues. Symlinks themselves
                # are intentionally absent from the ACL descriptor plan.
                link.unlink()
            ...
            with (
                ...
                # Exploit-positive success means the real production grant accepts a
                # plan after the held-directory policy check observed the safe target,
                # while the live symlink in the same held directory now points outside.
    ...
    def test_source_plan_has_no_symlink_identity_in_final_descriptor_verification(self) -> None:
        helper = load()
        import inspect
