#!/usr/bin/env python3
"""V14 witness for mutable private-runner execution after source preflight."""
...
class CaptureFieldPrivateRunnerExecutionAuthorityRedTeamTests(unittest.TestCase):
    def test_same_path_replacement_after_preflight_executes_replacement_bytes(self) -> None:
        """Keep #2886's attack primitive executable without touching product state."""
    ...
    def test_field_installer_must_not_import_private_runner_from_mutable_checkout(self) -> None:
        ...
        repaired_marker = 'PRIVATE_DEVICE_RUNNER_RELATIVE="scripts/ci/es80_signed_field_artifact_private_runner.py"'
        legacy_marker = 'PRIVATE_DEVICE_RUNNER="$ROOT/scripts/ci/es80_signed_field_artifact_private_runner.py"'
        if repaired_marker in source:
            start = source.index(repaired_marker)
        else:
