...
those bytes when it is executed. Candidate tests run arbitrary code between the current Git-object
check and the Simulator runner, so the final pre-execution custody step must also hash the actual
working-tree bytes and must sit immediately before runner execution.
"""
