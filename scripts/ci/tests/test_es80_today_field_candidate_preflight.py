...
class FieldCandidatePreflightTests(unittest.TestCase):
    ...
    def test_retired_production_handoff_is_explicitly_non_authorizing(self):
    ...
    def test_retired_production_handoff_does_not_publish_old_helper_or_producer_recipe(self):
        handoff = HANDOFF_PATH.read_text(encoding="utf-8")
    ...
    def test_retired_production_handoff_contains_no_executable_shell_recipe(self):
        handoff = HANDOFF_PATH.read_text(encoding="utf-8")

        self.assertNotIn("```bash", handoff)
        self.assertNotIn("HOME_PHYSICAL=", handoff)
        self.assertNotIn("PRIVATE_INPUT_HELPER=", handoff)
        self.assertNotIn("UDID_FILE=", handoff)
        self.assertNotIn("/usr/bin/python3 -I", handoff)
