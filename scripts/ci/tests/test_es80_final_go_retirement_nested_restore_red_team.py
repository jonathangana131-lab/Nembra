    ...
    def test_inherited_parent_finally_reopens_git_before_outer_retirement_exits(self) -> None:
        ...
        def fake_continuous(_root: Path, _source: str):
            try:
                ...
                # This runs after the inherited parent custody's finally. If that
                # finally restored its saved base.git, the outer retirement fence
                # has been silently undone before continuous-custody teardown ends.
        ...
        def fake_parent_candidate(_base, _root, _source):
            ...
            try:
                ...
                # Mirrors #2921's real finally: restore the functions captured
                # before the child retirement boundary overwrote them.
