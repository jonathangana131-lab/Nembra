...
with tempfile.TemporaryDirectory() as temp_dir:
    ...
    # This reproduces the exact class of false-green evidence seen on #2979:
    # bright status-bar pixels are present, but the entire app-content region is black.
    status_only = temp / "status-only.png"
    status_only.write_bytes(
        rgba_png(
            width,
            height,
            lambda _x, y: (255, 255, 255, 255) if y < status_bar_end else (0, 0, 0, 255),
        )
    )
    ...
    # A dark Nembra-like surface with bright content in separated vertical regions
    # must be admitted for human visual review; this is not aesthetic acceptance.
    ...
    corrupted[-5] ^= 0x01  # Break IEND CRC without changing the synthetic image body.
