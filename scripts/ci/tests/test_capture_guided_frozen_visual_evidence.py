...
for needle in required:
    if needle not in script:
        raise SystemExit(f"missing frozen visual-evidence contract: {needle}")
...
if "--args" in launch_lines[0]:
    raise SystemExit("frozen visual evidence must launch the standalone product without fixture arguments")

if 'preferred=[' in script or '"iPhone 17"' in script or '"iPhone 16"' in script:
    raise SystemExit("V14 frozen visual gate must not fall back from the exact iPhone 12 baseline")
...
print("frozen guided Capture visual evidence source contract: PASS")
