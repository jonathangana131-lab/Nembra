#!/usr/bin/env python3
"""Expected-red: a fixed accepted Podfile.lock must not authorize different generated CocoaPods build bytes.

The authenticated field bootstrap currently reviews/fingerprints the resolver lock and private Tuya
inputs, while CocoaPods-generated workspace/support files are ignored. This diagnostic drives the
real bootstrap twice with one identical lock and a deliberately substituted `pod` implementation.
The second run changes build-affecting generated bytes while preserving every currently reviewed
lock/private-input subject. Final field bootstrap must fail closed before xcodebuild; current source
is expected to accept the second run, making this regression RED until generated build authority is
bound.
"""
...
class CocoaPodsGeneratedBuildSubjectTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        # bootstrap_capture_tuya_sdk.sh uses the macOS `stat -f %Lp` spelling only to
        # assert the mode of its freshly created private provenance record. Keep this
        # diagnostic portable on Ubuntu without altering the product bootstrap bytes.
    ...
    def run_bootstrap(self, payload: str, *, review_only: bool, accepted_lock: str | None = None) -> subprocess.CompletedProcess[str]:
        environment = {
    ...
    def test_same_accepted_lock_cannot_authorize_changed_generated_build_graph(self) -> None:
        ...
        lock_bytes = (self.root / "Podfile.lock").read_bytes()
        accepted_lock = hashlib.sha256(lock_bytes).hexdigest()
        ...
        self.assertNotEqual(
            ...
            "red-team setup must materially replace one generated build-affecting CocoaPods file",
        )
        self.assertNotEqual(
            field.returncode,
            0,
            "field bootstrap admitted different CocoaPods-generated build bytes under the exact same accepted Podfile.lock; "
            "bind the generated build subject/toolchain before xcodebuild",
        )
