#!/usr/bin/env python3
"""Regression: one accepted lock cannot authorize substituted CocoaPods-generated build bytes."""
...
class CocoaPodsGeneratedBuildSubjectTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        pod.write_text(
            "#!/bin/bash\n"
            "set -euo pipefail\n"
            "rm -rf Pods NembraCapture.xcworkspace\n"
    ...
    def test_same_accepted_lock_rejects_changed_generated_build_graph(self) -> None:
        ...
        self.assertEqual(field_lock, accepted_lock, "fake pod must preserve the exact accepted Podfile.lock")
        self.assertNotEqual(reviewed_generated, substituted_generated, "test must materially replace generated build bytes")
        self.assertNotEqual(field.returncode, 0, "substituted generated build bytes crossed the accepted build-subject boundary")
    ...
    def test_exact_reviewed_generated_subject_is_admitted(self) -> None:
        review = self.run_bootstrap("STABLE_GRAPH", review_only=True)
        self.assertEqual(review.returncode, 0, review.stdout)
        accepted_lock = hashlib.sha256((self.root / "Podfile.lock").read_bytes()).hexdigest()
        accepted_generated = self.generated_digest()
        field = self.run_bootstrap(
