#!/usr/bin/env python3
"""V14 attack witnesses for field-installer Git/execution authority."""
...
class CaptureFieldInstallerGitAuthorityRedTeamTests(unittest.TestCase):
    ...
    def test_field_installer_must_not_split_git_authority_from_bootstrap_execution(self) -> None:
        ...
        self.assertIn(
            ...
            source,
            "field bootstrap must execute exact accepted source bytes",
        )
        self.assertIn(
            'authority_git_status',
            source,
            "field cleanliness must use the fenced authority index/worktree contract",
        )
