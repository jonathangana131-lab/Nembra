#!/usr/bin/env python3
"""Independent QA for #2937's materialized accepted-Git payload verifier."""
...
class CaptureFieldVerifiedGitPayloadCurrentQA(unittest.TestCase):
    ...
    def test_actual_materialized_verifier_rejects_forged_pack_alias_without_stdout(self) -> None:
        verifier = extract_verifier(INSTALLER.read_text(encoding="utf-8"))
        with tempfile.TemporaryDirectory(prefix="nembra-verified-git-payload-qa-") as directory:
            repo = Path(directory) / "repo"
            repo.mkdir()
            subprocess.run([GIT, "init", "-q", str(repo)], check=True)
            subprocess.run([GIT, "-C", str(repo), "config", "user.email", "nembra@example.invalid"], check=True)
            subprocess.run([GIT, "-C", str(repo), "config", "user.name", "Nembra QA"], check=True)
            path = repo / RELATIVE
