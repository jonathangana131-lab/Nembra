...
class T(unittest.TestCase):
 ...
 def test_installer_receives_exact_reviewed_lock(self):
  seen=[]
  def run(r,s,d,h,lock):seen.append(lock);return self.f.inst(r,s,d,h,lock)
 ...
 def test_installer_never_runs_before_public_visual_and_lock_acceptance(self):
  calls=[]
  def run(r,s,d,h,lock):calls.append((s,lock));return self.f.inst(r,s,d,h,lock)
 ...
 def test_post_install_revalidation_rejects_pr_artifact_reviews_or_device_drift(self):
  ...
  def change_lock_review(r,s,d,h,lock):x=self.f.inst(r,s,d,h,lock);self.f.map[lockreview]['body']=self.f.lock_body(podfileLockSHA256='e'*64);return x
