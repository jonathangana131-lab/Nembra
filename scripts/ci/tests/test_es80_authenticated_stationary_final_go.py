...
class F:
 def __init__(self,r):
  ...
  self.map={f'/pulls/{self.pr}':{'state':'open','merged_at':None,'head':{'sha':self.s,'repo':{'full_name':go.REPO}},'base':{'ref':'main'}},f'/actions/artifacts/{self.aid}':{'expired':False,'digest':'sha256:'+H(self.arc.read_bytes()),'workflow_run':{'id':self.ids[go.VISUAL]}}}
  for n,i in self.ids.items():self.map[f'/actions/runs/{i}']={'name':n,'head_sha':self.s,'status':'completed','conclusion':'success','event':'pull_request','pull_requests':[{'number':self.pr}]}
