...
class T(unittest.TestCase):
 ...
 def test_github_review_owner_commit_state_and_body_are_authority(self):
  ...
  r['body']='{"schemaVersion":3,"schemaVersion":3}';self.no(self.f.build)
 ...
 def test_reviewed_tuya_lock_is_bound_to_signed_artifact(self):
  ...
  self.no(lambda:self.f.build(inspect_signed_artifact=bad))
 def test_reviewed_cocoapods_subject_is_bound_to_go_record(self):
 ...
 def test_reviewed_tuya_lock_rejected_before_installer_if_noncanonical(self):
  calls=[]
  def capture(r,s,d,h,lock,build_subject):calls.append(lock);return self.f.inst(r,s,d,h,lock,build_subject)
 ...
 def test_reviewed_tuya_lock_drift_after_install_is_rejected(self):
  ...
  def drift(r,s,d,h,lock,build_subject):x=self.f.inst(r,s,d,h,lock,build_subject);self.f.map[review]['body']=self.f.body(tuyaDependencyLockSHA256='b'*64);return x
 ...
 def test_private_device_custody_and_installer_drift_rejected(self):
  ...
  def bad(r,s,d,h,lock,build_subject):x=self.f.inst(r,s,d,h,lock,build_subject);x['bundleIdentifier']='bad';return x
 ...
 def test_installer_never_runs_before_public_visual_acceptance(self):
  calls=[]
  def run(r,s,d,h,lock,build_subject):calls.append(s);return self.f.inst(r,s,d,h,lock,build_subject)
 ...
 def test_post_install_revalidation_rejects_pr_artifact_review_or_device_drift(self):
  ...
  def merge_pr(r,s,d,h,lock,build_subject):x=self.f.inst(r,s,d,h,lock,build_subject);self.f.map[pull]['state']='closed';self.f.map[pull]['merged_at']='2026-08-11T02:10:00Z';return x
  self.no(lambda:self.f.build(run_installer=merge_pr));self.f.map[pull]['state']='open';self.f.map[pull]['merged_at']=None
  def move_pr(r,s,d,h,lock,build_subject):x=self.f.inst(r,s,d,h,lock,build_subject);self.f.map[pull]['head']['sha']='1'*40;return x
  self.no(lambda:self.f.build(run_installer=move_pr));self.f.map[pull]['head']['sha']=self.f.s
  def expire(r,s,d,h,lock,build_subject):x=self.f.inst(r,s,d,h,lock,build_subject);self.f.map[artifact]['expired']=True;return x
  self.no(lambda:self.f.build(run_installer=expire));self.f.map[artifact]['expired']=False
  def dismiss(r,s,d,h,lock,build_subject):x=self.f.inst(r,s,d,h,lock,build_subject);self.f.map[review]['state']='DISMISSED';return x
  self.no(lambda:self.f.build(run_installer=dismiss));self.f.map[review]['state']='COMMENTED'
  def change_device(r,s,d,h,lock,build_subject):x=self.f.inst(r,s,d,h,lock,build_subject);self.f.dev.write_text('other-device\n');return x
 ...
 def test_candidate_rejects_skip_worktree_installer_tamper(self):
  ...
  subprocess.run(['/usr/bin/git','-C',str(self.f.repo),'checkout','--',go.INSTALLER],check=True)
 def test_candidate_rejects_hidden_build_subject_helper_tamper(self):
 ...
 def test_post_install_current_main_drift_is_rejected(self):
  def move_main(r,s,d,h,lock,build_subject):x=self.f.inst(r,s,d,h,lock,build_subject);self.f.main='1'*40;return x
