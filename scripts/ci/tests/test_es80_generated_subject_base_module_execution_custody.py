#!/usr/bin/env python3
"""Expected-red regression for R3 loading its authenticated-stationary base module."""
...
BASE_SOURCE = REPOSITORY / BASE_RELATIVE
...
class GeneratedSubjectBaseModuleExecutionCustodyTests(unittest.TestCase):
    def test_hidden_base_worktree_replacement_never_executes_from_r3_loader(self) -> None:
        with tempfile.TemporaryDirectory(prefix="nembra-r3-base-module-exec-") as temporary:
            root=Path(temporary).resolve(strict=True); sentinel=root/"attacker-base-executed"; r3=root/R3_RELATIVE; base=root/BASE_RELATIVE
            r3.parent.mkdir(parents=True,exist_ok=True); r3.write_bytes(R3_SOURCE.read_bytes()); base.write_bytes(BASE_SOURCE.read_bytes())
            ...
            subprocess.run(["/usr/bin/git","-C",str(root),"add","."],check=True); subprocess.run(["/usr/bin/git","-C",str(root),"commit","-qm","accepted R3/base fixture"],check=True)
            accepted_blob=subprocess.check_output(["/usr/bin/git","-C",str(root),"rev-parse",f"HEAD:{BASE_RELATIVE}"],text=True).strip()
            ...
            try:
                ...
                if spec is None or spec.loader is None:self.fail("could not load R3 fixture")
                module=importlib.util.module_from_spec(spec); spec.loader.exec_module(module); loaded=module._load_base_module()
            ...
            self.assertEqual(getattr(module,"PARENT_BASE_MODULE_GIT_BLOB",None),accepted_blob)
            self.assertTrue(hasattr(loaded,"GoError"))

    def test_committed_descendant_base_replacement_is_rejected_before_execution(self) -> None:
        with tempfile.TemporaryDirectory(prefix="nembra-r3-base-module-committed-exec-") as temporary:
            ...
            subprocess.run(["/usr/bin/git", "-C", str(root), "commit", "-qm", "malicious descendant base"], check=True)
            previous = sys.dont_write_bytecode
            sys.dont_write_bytecode = True
            try:
                spec = importlib.util.spec_from_file_location("r3_committed_loader_fixture", r3)
                if spec is None or spec.loader is None:
                    self.fail("could not load R3 committed fixture")
            ...
            self.assertFalse(sentinel.exists(), "R3 executed a descendant-committed base outside exact parent authority")
