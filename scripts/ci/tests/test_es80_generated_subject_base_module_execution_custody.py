...
R3 later pins inherited parent Git blobs, but `_load_base_module()` currently executes the sibling
checkout pathname first. A suppressed worktree replacement must never execute before the exact
parent execution subject has been established.
"""
    ...
    def test_hidden_base_worktree_replacement_never_executes_from_r3_loader(self) -> None:
        with tempfile.TemporaryDirectory(prefix="nembra-r3-base-module-exec-") as temporary:
            ...
            accepted_blob = subprocess.check_output(
                ["/usr/bin/git", "-C", str(root), "rev-parse", f"HEAD:{BASE_RELATIVE}"],
                text=True,
            ).strip()
            subprocess.run(
            ...
            current_blob = subprocess.check_output(
                ["/usr/bin/git", "-C", str(root), "hash-object", "--no-filters", "--", BASE_RELATIVE],
                text=True,
            ).strip()
            self.assertNotEqual(current_blob, accepted_blob, "fixture did not replace accepted base bytes")
            self.assertEqual(
                ...
                "",
                "fixture must hide the replacement from ordinary cleanliness checks",
            )
            ...
            self.assertEqual(
                ...
                "accepted",
                "R3 did not execute the exact accepted base-module subject",
            )
