...
This test is intentionally expected-red until inspection uses one immutable/snapshotted subject.
"""
        ...
        with tempfile.TemporaryDirectory(prefix="nembra-exact-subject-test-") as temporary:
            ...
            def racing_sha256_file(path: Path) -> str:
                ...
                if path == ipa_path and first_input_hash:
                    ...
                    # Model a concurrent replacement after the digest pass. The original bytes are
                    # restored by the extraction seam before publication, so a later retained-file
                    # digest comparison alone cannot prove which bytes were actually inspected.
            ...
            with (
                ...
                patch.object(
                    ...
                    return_value=(
                        ...
                        f"ABCDE12345.{inspector.BUNDLE_ID}",
                    ),
                ),
            ):
