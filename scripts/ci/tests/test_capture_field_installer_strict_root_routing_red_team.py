...
class CaptureFieldInstallerStrictRootRoutingRedTeamTests(unittest.TestCase):
    def test_field_installer_can_still_omit_accepted_generated_manifest_authority(self) -> None:
        ...
        # Bind the attacked production callsite, not a dead helper or documentation string.
        self.assertIn(
        ...
        # Exploit-positive result: the real field invocation has no accepted generated-input
        # digest, so argparse leaves the optional authority absent.
        ...
        # The routing consequence is explicit in production: only accepted-root mode
        # selects native Darwin ACL authority. Without the digest the field build remains
        # on the legacy live-input lease path.
        self.assertIn(
