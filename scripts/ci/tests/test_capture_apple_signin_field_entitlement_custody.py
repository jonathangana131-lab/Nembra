...
# Apple App ID prefixes can legitimately differ from Team ID. Never synthesize
# the signed application identifier as TEAM_ID.bundle-ID.
...
if '$PROFILE_APPLICATION_IDENTIFIER" == "$TEAM_ID.$BUNDLE_ID' in INSTALLER:
    raise SystemExit("profile identity must match the signed app, not a Team-ID-derived App ID")
...
# Every external process in the Apple signing-custody block runs from a closed
# startup environment; caller-controlled loader/Python state cannot influence
# the parser or entitlement/profile inspection.
...
# Signing custody never creates scooter/protocol authority.
for forbidden in ("SIMCTL_CHILD_", "NEMBRA_SIMULATION_", "connectBLE", "publishDps", "queryDps", "writeValue", "scanForPeripherals"):
