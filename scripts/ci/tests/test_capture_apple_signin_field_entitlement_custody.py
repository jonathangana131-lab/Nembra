...
# Apple App ID prefix and Team ID are distinct authorities. The installer must
# never synthesize application-identifier as TEAM_ID.bundle-ID.
...
# Every external process in the Apple entitlement custody block runs from a
# closed startup environment.
...
for forbidden in ("SIMCTL_CHILD_", "NEMBRA_SIMULATION_", "connectBLE", "queryDps", "publishDps", "writeValue"):
