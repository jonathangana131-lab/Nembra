...
# An App ID is App-ID-prefix + bundle-ID. The prefix may legitimately differ
# from Team ID, so the field gate must derive neither identity from the other.
...
# startup environment. Keep the two parsers consolidated so caller state cannot
# slip into a newly-added identity parser.
