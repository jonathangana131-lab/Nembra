#!/usr/bin/env python3
"""Expected-red witness for retained signed-IPA reinspection pathname custody.

The helper first reads/authenticates one stable IPA byte subject. Apple extraction must consume those
same bytes (or a private materialization of them), not reopen the caller-controlled retained path.
"""
    ...
    def test_extraction_consumes_authenticated_ipa_bytes_not_reopened_candidate_path(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            def simulated_ditto(arguments: list[str]) -> subprocess.CompletedProcess[bytes]:
                ...
                # This mutation happens after `_extract_ipa` has already accepted `authenticated`
                # through its `ipa_raw` argument. Current production then reopens `source` here.
            ...
            self.assertEqual(
                marker,
                b"authenticated-A\n",
                "Apple-tool extraction reopened the retained pathname and inspected bytes that were not the descriptor-bound IPA subject",
            )
