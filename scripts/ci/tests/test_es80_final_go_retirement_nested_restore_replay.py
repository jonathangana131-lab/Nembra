#!/usr/bin/env python3
"""Exact replay of #3074's nested-restore schedule against repaired Final-GO.

#3074 was exploit-positive on #3042@899e5dc5...: inherited candidate custody
captured raw base.git/base.git_bytes, retirement overwrote them, then inherited
finally restored the raw functions before enclosing continuous-custody teardown.
This replay preserves that ordering. Acceptance now requires the same outer
teardown Git/blob calls to remain fail-closed because the repaired child makes
the inherited restore target a retirement-aware dispatcher.
"""
...
SPEC = importlib.util.spec_from_file_location("nembra_final_go_nested_restore_replay", SCRIPT)
...
ATTACKED_PARENT = "899e5dc5cc5e71617bfe4151cdb1da20c576fbc2"
REPAIRED_PARENT = "0d7d6045fcf70129524b4d87da0a34d35dcde28c"
...
class RetirementNestedRestoreReplayTests(unittest.TestCase):
    def test_exact_repair_parent_is_selected(self) -> None:
        current = subprocess_check_head()
        self.assertEqual(current, REPAIRED_PARENT)
        self.assertNotEqual(current, ATTACKED_PARENT)
    ...
    def test_inherited_parent_finally_cannot_reopen_git_before_outer_retirement_exits(self) -> None:
        ...
        def fake_continuous(_root: Path, _source: str):
            try:
                ...
                # Same schedule as #3074: execute after inherited custody's
                # unconditional finally has restored its saved functions.
                with self_test.assertRaisesRegex(MODULE.PrivateReviewGoError, "authority retired"):
                    base.git(Path("/candidate"), "status", "--porcelain=v1")
        ...
        def fake_parent_candidate(_base, _root, _source):
            ...
            try:
                ...
                # Deliberately unchanged from the exploit: the inherited parent
                # still restores exactly what it captured before entry.
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="nembra-finalgo-nested-restore-replay-") as temporary:
                result = MODULE.build(
