#!/usr/bin/env python3
"""Regression for trusted Capture runner metadata provenance.
...
``4e9ae0cb6728dc68d9b8dd43aac7c50128702ed9`` retains ``githubRunID`` and
``githubRunAttempt`` in ``capture-runner-metadata.json`` and falls back to ``local`` / ``0`` only
when those values are absent for legitimate local execution.

A trusted GitHub Actions run must therefore reintroduce GitHub-context-expanded run identity inside
the clean interpreter environment. Inheriting caller environment variables would reopen the
boundary; omitting the values records false CI provenance.

This regression is software-provenance custody only. It does not authorize physical Experiment One.
"""
...
class TrustedRunnerMetadataContractTests(unittest.TestCase):
    def test_pinned_producer_requires_trusted_github_run_identity(self) -> None:
        self.assertEqual(
            ...
            "re-review this regression when the trusted Simulator producer pin changes",
        )
