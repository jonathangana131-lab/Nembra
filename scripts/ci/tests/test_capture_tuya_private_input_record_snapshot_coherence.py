...
class CaptureTuyaPrivateInputRecordSnapshotCoherenceTests(unittest.TestCase):
    ...
    def test_record_rejects_earlier_tree_entry_mutation_during_later_tree_fingerprint(self) -> None:
        original_tree_fingerprint = provenance._tree_fingerprint
        original_security_bytes = self.security_binary.read_bytes()
        mutated_and_restored = False
        ...
        def fingerprint_then_cross_tree_mutation(path: Path) -> str:
            ...
            result = original_tree_fingerprint(path)
            if path == self.identity_sources and not mutated_and_restored:
                self.security_binary.write_bytes(b"different-security-bytes")
                self.security_binary.write_bytes(original_security_bytes)
                mutated_and_restored = True
            return result
    ...
    def test_build_record_owns_explicit_whole_record_generation_fence(self) -> None:
        ...
        self.assertIn("generation", build_record_source.lower())
        self.assertIn("before", build_record_source.lower())
        self.assertIn("after", build_record_source.lower())
