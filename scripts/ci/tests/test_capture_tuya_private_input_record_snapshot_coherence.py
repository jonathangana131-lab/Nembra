...
class CaptureTuyaPrivateInputRecordSnapshotCoherenceTests(unittest.TestCase):
    ...
    def test_record_rejects_earlier_file_mutation_during_later_tree_fingerprint(self) -> None:
    ...
    def test_record_rejects_earlier_tree_entry_mutation_during_later_tree_fingerprint(self) -> None:
        original_tree_fingerprint = provenance._tree_fingerprint
        mutated_and_restored = False
        original_bytes = self.security_binary.read_bytes()

        def fingerprint_then_cross_tree_mutation(path: Path) -> str:
            nonlocal mutated_and_restored
            result = original_tree_fingerprint(path)
            if path == self.identity_sources and not mutated_and_restored:
                self.security_binary.write_bytes(b"security-bytes-v2")
                self.security_binary.write_bytes(original_bytes)
                mutated_and_restored = True
            return result
        ...
        with mock.patch.object(
            provenance,
            "_tree_fingerprint",
            side_effect=fingerprint_then_cross_tree_mutation,
        ):
            with self.assertRaises(provenance.ProvenanceError):
                self.build_record()
    ...
    def test_build_record_has_one_whole_snapshot_boundary_not_only_per_component_reads(self) -> None:
        ...
        self.assertIn("record_snapshot", build_record_source)
        self.assertIn("_record_input_generation_snapshot", build_record_source)
        self.assertIn("_assert_record_snapshot_unchanged", build_record_source)
        self.assertLess(
            build_record_source.index("record_snapshot"),
            build_record_source.index("_read_stable_regular_file_sha256(lockfile)"),
        )
        self.assertGreater(
            build_record_source.rindex("_assert_record_snapshot_unchanged"),
            build_record_source.index("_tree_fingerprint(identity_sources)"),
        )
