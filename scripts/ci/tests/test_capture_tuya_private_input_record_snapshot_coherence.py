...
class CaptureTuyaPrivateInputRecordSnapshotCoherenceTests(unittest.TestCase):
    ...
    def test_generation_snapshot_tracks_tree_entry_and_directory_chronology(self) -> None:
        before = provenance._record_input_generation_snapshot(
            lockfile=self.lockfile,
            security_podspec=self.security_podspec,
            security_build=self.security_build,
            identity_podspec=self.identity_podspec,
        ...
        target = self.identity_sources / "NembraTuyaPrivateIdentity.swift"
        original = target.read_text(encoding="utf-8")
        target.write_text(original + "// transient mutation\n", encoding="utf-8")
        target.write_text(original, encoding="utf-8")
        after = provenance._record_input_generation_snapshot(
            lockfile=self.lockfile,
            security_podspec=self.security_podspec,
            security_build=self.security_build,
            identity_podspec=self.identity_podspec,
