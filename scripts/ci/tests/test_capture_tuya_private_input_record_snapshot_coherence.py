...
class CaptureTuyaPrivateInputRecordSnapshotCoherenceTests(unittest.TestCase):
    ...
    def test_record_rejects_earlier_input_mutation_during_later_tree_fingerprint(self) -> None:
        ...
        with mock.patch.object(
            provenance,
            "_tree_fingerprint",
            side_effect=fingerprint_then_cross_input_mutation,
        ):
            with self.assertRaises(provenance.ProvenanceError):
                self.build_record()
        self.assertTrue(mutated_and_restored)

    def test_build_record_has_explicit_whole_record_generation_boundary(self) -> None:
        ...
        self.assertIn("_record_generation_witness", build_record_source)
        self.assertIn("before_generation", build_record_source)
        self.assertIn("after_generation", build_record_source)
        self.assertIn("private build input set changed while provenance record was built", build_record_source)

    def test_file_path_rendezvous_rejects_same_inode_metadata_drift(self) -> None:
    ...
    def test_tree_generation_witness_covers_nested_identity_and_membership(self) -> None:
        witness = provenance._tree_generation_witness(self.identity_sources)
        paths = {entry[1] for entry in witness}
        self.assertIn(".", paths)
        self.assertIn("NembraTuyaPrivateIdentity.swift", paths)

        previous = witness
        nested = self.identity_sources / "Nested"
        nested.mkdir()
        (nested / "config.bin").write_bytes(b"opaque")
        self.assertNotEqual(previous, provenance._tree_generation_witness(self.identity_sources))
