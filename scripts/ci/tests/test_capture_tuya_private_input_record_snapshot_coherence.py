...
class CaptureTuyaPrivateInputRecordSnapshotCoherenceTests(unittest.TestCase):
    ...
    def test_build_record_has_explicit_whole_record_identity_snapshot(self) -> None:
        ...
        self.assertIn("_record_identity_snapshot", build_record_source)
        self.assertIn("record_identity_before", build_record_source)
        self.assertIn("record_identity_after", build_record_source)
        self.assertIn(
            "private Tuya build inputs changed while the complete provenance record was constructed",
            build_record_source,
        )

    def test_snapshot_identity_covers_every_nested_tree_path(self) -> None:
        nested = self.identity_sources / "Nested"
        nested.mkdir()
        (nested / "opaque.bin").write_bytes(b"opaque")
        snapshot = provenance._record_tree_identity_snapshot(self.identity_sources)
        paths = {relative for relative, _ in snapshot}
        self.assertEqual(
            paths,
            {".", "NembraTuyaPrivateIdentity.swift", "Nested", "Nested/opaque.bin"},
        )
