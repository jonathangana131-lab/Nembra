...
class CaptureTuyaPrivateInputRecordSnapshotCoherenceTests(unittest.TestCase):
    ...
    def test_record_rejects_earlier_input_mutation_during_later_tree_fingerprint(self) -> None:
        ...
        def fingerprint_then_cross_input_mutation(path: Path) -> str:
            ...
            if path == self.identity_sources and not mutated_and_restored:
                # The lockfile was fingerprinted before either private tree. Mutate and
                # restore it while the final tree is being admitted. A record-level
                # snapshot fence must notice that the earlier authority changed even
                # though its final bytes match the bytes originally fingerprinted.
    ...
    def test_build_record_has_one_whole_snapshot_boundary_not_only_per_component_reads(self) -> None:
        ...
        # A finite private-input authority record needs a record-level before/after
        # fence or an equivalent generation witness. Returning directly from five
        # sequential fingerprints is not sufficient whole-record chronology.
