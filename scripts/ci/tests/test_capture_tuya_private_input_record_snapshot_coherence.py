...
class CaptureTuyaPrivateInputRecordSnapshotCoherenceTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.identity_sources / "NembraTuyaPrivateIdentity.swift").write_text(
            "private fixture identity source\n",
            encoding="utf-8",
        )
    ...
    def test_build_record_has_one_whole_snapshot_boundary_not_only_per_component_reads(self) -> None:
        ...
        self.assertIn("record_snapshot_before != record_snapshot_after", build_record_source)

    def test_mutate_restore_inside_earlier_tree_is_rejected_while_later_authority_is_fingerprinted(self) -> None:
        ...
        with mock.patch.object(provenance, "_tree_fingerprint", side_effect=fingerprint_then_cross_tree_mutation):
            with self.assertRaises(provenance.ProvenanceError):
                self.build_record()
        self.assertTrue(mutated_and_restored)
