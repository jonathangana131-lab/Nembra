#!/usr/bin/env python3
"""Permanent regressions for private identity descendant/name rebinding.

These attacks are retained from #3206 against the recovered #2755 production
line. Rejection may not publish credential identity bytes into a replacement
canonical descendant tree, and success/rejection may not leave attacker bytes
at the canonical credential destination after the accepted final descriptor
...
def load_writer():
    spec = importlib.util.spec_from_file_location(
        "nembra_private_identity_descendant_rebind_current",
        WRITER_PATH,
    )
        ...
        with tempfile.TemporaryDirectory(prefix="nembra-private-final-name-rebind-") as temporary:
            ...
            def adversarial_open(root_fd: int, relative_path: str) -> int:
                ...
                if not attacked and relative_path.endswith("NembraTuyaPrivateIdentity.swift"):
                    ...
                    displaced_identity = canonical.with_name(canonical.name + ".sealed-displaced")
            ...
            self.assertNotEqual(
                canonical_bytes,
                attacker_payload,
                "writer returned/rejected while attacker bytes remained at the canonical identity path",
            )
            if not rejected:
                self.assertIsNotNone(canonical_bytes)
                assert canonical_bytes is not None
                self.assertIn(key_b64.encode("ascii"), canonical_bytes)
