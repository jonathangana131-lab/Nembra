#!/usr/bin/env python3
"""Expected-red current-R3 diagnostic for generated-subject helper execution custody."""
...
if SPEC is None or SPEC.loader is None: raise RuntimeError("could not load generated-subject Final-GO R3 child")
...
class GeneratedSubjectHelperExecutionCustodyTests(unittest.TestCase):
    def test_swap_restore_helper_at_execution_boundary_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory(prefix="nembra-r3-generated-helper-exec-") as temporary:
            root = Path(temporary).resolve(strict=True); repository = root / "candidate"; repository.mkdir()
            ...
            accepted_digest="a"*64; reviewed_helper_output="b"*64
            helper=repository/GO.GENERATED_HELPER_PATH; helper.parent.mkdir(parents=True,exist_ok=True)
            accepted_helper_bytes=("#!/usr/bin/env python3\n"+f"# {GO.GENERATED_SCHEMA}\n"+f"print({reviewed_helper_output!r})\n")
            substituted_helper_bytes=("#!/usr/bin/env python3\n# attacker-controlled replacement; ignores build inputs\n"+f"print({accepted_digest!r})\n")
            helper.write_text(accepted_helper_bytes,encoding="utf-8")
            fixtures={
                "Scripts/bootstrap_capture_tuya_sdk.sh":f"# {GO.GENERATED_ENV}\n# capture_cocoapods_generated_build_subject.py\n",
                "Scripts/capture_tuya_private_input_provenance.py":"# private provenance helper\n",
                "Scripts/capture_tuya_private_input_build_guard.py":"# capture_cocoapods_generated_build_subject.py\n# _verify_accepted_generated_build_subject\n# require_accepted_generated_subject=True\n# _require_real_checkout_ancestry\n# _ensure_fd_budget\n# KQ_NOTE_ATTRIB\n",
                "scripts/field/install_one_time_capture.command":"# bootstrap_capture_tuya_sdk.sh\n# capture_tuya_private_input_build_guard.py\n",
                GO.GENERATED_BUILD_WORKFLOW_PATH:"name: Capture CocoaPods Build Subject Authority\n# Require exact generated CocoaPods build authority\n# test_capture_private_input_ancestor_retarget.py\n",
                GO.VNODE_WORKFLOW_PATH:"name: Capture CocoaPods Vnode Attribute Convergence\n# Real macOS chmod vnode evidence\n# macos-15\n",
            }
            for relative,text in fixtures.items():
                path=repository/relative; path.parent.mkdir(parents=True,exist_ok=True); path.write_text(text,encoding="utf-8")
            (repository/"Podfile.lock").write_text("PODS:\n",encoding="utf-8"); (repository/"Pods").mkdir(); (repository/"NembraCapture.xcworkspace").mkdir()
            ...
            base=GO._load_base_module(); real_run=subprocess.run; swapped=False; replacement_was_consumed=False
            def swap_at_helper_exec(args,*positional,**keyword):
                ...
                if len(argv)>=4 and argv[0]=="/usr/bin/python3" and argv[1:3]==["-I","-B"] and argv[3]==str(helper):
                    swapped=True; helper.write_text(substituted_helper_bytes,encoding="utf-8")
                    try:
                        result=real_run(args,*positional,**keyword); replacement_was_consumed=result.stdout.strip()==accepted_digest; return result
                    finally: helper.write_text(accepted_helper_bytes,encoding="utf-8")
                return real_run(args,*positional,**keyword)
            with mock.patch.object(GO.subprocess,"run",side_effect=swap_at_helper_exec):
                with self.assertRaises(GO.GeneratedSubjectGoError,msg="R3 accepted a reviewed digest emitted by substituted helper execution bytes"):
                    GO.candidate_generated_authority(repository,source,accepted_digest,base=base)
            self.assertTrue(swapped,"diagnostic never reached the real helper execution boundary")
            self.assertTrue(replacement_was_consumed,"substituted helper bytes did not control the derived digest")
