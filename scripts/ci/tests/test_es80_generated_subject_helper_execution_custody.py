#!/usr/bin/env python3
"""Regression for R3 generated-subject helper execution custody."""
...
SPEC = importlib.util.spec_from_file_location("generated_subject_final_go_custody", MODULE)
...
class GeneratedSubjectHelperExecutionCustodyTests(unittest.TestCase):
    def test_swap_restore_path_cannot_change_accepted_git_blob_execution(self) -> None:
        with tempfile.TemporaryDirectory(prefix="nembra-r3-generated-helper-exec-") as temporary:
            ...
            for relative, source_text in fixtures.items():
            ...
            attacker_path_was_present = False
            accepted_blob_controlled_stdout = False

            def swap_at_helper_exec(args, *positional, **keyword):
                nonlocal execution_boundary_reached, attacker_path_was_present, accepted_blob_controlled_stdout
                ...
                if (
                    ...
                    and argv[1:3] == ["-I", "-B"]
                    and argv[3] == "-c"
                    and argv[5] == f"git:{GO.GENERATED_HELPER_PATH}"
                ):
                    execution_boundary_reached = True
                    helper.write_text(substituted_helper_bytes, encoding="utf-8")
                    attacker_path_was_present = helper.read_text(encoding="utf-8") == substituted_helper_bytes
                    try:
                        result = real_run(args, *positional, **keyword)
                        accepted_blob_controlled_stdout = result.stdout.decode("utf-8").strip() == accepted_digest
                        return result
                    finally:
                        helper.write_text(accepted_helper_bytes, encoding="utf-8")
            ...
            self.assertEqual(authority[GO.GENERATED_KEY], accepted_digest)
            self.assertTrue(execution_boundary_reached, "regression never reached accepted helper execution")
            self.assertTrue(attacker_path_was_present, "regression did not replace the mutable helper pathname")
            self.assertTrue(
                accepted_blob_controlled_stdout,
                "mutable helper pathname controlled digest instead of accepted Git blob bytes",
            )
