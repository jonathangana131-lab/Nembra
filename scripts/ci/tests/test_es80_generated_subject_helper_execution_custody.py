    ...
    def test_swap_restore_worktree_helper_cannot_change_executed_semantics(self) -> None:
        with tempfile.TemporaryDirectory(prefix="nembra-r3-generated-helper-exec-") as temporary:
            ...
            fixtures = {
                ...
                GO.GENERATED_BUILD_WORKFLOW_PATH: (
            ...
            def swap_at_python_stdin_exec(args, *positional, **keyword):
                ...
                if len(argv) >= 4 and argv[0] == "/usr/bin/python3" and argv[1:4] == ["-I", "-B", "-"]:
                    swapped = True
            ...
            with mock.patch.object(GO.subprocess, "run", side_effect=swap_at_python_stdin_exec):
                record = GO.candidate_generated_authority(repository, source, accepted_digest, base=base)
            ...
            self.assertEqual(
                subprocess.check_output(["/usr/bin/git", "-C", str(repository), "status", "--porcelain=v1", "--untracked-files=all"], text=True),
                "",
