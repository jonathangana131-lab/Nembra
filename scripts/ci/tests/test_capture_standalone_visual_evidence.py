...
if 'INFOPLIST_KEY_NembraCaptureProcedureIdentifier="$procedure"' in workflow:
    raise SystemExit("visual workflow must exercise the named project procedure setting, not bypass it with a direct plist override")

project_mapping = 'INFOPLIST_KEY_NembraCaptureProcedureIdentifier = "$(NEMBRA_CAPTURE_PROCEDURE_IDENTIFIER)";'
if project.count(project_mapping) != 2:
    raise SystemExit(f"expected exactly two project procedure mappings, found {project.count(project_mapping)}")
