#!/usr/bin/env python3
"""Expected-red macOS witness for current selected Xcode tool ACL custody.
...
class CaptureFieldSelectedXcodeToolACLCurrentRedTeamTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.fixture_dir = Path(f"/Applications/NembraToolACLCurrent-{os.getpid()}")
    ...
    def test_root_owned_0755_tool_with_write_acl_is_rejected_before_authority(self) -> None:
        ...
        self.assertEqual(
            ...
            0,
            "fixture must look non-group/world-writable in BSD mode bits",
        )

        acl = self._run(["/bin/ls", "-lde", str(self.fixture)]).stdout
        print("NEMBRA_ACL_CURRENT_WITNESS_LS_BEGIN")
        print(acl.rstrip())
        print("NEMBRA_ACL_CURRENT_WITNESS_LS_END")
        ...
        self.assertNotEqual(
            before,
            after,
            "ACL-granted field user must mechanically mutate the root-owned 0755 subject",
        )
