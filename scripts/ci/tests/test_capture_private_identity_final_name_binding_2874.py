#!/usr/bin/env python3
"""Expected-red attack on #2874 private identity destination-name binding.

The attacked parent binds payload bytes to the held final descriptor, but a distinct
same-UID race can replace the destination pathname after that descriptor is opened
and verified. Success must not coexist with attacker bytes at the credential name.
"""
...
def load_writer():
    spec = importlib.util.spec_from_file_location(
        "nembra_private_identity_final_name_2874_redteam",
        WRITER_PATH,
    )
...
class PrivateIdentityFinalNameBinding2874Tests(unittest.TestCase):
    def test_destination_name_swap_after_verified_open_cannot_return_success(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="nembra-private-final-name-2874-") as temporary:
            ...
            self.assertFalse(
                ...
                "#2874 writer reported success while the credential destination named attacker bytes",
            )
            if succeeded:
