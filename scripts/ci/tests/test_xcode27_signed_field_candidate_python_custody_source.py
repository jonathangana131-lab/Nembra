...
class SignedFieldCandidatePythonCustodySourceTests(unittest.TestCase):
    ...
    def test_every_pinned_python_invocation_uses_isolated_mode(self):
        invocation_lines = [
            ...
            if '"$PYTHON3"' in line and 'validate_root_custodied_path' not in line
        ]
        self.assertTrue(invocation_lines, "Expected field-candidate producer to invoke its pinned Python executable")
        non_isolated = [
            line for line in invocation_lines
            if not re.search(r'"\$PYTHON3"\s+-I(?:\s|$)', line)
        ]
        self.assertEqual(
            non_isolated,
            [],
