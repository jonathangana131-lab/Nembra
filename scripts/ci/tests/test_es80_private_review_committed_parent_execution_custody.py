#!/usr/bin/env python3
"""Prove committed descendant parent bytes cannot replace exact Final-GO parents.

The current Final-GO is now a successor of exact #2921, and #2921 is itself a
successor of exact #2873. The fixture preserves the original committed-parent
execution invariant at both layers instead of assuming the current worktree
module directly loads #2873.
"""
...
def _exact_direct_parent_source() -> str:
    payload = subprocess.check_output(
        ["/usr/bin/git", "-C", str(REPOSITORY), "cat-file", "blob", DIRECT_PARENT_BLOB]
    )
    if _blob_oid(payload) != DIRECT_PARENT_BLOB:
        raise RuntimeError("test fixture direct-parent Git lookup changed exact #2921 bytes")
    return payload.decode("utf-8")


class PrivateReviewCommittedParentExecutionCustodyTests(unittest.TestCase):
    def _write_historical_parent_fixture(self, parent: Path) -> None:
        parent.write_text(
            "#!/usr/bin/env python3\n"
            "import contextlib\n"
            "import types\n"
    ...
    def test_committed_descendant_parent_replacement_never_executes(self) -> None:
        ...
        accepted_2921_source = _exact_direct_parent_source()
        ...
        with tempfile.TemporaryDirectory(prefix="nembra-private-parent-descendant-") as temporary:
            ...
            # Commit the accepted historical semantic parent first. #2921's
            # pinned source/blob will be rewritten to this exact fixture commit.
            self._write_historical_parent_fixture(parent)
            subprocess.run(["/usr/bin/git", "-C", str(root), "add", PARENT_RELATIVE], check=True)
            subprocess.run(
                ["/usr/bin/git", "-C", str(root), "commit", "-qm", "accepted historical private parent"],
                check=True,
            )
            ...
            historical_blob = subprocess.check_output(
                ["/usr/bin/git", "-C", str(root), "rev-parse", f"{historical_source}:{PARENT_RELATIVE}"],
                text=True,
            ).strip().lower()

            # Materialize exact #2921 source with only its accepted historical
            # parent pins redirected to the fixture above. This exercises the
            # same nested exact-Git execution mechanism as production #2921.
            fixture_2921 = accepted_2921_source.replace(
            ...
            self.assertIn(f'PARENT_SOURCE = "{historical_source}"', fixture_2921)
            self.assertIn(f'PARENT_MODULE_GIT_BLOB = "{historical_blob}"', fixture_2921)
            parent.write_text(fixture_2921, encoding="utf-8")
            subprocess.run(["/usr/bin/git", "-C", str(root), "add", PARENT_RELATIVE], check=True)
            subprocess.run(
                ["/usr/bin/git", "-C", str(root), "commit", "-qm", "accepted exact-parent successor"],
                check=True,
            )
            direct_source = subprocess.check_output(
                ["/usr/bin/git", "-C", str(root), "rev-parse", "HEAD"], text=True
            ).strip().lower()
            direct_blob = subprocess.check_output(
                ["/usr/bin/git", "-C", str(root), "rev-parse", f"{direct_source}:{PARENT_RELATIVE}"],
                text=True,
            ).strip().lower()

            # Patch only the current successor's direct-parent identity. Its
            # public PARENT_SOURCE/_parent surface remains inherited from the
            # pinned #2921 fixture and therefore continues proving #2873-style
            # execution custody as well.
            ...
            self.assertIn(f'DIRECT_PARENT_SOURCE = "{direct_source}"', patched_child)
            self.assertIn(f'DIRECT_PARENT_MODULE_GIT_BLOB = "{direct_blob}"', patched_child)
            ...
            # Commit attacker-controlled descendant bytes at the same module
            # pathname. Both the current child and its exact #2921 parent must
            # continue executing only their previously accepted Git blobs.
            parent.write_text(
                "#!/usr/bin/env python3\n"
                "from pathlib import Path\n"
            ...
            subprocess.run(["/usr/bin/git", "-C", str(root), "add", PARENT_RELATIVE], check=True)
            subprocess.run(
                ["/usr/bin/git", "-C", str(root), "commit", "-qm", "attacker replaces current parent path"],
                check=True,
            )
            ...
            self.assertFalse(
                sentinel.exists(),
                "committed descendant parent bytes executed instead of pinned accepted Git parents",
            )
