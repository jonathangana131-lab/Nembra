#!/usr/bin/env python3
"""Permanent contract for exact selected Xcode executable-file custody.
...
Source-only, hardware-free regression. It authorizes no physical experiment.
"""
...
class CaptureFieldSelectedXcodeToolFileCustodyTests(unittest.TestCase):
    ...
    def test_parent_directory_custody_cannot_substitute_for_exact_file_custody(self) -> None:
        ...
        self.assertTrue(
            ...
            "selected xcodebuild parent custody exists, but the exact executable file never becomes a root-owned non-group/world-writable authority subject",
        )
    ...
    def test_exact_file_custody_primitive_proves_file_identity_owner_and_mode(self) -> None:
        ...
        if helper is None:
            self.fail("no exact selected-xcodebuild file-custody helper is called")
    ...
    def test_physical_xcrun_tools_have_file_level_authority_not_only_developer_dir_selection(self) -> None:
        for tool in ("xctrace", "devicectl"):
            ...
            self.assertIsNotNone(
                selected_assignment,
                f"physical {tool} must resolve one exact executable subject for file-level custody",
            )
            ...
            self.assertIsNotNone(
                exact_file_call,
                f"selected {tool} executable has no exact file-level root/nonwritable custody before physical use",
            )
            self.assertRegex(
                self.source,
                re.compile(rf'(?m)^.*"\${variable}"\s+(?:list|device|process)\b'),
                f"physical {tool} must execute the exact admitted executable rather than re-resolve through xcrun",
            )
