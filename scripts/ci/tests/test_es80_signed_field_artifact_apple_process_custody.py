...
def assert_closed_process_boundary(testcase: unittest.TestCase, kwargs: dict) -> None:
    ...
    testcase.assertIsInstance(environment, dict)
    assert isinstance(environment, dict)
    testcase.assertEqual(environment.get("PATH"), "/usr/bin:/bin")
    for key, poisoned_value in POISONED_ENV.items():
        if key != "PATH":
            testcase.assertNotEqual(environment.get(key), poisoned_value)
    testcase.assertEqual(kwargs.get("cwd"), "/")
    testcase.assertEqual(kwargs.get("stdin"), subprocess.DEVNULL)
...
class SignedFieldArtifactAppleProcessCustodyTests(unittest.TestCase):
    ...
    def test_security_profile_decode_ignores_poisoned_caller_process_state(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="nembra-apple-process-custody-") as temporary:
            ...
            app_path.mkdir()
            (app_path / "embedded.mobileprovision").write_bytes(b"profile fixture")
            with (
                mock.patch.dict(os.environ, POISONED_ENV, clear=False),
                mock.patch.object(inspector.sys, "platform", "darwin"),
                mock.patch.object(inspector, "_trusted_system_apple_tool", return_value="/usr/bin/security"),
                mock.patch.object(inspector, "read_effective_signed_entitlements", return_value={}),
                mock.patch.object(inspector, "read_leaf_signing_certificate_der", return_value=b"certificate fixture"),
                mock.patch.object(inspector, "validate_provisioning_profile", return_value=("PROFILE-UUID", "2099-01-01T00:00:00Z", "TEAM.bundle")),
                mock.patch.object(inspector.subprocess, "run", return_value=completed) as run,
            ):
    ...
    def test_codesign_timeout_is_fail_closed(self) -> None:
        inspector = load_inspector()
        with mock.patch.object(
            inspector.subprocess,
            "run",
            side_effect=subprocess.TimeoutExpired(cmd=["/usr/bin/codesign"], timeout=30),
        ):
            with self.assertRaises(inspector.EvidenceError) as caught:
                inspector._run_text(["/usr/bin/codesign", "--version"])
        self.assertIn("timed out", str(caught.exception).lower())

    def test_apple_tool_spawn_failure_is_fail_closed(self) -> None:
        inspector = load_inspector()
        with mock.patch.object(inspector.subprocess, "run", side_effect=OSError("fixture")):
            with self.assertRaises(inspector.EvidenceError) as caught:
                inspector._run_text(["/usr/bin/codesign", "--version"])
        self.assertIn("execute", str(caught.exception).lower())
