...
class PrivateIdentityCheckoutRootSwapTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        # symlink to /private/var. Production ancestry custody correctly rejects
        # symlink ancestry, so tests must use the real physical spelling rather
        # than weakening O_NOFOLLOW just to accommodate the platform alias.
        ...
        self.sandbox = Path(self.temporary.name).resolve(strict=True)
