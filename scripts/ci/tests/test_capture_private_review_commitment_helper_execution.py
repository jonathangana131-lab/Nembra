...
class BootstrapCommitmentHelperExecutionTests(authority.PrivateReviewAuthorityTests):
    def test_substituted_commitment_helper_cannot_admit_generation_b(self) -> None:
        ...
        # Replace the private generation and its continuity witness coherently.
        # This is allowed to be same-UID mutable; the fixed external HMAC tag A is
        # what must prevent B from becoming field authority.
        ...
        # Replace only the helper implementation that normal bootstrap reopens.
        # It cannot forge tag A from B; it simply lies and exits successfully.
...
class BuildGuardCommitmentHelperExecutionTests(unittest.TestCase):
    def test_substituted_neighbor_commitment_helper_is_not_build_authority(self) -> None:
        with tempfile.TemporaryDirectory(prefix="nembra-private-commitment-guard-neighbor-") as temporary:
            ...
            # Satisfy every symbol the guard expects from the neighbor while
            # intentionally making HMAC verification attacker-controlled.
