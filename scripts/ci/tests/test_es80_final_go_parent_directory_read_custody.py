        ...
        with tempfile.TemporaryDirectory(prefix="nembra-final-go-parent-directory-") as temporary:
            ...
            try:
                with self.assertRaises(
                    RuntimeError,
                    msg="Final-GO admitted a payload after its accepted parent namespace was replaced",
                ):
                    MODULE._read_physical_payload(root, relative, b"100644")
            finally:
                MODULE.os.stat = original_stat

            self.assertTrue(attack_fired, "fixture did not reach post-ancestry final admission")
            self.assertEqual(
