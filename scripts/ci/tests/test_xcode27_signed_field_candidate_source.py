...
class SignedFieldCandidateProducerSourceTests(unittest.TestCase):
    ...
    def test_targets_real_ios_and_injects_exact_build_rendezvous_and_launch_recipe(self):
        ...
        self.assertIn('NEMBRA_CAPTURE_BUILD_IDENTIFIER=$BUILD_IDENTIFIER', self.source)
        self.assertIn('NEMBRA_CAPTURE_BUILD_INSTANCE_ID=$BUILD_INSTANCE_ID', self.source)
        self.assertIn('NEMBRA_CAPTURE_BUILD_COMMIT_SHA=$SOURCE_SHA', self.source)
        self.assertIn('FIELD_RECIPE_ID="ES80-FINGERPRINT-v1"', self.source)
        self.assertIn('NEMBRA_CAPTURE_FIELD_RECIPE=$FIELD_RECIPE_ID', self.source)
        self.assertIn('info.get("NembraCaptureFieldRecipe") != field_recipe', self.source)
        self.assertNotIn('INFOPLIST_KEY_NembraCaptureBuild', self.source)
