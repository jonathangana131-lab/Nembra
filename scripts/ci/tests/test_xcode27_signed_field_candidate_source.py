...
class SignedFieldCandidateProducerSourceTests(unittest.TestCase):
    ...
    def test_verifies_exact_intended_device_without_persisting_udid(self):
        self.assertIn(
            ': "${NEMBRA_INTENDED_DEVICE_UDID:?Set NEMBRA_INTENDED_DEVICE_UDID',
            self.source,
        )
        self.assertIn(
        ...
        self.assertNotIn('echo "device_udid=', self.source)
