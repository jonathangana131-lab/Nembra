...
class SignedFieldCandidateProducerSourceTests(unittest.TestCase):
    ...
    def test_forwards_intended_device_only_through_private_runner(self):
        ...
        self.assertIn('--validate-only', self.source)
        self.assertIn(
            '--intended-device-udid-file "$NEMBRA_INTENDED_FIELD_DEVICE_UDID_FILE"',
            self.source,
        )
        self.assertIn('did not pass private verification-input validation', self.source)
        self.assertNotIn(
