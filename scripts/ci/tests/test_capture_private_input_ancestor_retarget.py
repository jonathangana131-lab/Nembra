#!/usr/bin/env python3
"""Expected-red regression for Capture field-build private-input ancestry.
...
path while a same-UID actor retargets that ancestor to different private bytes.
"""
...
class PrivateInputAncestorRetargetTests(unittest.TestCase):
    def test_guard_rejects_private_sdk_ancestor_retarget_before_child_consumption(self) -> None:
        with tempfile.TemporaryDirectory(prefix="nembra-private-input-retarget-") as temporary:
            ...
            def launch(_command):
                # The guard has admitted/watched resolved SDK A. Retarget the
                # checkout-visible path that the real generated build may reopen.
                sdk_link.unlink()
