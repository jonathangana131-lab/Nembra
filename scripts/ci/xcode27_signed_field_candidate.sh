...
# Device identity is needed only to prove the provisioning profile authorizes the intended field
# iPhone. Never print, persist, hash into evidence, or pass this value to xcodebuild.
if ! python3 - "$NEMBRA_FIELD_DEVICE_UDID" <<'PY'
import sys
value = sys.argv[1]
valid = (
...
PY
then
  echo "NEMBRA_FIELD_DEVICE_UDID must be one non-empty canonical field-device identifier without whitespace/control characters." >&2
  exit 5
fi
...
# macOS still ships Bash 3.2. Avoid optionally empty arrays under `set -u`.
run_xcodebuild() {
...
# A dirty invocation checkout is never accepted. The actual build below is also performed from a
# fresh detached worktree at SOURCE_SHA so concurrent workers/ignored files cannot become stamped
# field bytes.
...
INSPECTOR_OUTPUT_DIR="$WORK_ROOT/inspector-output"
...
# Final field-production evidence is one immutable directory. Resolve lexical traversal/symlink
# ancestors, refuse destructive destinations, and never mix a new candidate into old evidence.
...
if [[ -e "$ARTIFACTS_DIR" || -e "$FINAL_STAGING_DIR" ]]; then
  echo "Field-production destination/staging already exists; refusing to mix or overwrite evidence." >&2
  exit 11
fi
...
mkdir -p "$ARTIFACTS_PARENT" "$WORK_ROOT" "$LOG_DIR"
cp -p "$EXPORT_OPTIONS_PLIST" "$EXPORT_OPTIONS_SNAPSHOT"
/usr/bin/plutil -lint "$EXPORT_OPTIONS_SNAPSHOT" >/dev/null

# Export policy is an external release input. Snapshot the exact bytes xcodebuild will consume,
# verify team ownership, and retain/hash the snapshot for independent acceptance.
...
cleanup() {
  ...
  rm -rf "$WORK_ROOT" "$FINAL_STAGING_DIR"
}
trap cleanup EXIT
...
# Python performs exact IPA selection so Bash 3.2 never expands an optionally empty nounset array.
...
# Inspector output is staged separately. The intended-device UDID is verification-only: the
# inspector validates provisioning coverage but deliberately never persists or prints that value.
python3 scripts/ci/es80_signed_field_artifact_evidence.py \
  --ipa "$IPA_PATH" \
  --expected-source-sha "$SOURCE_SHA" \
  --intended-device-udid "$NEMBRA_FIELD_DEVICE_UDID" \
  --output-dir "$INSPECTOR_OUTPUT_DIR"

EXTERNAL_RECORD="$INSPECTOR_OUTPUT_DIR/NembraCaptureExternalBuildRecord.json"
FIELD_BUILD_RECORD="$INSPECTOR_OUTPUT_DIR/NembraCaptureFieldBuildEvidenceRecord.json"
SIGNING_INSPECTION="$INSPECTOR_OUTPUT_DIR/NembraCaptureSignedFieldArtifactInspection.json"
RETAINED_IPA="$INSPECTOR_OUTPUT_DIR/build-evidence/NembraField.ipa"

# Re-prove the stable exact-byte authority links after the canonical inspector has published its
# complete temporary directory. Signing/provisioning semantics themselves remain owned by inspector.
python3 - \
  "$EXTERNAL_RECORD" \
  "$FIELD_BUILD_RECORD" \
  "$SIGNING_INSPECTION" \
...
for record_name, record in (("field-build evidence", field), ("signing inspection", inspection)):
    for key, value in shared_expected.items():
        if record.get(key) != value:
            raise SystemExit(f"{record_name} mismatch for {key}")
...
if digest != field.get("signedInstallableSHA256"):
    raise SystemExit("Retained IPA bytes do not match canonical field-build evidence")
PY

# Assemble release inputs/logs with canonical inspector output, then publish the complete candidate
# once using macOS RENAME_EXCL. A crash/race cannot expose a partially assembled final directory or
# overwrite a concurrently published candidate.
mkdir -p "$FINAL_STAGING_DIR"
cp -R "$INSPECTOR_OUTPUT_DIR"/. "$FINAL_STAGING_DIR"/
cp -p "$EXPORT_OPTIONS_SNAPSHOT" "$FINAL_STAGING_DIR/ExportOptions.plist"
mkdir -p "$FINAL_STAGING_DIR/logs"
...
# Ownership moved atomically to ARTIFACTS_DIR; prevent cleanup from targeting the published path.
FINAL_STAGING_DIR="$WORK_ROOT/published-staging-no-longer-exists"
