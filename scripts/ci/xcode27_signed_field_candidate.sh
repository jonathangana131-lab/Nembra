...
case "$NEMBRA_INTENDED_FIELD_DEVICE_UDID_FILE" in
  /*) INTENDED_DEVICE_UDID_FILE="$NEMBRA_INTENDED_FIELD_DEVICE_UDID_FILE" ;;
  *) INTENDED_DEVICE_UDID_FILE="$ROOT/$NEMBRA_INTENDED_FIELD_DEVICE_UDID_FILE" ;;
esac
...
# The intended-device UDID is verification-only input. The producer passes only its private-file
# path to a wrapper; the wrapper reads the raw value in memory and invokes the canonical inspector
# in-process. The raw identifier never enters Bash argv/environment, evidence, filenames, or logs.
# INSPECTION_DIR must remain absent until the failure-atomic inspector publishes.
