...
then
  echo "NEMBRA_INTENDED_FIELD_DEVICE_UDID_FILE did not pass private verification-input validation." >&2
  exit 4
fi
...
# The intended-device UDID is verification-only input. Only the private file path crosses an OS
# process/environment boundary; the wrapper reads the raw identifier in memory and invokes the
# canonical inspector's main(argv) directly. INSPECTION_DIR must remain absent until publication.
