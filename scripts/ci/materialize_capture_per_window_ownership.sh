...
[[ "$(git branch --show-current)" == "$BRANCH" ]] || {
  echo "Refusing materialization on the wrong branch" >&2
  exit 2
}
git fetch --no-tags --depth=1 origin "$EXPECTED_PRODUCT_PARENT"
unexpected="$(git diff --name-only "$EXPECTED_PRODUCT_PARENT" HEAD | grep -Ev '^(.github/workflows/capture-per-window-ownership-materialize.yml|scripts/ci/materialize_capture_per_window_ownership.sh)$' || true)"
[[ -z "$unexpected" ]] || {
  echo "Refusing materialization because the claim branch already contains non-helper drift:" >&2
  printf '%s\n' "$unexpected" >&2
  exit 3
}
