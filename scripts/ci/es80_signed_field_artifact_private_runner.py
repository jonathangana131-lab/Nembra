...
def run_inspector(args: argparse.Namespace) -> int:
    ...
    inspector_arguments = [
        ...
        "--intended-device-udid", intended_device_identifier,
    ]
    try:
    ...
    with tempfile.TemporaryDirectory(prefix="nembra-private-device-input-") as temporary:
        ...
        for name, raw, mode in (
            ...
            try:
                ...
                raise AssertionError(f"invalid {name} verification input must fail closed")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--intended-device-udid-file", type=Path)
