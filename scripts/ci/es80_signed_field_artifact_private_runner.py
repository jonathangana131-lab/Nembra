#!/usr/bin/env python3
"""Invoke the canonical signed-field inspector through private verification inputs and one frozen IPA subject.
...
def private_ipa_snapshot(path: Path) -> Iterator[Path]:
    """Freeze the caller's current IPA bytes into one private exact inspection subject.

    The caller-owned pathname is opened once with O_NOFOLLOW. All later canonical inspection and
    publication consume only the attempt-local snapshot. Source identity is sampled before/after the
    copy so ordinary concurrent in-place mutation fails closed; regardless, the snapshot's exact
    bytes—not a later reopen of the caller path—are the sole evidence subject.
    """
    try:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="nembra-private-signed-field-subject-") as temporary:
            root = Path(temporary)
            snapshot = root / "inspection-subject.ipa"
            ...
            try:
                while True:
                    ...
                    while view:
                        ...
                        if written <= 0:
                            raise PrivateInputError("signed field candidate IPA snapshot write made no progress")
                        copied += written
                        view = view[written:]
            ...
            if _source_identity(after) != _source_identity(before) or copied != before.st_size:
                raise PrivateInputError("signed field candidate IPA changed while its exact subject was frozen")
...
def run_inspector(args: argparse.Namespace) -> int:
    ...
    try:
        with private_ipa_snapshot(args.ipa) as ipa_snapshot:
            # Raw device identity exists only in this process's memory. The canonical inspector sees
            # one private frozen IPA pathname, not the caller-owned mutable source path.
            inspector_arguments = [
                "--ipa",
                str(ipa_snapshot),
                "--output-dir",
