...
PREMIUM_SOURCE = "3c1d4394b7b443936240131e6d0d91ca01e49383"
BRANCH = "recovery/v14-capture-final-premium-root-sol"
TARGET = Path("NembraApp/App/NembraCaptureEntrypoint.swift")
TEST_TARGET = Path("Packages/NembraBluetoothCapture/Tests/NembraBluetoothCaptureTests/CaptureP0RootPrecisionAccessibilitySourceTests.swift")
WORKFLOW = Path(".github/workflows/capture-final-premium-root-materialize.yml")
SCRIPT = Path("scripts/ci/materialize_capture_final_premium_root.py")
...
allowed_bootstrap = {str(WORKFLOW), str(SCRIPT)}
assert changed <= allowed_bootstrap, f"unexpected bootstrap drift: {sorted(changed - allowed_bootstrap)}"

run("git", "fetch", "--no-tags", "origin", PREMIUM_SOURCE)
source = run("git", "show", f"{PREMIUM_SOURCE}:{TARGET}", capture=True)

heading_needle = '''            .font(dynamicTypeSize.isAccessibilitySize ? .title2.bold() : .largeTitle.bold())
...
heading_replacement = '''            .font(dynamicTypeSize.isAccessibilitySize ? .title2.bold() : .largeTitle.bold())
            ...
            Text(
'''
assert source.count(heading_needle) == 1, "hero heading anchor drift"
source = source.replace(heading_needle, heading_replacement, 1)
...
combine_replacement = '''        }
    ...
    private var buildAuthorityStatus: some View {
'''
assert source.count(combine_needle) == 1, "hero accessibility container anchor drift"
source = source.replace(combine_needle, combine_replacement, 1)
...
for token in required:
    assert token in source, f"missing required product token: {token}"
assert 'private func rootPanel<Content: View>' not in source
TARGET.write_text(source)

TEST_TARGET.write_text(r'''import Foundation
import Testing
...
run("git", "add", "-A")
run("git", "commit", "-m", "fix(capture): converge premium root on frozen final truth")
