...
def retain_simctl_help(topic: str, output: Path) -> None:
    probe = subprocess.run(["xcrun", "simctl", "help", topic], check=True, text=True, capture_output=True)
...
def exact_runtime() -> str:
    ...
    choices = [item for item in payload["runtimes"] if item.get("isAvailable", True) and str(item.get("identifier", "")).startswith("com.apple.CoreSimulator.SimRuntime.iOS-27")]
    if not choices:
...
def main() -> int:
    ...
    try:
        ...
        terminate(udid)
        system_light = ARTIFACTS / "screenshots/standalone-unprovisioned-system-light-app-forced-dark-iphone12.png"
        pid = launch(udid)
        capture_ready(guard, udid, pid, system_light, "system-light-app-forced-dark-standard", ARTIFACTS / "logs/screenshot-readiness.jsonl")
        ...
        record = {
            ...
            "screenshots": [
                ...
                {"state": "unprovisioned-system-light-app-forced-dark-standard", "relativePath": str(system_light.relative_to(ARTIFACTS)), "sha256": sha256(system_light)},
