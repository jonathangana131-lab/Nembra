...
method = s[start:end]
if method.count('        let preview = admission.preview\n') != 1:
    raise SystemExit('unexpected preview read')
...
t = test.read_text()
if 'staging preview is narrow' not in t:
    ...
    pos = t.rfind('\n}\n')
    if pos < 0:
        raise SystemExit('test suite close not found')
...
assert 'var isConsumed: Bool' in r
...
assert 'if admission.isConsumed' in coord
assert 'completeTerminalFreshTargetSessionIfReady' in c
assert Path('Packages/NembraBluetoothCapture/Sources/NembraBluetoothCapture/PassiveCoreBluetoothAbortedFreshTargetSession.swift').exists()
