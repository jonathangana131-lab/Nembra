...
def write_envelope_no_replace(
    ...
    *,
    ...
    """Publish one complete envelope atomically without replacing an existing authority subject.

    The public output path remains absent while bytes are staged and fsynced into a private sibling.
    A same-filesystem hard link is then the single publication operation: it is atomic and fails if
    the destination already exists. The published inode and exact bytes are rechecked before return.
    """
    ...
    try:
        ...
        if os.read(staging_descriptor, len(envelope) + 1) != envelope:
            raise AuthorizationEnvelopeError(
        ...
        if staging_path is not None:
            try:
                ...
                # cleanup failure must not turn a successful no-replace publication into a false
                # failure that could encourage an operator to sign again.
                pass
