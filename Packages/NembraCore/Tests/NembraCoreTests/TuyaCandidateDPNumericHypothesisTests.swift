...
struct TuyaCandidateDPNumericHypothesisTests {
    ...
    private func report(
        ...
        let observations = try zip(markerTimes, rawValues).map { pair in
            try observation(time: pair.0, record: record(type: type, value: pair.1))
        }
    ...
    func explicitTenthsHypothesis() throws {
        ...
        let identity = try TuyaCandidateDPNumericTransformHypothesis(identifier: "identity", scale: 1)
        let tenths = try TuyaCandidateDPNumericTransformHypothesis(identifier: "explicit /10", scale: 0.1)
    ...
    func displayedTextDoesNotCreateNumbers() throws {
        let parent = try report(
            displayed: ["forty one point three volts"],
            rawValues: [[0x01, 0x9D]]
        )
        let result = try TuyaCandidateDPNumericHypothesisEvaluator.evaluate(
            report: parent,
    ...
    @Test("raw candidate bytes are never coerced to scalar magnitudes")
    ...
    func unusedReferenceIsVisible() throws {
        let parent = try TuyaCandidateDPMarkerCorrelator.analyze(
            scope: scope(),
            markers: [try marker(100, "41.3 V"), try marker(300, "41.2 V")],
            observations: [try observation(time: 100, record: record(value: [0x01, 0x9D]))],
        ...
        #expect(result.ambiguousReferenceMarkerIndices.isEmpty)
        #expect(result.sharedObservationReferenceMarkerIndices.isEmpty)
    ...
    @Test("parent shared-observation rejection remains distinct from generic missing evidence")
    func parentSharedObservationRemainsExplicit() throws {
        let parent = try TuyaCandidateDPMarkerCorrelator.analyze(
            scope: scope(),
            markers: [try marker(95, "41.3 V"), try marker(105, "41.2 V")],
            observations: [try observation(time: 100, record: record(value: [0x01, 0x9D]))],
            policy: correlationPolicy(distance: 5)
        )
        let result = try TuyaCandidateDPNumericHypothesisEvaluator.evaluate(
        ...
        #expect(result.sharedObservationReferenceMarkerIndices == [0, 1])
        #expect(result.ambiguousReferenceMarkerIndices.isEmpty)
        #expect(result.unusedReferenceMarkerIndices.isEmpty)
        #expect(result.evidence.first?.samples.isEmpty == true)
    }
    ...
    func preservesTemporalProvenance() throws {
        let parent = try TuyaCandidateDPMarkerCorrelator.analyze(
            ...
            observations: [try observation(time: 100, end: 110, record: record(value: [0x01, 0x9D]))],
            policy: correlationPolicy(distance: 5)
        )
        let result = try TuyaCandidateDPNumericHypothesisEvaluator.evaluate(
            report: parent,
