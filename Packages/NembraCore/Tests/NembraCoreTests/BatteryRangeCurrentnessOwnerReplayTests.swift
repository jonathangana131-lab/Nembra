...
struct BatteryRangeCurrentnessOwnerReplayTests {
    ...
    func freshOwnerCannotRevivePriorEstimate() throws {
        ...
        let acceptedOriginal = try originalStream.accept(originalObservation)
        let originalAnchor = try #require(acceptedOriginal)
        ...
        let acceptedReplacement = try originalStream.accept(replacementObservation)
        _ = try #require(acceptedReplacement)
        ...
        let acceptedReplay = try replayStream.accept(originalObservation)
        let replayAnchor = try #require(acceptedReplay)
