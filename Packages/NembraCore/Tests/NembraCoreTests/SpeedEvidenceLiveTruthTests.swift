...
struct SpeedEvidenceLiveTruthTests {
    ...
    func authoritativeSampleMakesSpeedLive() throws {
        var truth = SpeedEvidenceLiveTruth()
        let generation = SpeedEvidenceConnectionGeneration(rawValue: 1)
        let measured = try sample(kilometersPerHour: 12.4, uptimeNanoseconds: 100)
        expectSuccess(truth.beginConnectedGeneration(generation))
        let token = try #require(truth.activeContinuityToken)
    ...
    func disconnectDemotesLiveToRetained() throws {
        var truth = SpeedEvidenceLiveTruth()
        let generation = SpeedEvidenceConnectionGeneration(rawValue: 1)
        let measured = try sample(kilometersPerHour: 18.4, uptimeNanoseconds: 100)
        expectSuccess(truth.beginConnectedGeneration(generation))
        let token = try #require(truth.activeContinuityToken)
        expectSuccess(truth.accept(measured, attributedTo: token))
    ...
    func reconnectRequiresFreshSpeedEvidence() throws {
        ...
        let before = try sample(kilometersPerHour: 0, uptimeNanoseconds: 100)
        let after = try sample(kilometersPerHour: 0, uptimeNanoseconds: 200)
        expectSuccess(truth.beginConnectedGeneration(first))
        let firstToken = try #require(truth.activeContinuityToken)
        expectSuccess(truth.accept(before, attributedTo: firstToken))
        ...
        #expect(truth.availability == .retained(before))
        expectSuccess(truth.accept(after, attributedTo: secondToken))
        #expect(truth.availability == .live(after))
    }
    ...
    func delayedOldGenerationFailsClosed() throws {
        ...
        let first = SpeedEvidenceConnectionGeneration(rawValue: 1)
        let second = SpeedEvidenceConnectionGeneration(rawValue: 2)
        let firstSample = try sample(kilometersPerHour: 8, uptimeNanoseconds: 100)
        let delayed = try sample(kilometersPerHour: 0, uptimeNanoseconds: 200)
        expectSuccess(truth.beginConnectedGeneration(first))
        let firstToken = try #require(truth.activeContinuityToken)
        expectSuccess(truth.accept(firstSample, attributedTo: firstToken))
        expectSuccess(truth.endConnectedGeneration(first))
        expectSuccess(truth.beginConnectedGeneration(second))
        expectFailure(.continuityTokenMismatch, truth.accept(delayed, attributedTo: firstToken))
    ...
    func evidenceGapRequiresNewSourceToken() throws {
        ...
        let before = try sample(kilometersPerHour: 15, uptimeNanoseconds: 100)
        let after = try sample(kilometersPerHour: 14, uptimeNanoseconds: 200)
        expectSuccess(truth.beginConnectedGeneration(generation))
        let beforeToken = try #require(truth.activeContinuityToken)
        expectSuccess(truth.accept(before, attributedTo: beforeToken))
        expectSuccess(truth.markEvidenceGap(after: beforeToken))
        let afterToken = try #require(truth.activeContinuityToken)
        #expect(afterToken.segmentSequence == beforeToken.segmentSequence + 1)
        #expect(truth.availability == .retained(before))
        expectSuccess(truth.accept(after, attributedTo: afterToken))
        #expect(truth.availability == .live(after))
    }
    ...
    func queuedPreGapSampleIsRejected() throws {
        ...
        let acceptedBefore = try sample(kilometersPerHour: 10, uptimeNanoseconds: 100)
        let queuedBefore = try sample(kilometersPerHour: 0, uptimeNanoseconds: 150)
        let acceptedAfter = try sample(kilometersPerHour: 9, uptimeNanoseconds: 250)
        expectSuccess(truth.beginConnectedGeneration(generation))
        let preGapToken = try #require(truth.activeContinuityToken)
        expectSuccess(truth.accept(acceptedBefore, attributedTo: preGapToken))
        expectSuccess(truth.markEvidenceGap(after: preGapToken))
        let postGapToken = try #require(truth.activeContinuityToken)
        expectFailure(.continuityTokenMismatch, truth.accept(queuedBefore, attributedTo: preGapToken))
        #expect(truth.availability == .retained(acceptedBefore))
        expectSuccess(truth.accept(acceptedAfter, attributedTo: postGapToken))
        #expect(truth.availability == .live(acceptedAfter))
    }
    ...
    func staleGapCallbackCannotDemoteNewerSegment() throws {
        ...
        let first = try sample(kilometersPerHour: 4, uptimeNanoseconds: 100)
        let newer = try sample(kilometersPerHour: 6, uptimeNanoseconds: 250)
        expectSuccess(truth.beginConnectedGeneration(generation))
        let firstToken = try #require(truth.activeContinuityToken)
        ...
        expectFailure(.continuityTokenMismatch, truth.markEvidenceGap(after: firstToken))
    ...
    func shortHorizonEstimateIsRejected() throws {
        var truth = SpeedEvidenceLiveTruth()
        let generation = SpeedEvidenceConnectionGeneration(rawValue: 1)
        let estimate = try sample(source: .motionAssist, provenance: .shortHorizonEstimate, kilometersPerHour: 10, uptimeNanoseconds: 100)
        expectSuccess(truth.beginConnectedGeneration(generation))
        let token = try #require(truth.activeContinuityToken)
        expectFailure(.nonAuthoritativeSample, truth.accept(estimate, attributedTo: token))
        #expect(truth.availability == .unavailable)
    }
    ...
    func receiptChronologyIsGlobalWithinProcess() throws {
        ...
        let first = SpeedEvidenceConnectionGeneration(rawValue: 1)
        let second = SpeedEvidenceConnectionGeneration(rawValue: 2)
        let initial = try sample(kilometersPerHour: 5, uptimeNanoseconds: 500)
        let older = try sample(kilometersPerHour: 0, uptimeNanoseconds: 499)
        let equal = try sample(kilometersPerHour: 0, uptimeNanoseconds: 500)
        let newer = try sample(kilometersPerHour: 0, uptimeNanoseconds: 501)
        expectSuccess(truth.beginConnectedGeneration(first))
        let firstToken = try #require(truth.activeContinuityToken)
    ...
    func staleConnectionGenerationIsRejected() {
        ...
        expectFailure(.staleConnectionGeneration, truth.beginConnectedGeneration(first))
    ...
    func generationValidationAndIdempotency() throws {
        ...
        expectFailure(.invalidConnectionGeneration, truth.beginConnectedGeneration(invalid))
        expectSuccess(truth.beginConnectedGeneration(generation))
        let token = try #require(truth.activeContinuityToken)
        expectSuccess(truth.beginConnectedGeneration(generation))
        #expect(truth.activeContinuityToken == token)
    }
    ...
    func noActiveConnectionFailsClosed() throws {
        var truth = SpeedEvidenceLiveTruth()
        let generation = SpeedEvidenceConnectionGeneration(rawValue: 1)
        let measured = try sample(kilometersPerHour: 0, uptimeNanoseconds: 100)
        expectSuccess(truth.beginConnectedGeneration(generation))
        let endedToken = try #require(truth.activeContinuityToken)
        expectSuccess(truth.endConnectedGeneration(generation))
        expectFailure(.noActiveConnection, truth.accept(measured, attributedTo: endedToken))
        expectFailure(.noActiveConnection, truth.markEvidenceGap(after: endedToken))
