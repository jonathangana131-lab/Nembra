...
struct AdaptiveBatteryRangePrimaryPresentationTests {
    @Test("no receipt-bound estimate stays unavailable")
    func noEstimateStaysUnavailable() throws {
        ...
        let stream = AcceptedBatterySOCStream()
    ...
    @Test("newer same-uptime receipt demotes old range by identity, not by clock guess")
