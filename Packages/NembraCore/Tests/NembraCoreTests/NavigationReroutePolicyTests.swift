...
struct NavigationReroutePolicyTests {
    ...
    func oneNoisyPointCannotReroute() throws {
        ...
        let update = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 1)
    ...
    func durationThresholdIsRequired() throws {
        ...
        _ = try tracker.ingest(distanceFromRouteMeters: 24, receiptUptime: 2)
        let update = try tracker.ingest(distanceFromRouteMeters: 23, receiptUptime: 3)
    ...
    func sustainedDeviationRequestsOneReroute() throws {
        ...
        _ = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 1)
        _ = try tracker.ingest(distanceFromRouteMeters: 24, receiptUptime: 3)
        let confirmed = try tracker.ingest(distanceFromRouteMeters: 23, receiptUptime: 5)
        let repeated = try tracker.ingest(distanceFromRouteMeters: 30, receiptUptime: 6)
    ...
    func hysteresisBandDoesNotAdvanceEvidence() throws {
        ...
        _ = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 1)
        let band = try tracker.ingest(distanceFromRouteMeters: 15, receiptUptime: 3)
        let secondFar = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 5)
    ...
    func returnOnRouteClearsEpisode() throws {
        ...
        _ = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 1)
        let onRoute = try tracker.ingest(distanceFromRouteMeters: 10, receiptUptime: 2)
        let next = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 3)
    ...
    func cooldownSuppressesSecondEpisode() throws {
        ...
        let suppressed = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 14)
        let eligibleLater = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 35)
    ...
    func cooldownBoundaryIsInclusive() throws {
        ...
        _ = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 1)
        _ = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 3)
        _ = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 5)
        _ = try tracker.ingest(distanceFromRouteMeters: 5, receiptUptime: 6)
        _ = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 29)
        _ = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 32)
        let confirmed = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 35)

        #expect(confirmed.recommendation == .requestNewRoute)
    }

    @Test("known continuity interruption discards in-flight deviation evidence")
    func continuityGapResetsConfidence() throws {
        var tracker = NavigationRerouteTracker(policy: try policy())

        _ = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 1)
        _ = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 3)
        let gap = try tracker.markContinuityInterrupted(receiptUptime: 4)
        let postGap = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 5)
    ...
    func replacementRouteResetsAdherence() throws {
        ...
        _ = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 10)
        _ = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 12)
        let update = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 14)
    ...
    func uptimeMustStrictlyIncrease() throws {
        ...
        #expect(throws: NavigationReroutePolicyError.nonMonotonicReceiptUptime) {
            _ = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: 9)
        }
        #expect(tracker == before)
    }
    ...
    func invalidDeviationFailsAtomically() throws {
        var tracker = NavigationRerouteTracker(policy: try policy())
        _ = try tracker.ingest(distanceFromRouteMeters: 5, receiptUptime: 1)
        let before = tracker

        #expect(throws: NavigationReroutePolicyError.invalidDistanceFromRoute) {
            _ = try tracker.ingest(distanceFromRouteMeters: .nan, receiptUptime: 2)
        }
        #expect(tracker == before)

        #expect(throws: NavigationReroutePolicyError.invalidDistanceFromRoute) {
            _ = try tracker.ingest(distanceFromRouteMeters: -1, receiptUptime: 2)
        }
        #expect(tracker == before)
    }

    @Test("invalid receipt uptime fails atomically")
    func invalidUptimeFailsAtomically() throws {
        var tracker = NavigationRerouteTracker(policy: try policy())
        let before = tracker

        #expect(throws: NavigationReroutePolicyError.invalidReceiptUptime) {
            _ = try tracker.ingest(distanceFromRouteMeters: 25, receiptUptime: .infinity)
        }
        #expect(tracker == before)
    }
