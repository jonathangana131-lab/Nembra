...
final class BatteryEvidenceCurrentnessOwner: @unchecked Sendable {
    ...
    // NembraCore supports macOS 12 for package validation. `Synchronization.Mutex` is available
    // only on newer OS versions, so keep the same single critical-section state machine on the
    // long-supported Foundation lock. Every mutable `State` access goes through `withState`.
