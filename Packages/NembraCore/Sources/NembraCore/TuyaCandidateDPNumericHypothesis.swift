...
/// exists in a #262 temporal correlation report.
///
...
/// One auditable comparison tied directly to #262's accepted temporal hit.
...
/// supplied marker references and #262-accepted hits; they are not confidence.
...
/// Numeric hypothesis evaluation for one exact candidate from one #262 report.
/// Scope/continuity/framing truth remains owned by that parent report rather than
...
public enum TuyaCandidateDPNumericHypothesisEvaluator {
    /// Evaluates explicit numeric transforms only after #262 has selected one
    /// exact stream/generation/framing scope and produced at most one unambiguous
    /// candidate hit per marker.
    public static func evaluate(
        ...
        let unusedReferenceMarkerIndices = referencesByMarker.keys
            .filter { !hitMarkerIndices.contains($0) && !ambiguousMarkerIndices.contains($0) }
            .sorted()
    ...
    /// bytes retained by #262. No signedness, scale, unit or field meaning is
    /// assigned. Malformed documented shapes remain unavailable.
