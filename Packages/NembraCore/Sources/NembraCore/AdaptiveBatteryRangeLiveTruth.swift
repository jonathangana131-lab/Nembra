...
/// Derived range tied to the exact accepted SoC receipt used to compute it.
///
/// This wrapper is deliberately non-Codable. The nested numeric estimate remains a derived
/// calculation, while `sourceReceiptIdentity` is the process-local evidence binding a primary
/// UI may use to distinguish genuinely current range from a retained/stale value.
...
public extension AdaptiveBatteryRangeModel {
    /// Computes range from a receipt-bound, currently accepted SoC anchor.
    ///
    /// The existing generic estimator remains useful for simulation/offline model tests. New
    /// production presentation code should consume this API (or a stricter successor) so a
    /// range number always carries the exact accepted battery receipt that produced it.
    func estimateRemainingRange(
