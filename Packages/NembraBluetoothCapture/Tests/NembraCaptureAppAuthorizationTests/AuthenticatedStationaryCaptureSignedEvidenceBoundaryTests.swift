...
@Suite("Authenticated stationary Capture evidence-to-manifest boundary")
struct AuthenticatedStationaryCaptureSignedEvidenceBoundaryTests {
    ...
    func productionBeginAttemptDoesNotAcceptArbitraryBindingTuples() throws {
        ...
        #expect(!source.contains("public func beginAttempt(\n        externalBindings:"))
        #expect(source.contains("installManifestData: Data"))
        #expect(source.contains("manifest.authorizationEnvelopeSHA256"))
        #expect(!source.contains("publicKeyX963Representation:"))
        #expect(!source.contains("isAuthoritativeFieldBuild = true"))
    }

    @Test("signed-build evidence cannot mint physical capability")
