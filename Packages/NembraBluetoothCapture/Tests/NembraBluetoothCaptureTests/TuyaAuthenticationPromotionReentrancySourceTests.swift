...
struct TuyaAuthenticationPromotionReentrancySourceTests {
    ...
    @Test("retired token returns silently while current source drift uses the source-authority terminal")
    func postAwaitFenceDistinguishesRetiredTokenFromSourceDrift() throws {
        let source = try readRepositoryFile("NembraApp/App/NembraCaptureEntrypoint.swift")
        let authenticated = String(try section(
            in: source,
            from: "private func authenticated(token: TuyaReadOnlyConnectionToken) async",
        ...
        #expect(fence.contains("guard currentConnectionToken == token else"))
        #expect(fence.contains("guard phase == .authenticating else"))
        #expect(fence.contains("guard sdkAccountLoggedIn"))
        #expect(fence.contains("sdkDeviceMembershipVerified"))
        #expect(fence.contains("accountIdentityLeaseIsAuthorized"))
        #expect(fence.contains("invalidateSourceAuthority("))
        #expect(fence.contains("sdk_source_authority_lost_after_auth_promotion"))
    }
