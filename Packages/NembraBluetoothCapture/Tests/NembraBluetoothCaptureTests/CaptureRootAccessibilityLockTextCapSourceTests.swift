...
struct CaptureRootAccessibilityLockTextCapSourceTests {
    ...
    func lockTextHasItsOwnAccessibilityCap() throws {
        ...
        // The icon already has its own cap. It must not satisfy this text-specific contract:
        // Accessibility XXXL must retain product hierarchy instead of letting the redundant lock
        // banner become a two-line hero that pushes Account Setup down the first viewport.
