...
struct TuyaAuthenticatedReadOnlySessionLedgerTests {
    ...
    func reconnectClearsPriorEvidence() async throws {
        ...
        try await ledger.recordApplicationObservation(fieldCount: 2, for: first)
    ...
    @Test("stale callback cannot contaminate a newer generation")
    ...
    @Test("application observation is post-auth and non-empty")
    func applicationAdmissionFailsClosed() async throws {
    ...
    @Test("structured SDK observation plus exact stability window earns canonical verdict")
    func acceptedGateUsesSealedChronology() async throws {
        ...
        try await ledger.markAuthenticated(for: token, method: .smartLifeAppSDK)
        clock.advance(to: 3_000)
        try await ledger.recordApplicationObservation(fieldCount: 3, for: token)
        clock.advance(to: 2_000 + TuyaAuthenticatedReadOnlyPreflight.minimumAuthenticatedConnectionNanoseconds)
        try await ledger.observeCurrentAuthenticatedConnection(for: token)
    ...
    @Test("application observation never claims raw byte count")
    func fieldCountIsAdmissionOnly() async throws {
        let clock = TestUptimeClock(1_000)
        let ledger = TuyaAuthenticatedReadOnlySessionLedger(nowUptimeNanoseconds: clock.now)
        let token = try await ledger.beginConnection()
        clock.advance(to: 2_000)
    ...
    @Test("terminal authentication failure retires provenance and application authority")
    func authenticationFailureRetiresAuthority() async throws {
        ...
        try await ledger.markAuthenticationFailed(for: token, reason: "SDK session invalidated.")

        let snapshot = await ledger.currentPreflightSnapshot()
        #expect(snapshot.authenticationState == .failed(reason: "SDK session invalidated."))
        #expect(snapshot.authenticationMethod == nil)
        #expect(snapshot.applicationPayloadCount == 0)
        #expect(snapshot.authenticatedAtUptimeNanoseconds == nil)
        #expect(snapshot.latestApplicationPayloadUptimeNanoseconds == nil)
        #expect(TuyaAuthenticatedReadOnlyPreflight.verdict(for: snapshot) == .blocked(reason: "SDK session invalidated."))
    }

    @Test("disconnect retires current token authority")
