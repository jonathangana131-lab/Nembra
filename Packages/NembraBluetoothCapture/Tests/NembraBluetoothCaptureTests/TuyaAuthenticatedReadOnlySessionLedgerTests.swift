...
struct TuyaAuthenticatedReadOnlySessionLedgerTests {
    ...
    @Test("connection-authority end retires auth provenance without inventing transport cause")
    func connectionEndRetiresAuthority() async throws {
        let clock = TestUptimeClock(100)
        let ledger = TuyaAuthenticatedReadOnlySessionLedger(nowUptimeNanoseconds: clock.now)
        let token = try await ledger.beginConnection()
    ...
    @Test("observation invalidation retires authority without inventing a disconnect")
    func observationInvalidationIsTruthfulAndTerminal() async throws {
        let clock = TestUptimeClock(1_000)
        let ledger = TuyaAuthenticatedReadOnlySessionLedger(nowUptimeNanoseconds: clock.now)
        let token = try await ledger.beginConnection()
        clock.advance(to: 2_000)
        ...
        #expect(snapshot.applicationPayloadCount == 0)
        #expect(snapshot.authenticatedAtUptimeNanoseconds == nil)
        #expect(snapshot.connectionStartedAtUptimeNanoseconds == 1_000)
    ...
    @Test("authentication failure terminally retires the connection token")
    func authenticationFailureRejectsLateSuccess() async throws {
        let clock = TestUptimeClock(1_000)
        let ledger = TuyaAuthenticatedReadOnlySessionLedger(nowUptimeNanoseconds: clock.now)
        let token = try await ledger.beginConnection()
