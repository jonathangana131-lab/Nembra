...
struct TuyaAuthenticatedReadOnlySessionLedgerTests {
    ...
    @Test("post-auth observation timeout is terminal without inventing disconnect or liveness")
    func observationTimeoutRejectsLateUpdate() async throws {
        ...
        #expect(failed.latestObservedUptimeNanoseconds == 2_000)
    ...
    @Test("late SDK failure revokes authority without erasing earned chronology or inventing liveness")
    ...
    @Test("pre-auth SDK failure keeps never-earned evidence empty and retires the token")
    func preAuthSDKFailureIsTerminalWithoutFabrication() async throws {
        let clock = TestUptimeClock(100)
        let ledger = TuyaAuthenticatedReadOnlySessionLedger(nowUptimeNanoseconds: clock.now)
        let token = try await ledger.beginConnection()
        clock.advance(to: 200)
        ...
        #expect(failed.latestObservedUptimeNanoseconds == 200)
