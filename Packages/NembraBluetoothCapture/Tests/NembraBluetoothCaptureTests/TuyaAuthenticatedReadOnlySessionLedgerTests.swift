...
struct TuyaAuthenticatedReadOnlySessionLedgerTests {
    @Test("new connection resets prior authenticated payload authority")
    func reconnectClearsPriorEvidence() async throws {
        ...
        try await ledger.markAuthenticated(for: first)
    ...
    func staleGenerationRejected() async throws {
        ...
        let second = try await ledger.beginConnection()
        clock.advance(to: 30)
        try await ledger.markAuthenticated(for: second)
    ...
    @Test("payload requires current authenticated connection and non-empty bytes")
    func payloadAdmissionIsFailClosed() async throws {
        ...
        try await ledger.markAuthenticated(for: token)
    ...
    @Test("post-auth payload plus exact stability window earns stationary mapping")
    func acceptedGateUsesLedgerChronology() async throws {
        let clock = TestUptimeClock(1_000)
        let ledger = TuyaAuthenticatedReadOnlySessionLedger(nowUptimeNanoseconds: clock.now)
        let token = try await ledger.beginConnection()
    ...
    @Test("clock regression rejects mutation without rewriting accepted chronology")
    ...
    @Test("ending connection retires authentication and payload evidence")
