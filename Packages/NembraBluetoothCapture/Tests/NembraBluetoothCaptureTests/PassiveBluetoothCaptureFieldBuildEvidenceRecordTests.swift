...
struct PassiveBluetoothCaptureFieldBuildEvidenceRecordTests {
    ...
    func exactEvidenceDigestTracksExactBytes() throws {
        ...
        let compactRecord = try PassiveBluetoothCaptureFieldBuildEvidenceRecordJSON.decodeDeclaration(compact)
        let prettyRecord = try PassiveBluetoothCaptureFieldBuildEvidenceRecordJSON.decodeDeclaration(pretty)
    ...
    func unsupportedSchemaAndInstallableKindFailClosed() throws {
        ...
        #expect(throws: PassiveBluetoothCaptureFieldBuildEvidenceRecordError.unsupportedSchemaVersion(2)) {
        ...
        #expect(
            throws: PassiveBluetoothCaptureFieldBuildEvidenceRecordError.unsupportedSignedInstallableKind("app")
        ) {
            _ = try PassiveBluetoothCaptureFieldBuildEvidenceRecordJSON.decodeDeclaration(
                try makeFieldRecordJSON(
                    externalRecordData: externalData,
    ...
    func mismatchedExternalRecordEncodingCannotProject() throws {
        ...
        let fieldRecord = try PassiveBluetoothCaptureFieldBuildEvidenceRecordJSON.decodeDeclaration(fieldData)

        let semanticallyEquivalentExternalData = try JSONSerialization.data(
            withJSONObject: baseExternalRecordObject(),
            options: [.prettyPrinted, .sortedKeys]
        )
        let semanticallyEquivalentExternalRecord = try PassiveBluetoothCaptureExternalBuildRecordJSON.decodeDeclaration(
            semanticallyEquivalentExternalData
        )
        ...
        #expect(throws: PassiveBluetoothCaptureFieldBuildEvidenceRecordError.externalBuildRecordMismatch) {
            _ = try fieldRecord.makeSoftwareExportBuildReference(matching: semanticallyEquivalentExternalRecord)
        }
    }

    private func makeExternalRecordJSON() throws -> Data {
        try JSONSerialization.data(withJSONObject: baseExternalRecordObject(), options: [.sortedKeys])
    }
