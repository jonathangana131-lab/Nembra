...
struct PassiveBluetoothExperimentOneCoordinatorContractTests {
    ...
    func connectionDoesNotBurnAdmissionBeforeTargetReappears() throws {
        ...
        let catalogGuard = try #require(connection.range(of: "guard controller.hasDiscoveredPeripheral(identifier: identifier),"))
        let exactLookup = try #require(connection.range(of: "let discovery = controller.discoveredPeripheral(identifier: identifier)"))
