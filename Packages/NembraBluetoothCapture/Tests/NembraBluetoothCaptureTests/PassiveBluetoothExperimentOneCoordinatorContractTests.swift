...
struct PassiveBluetoothExperimentOneCoordinatorContractTests {
    ...
    func connectionDoesNotBurnAdmissionBeforeTargetReappears() throws {
        ...
        let presenceGuard = try #require(connection.range(of: "controller.hasDiscoveredPeripheral(identifier: identifier)"))
        let discoveryLookup = try #require(connection.range(of: "controller.discoveredPeripheral(identifier: identifier)"))
        let connect = try #require(connection.range(of: "controller.connectUsingExperimentOneAdmission(admission, timeout: 12)"))
        #expect(presenceGuard.lowerBound < discoveryLookup.lowerBound)
        #expect(discoveryLookup.lowerBound < connect.lowerBound)
