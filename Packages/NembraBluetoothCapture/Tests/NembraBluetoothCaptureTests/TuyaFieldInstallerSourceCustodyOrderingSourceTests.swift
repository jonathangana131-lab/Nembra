...
struct TuyaFieldInstallerSourceCustodyOrderingSourceTests {
    ...
    func exactSourcePrecedesPrivateAdmission() throws {
        ...
        guard let input = installer.range(of: "EXPECTED_SOURCE_SHA=\"${1:-${NEMBRA_CAPTURE_EXPECTED_SOURCE_SHA:-}}\""),
              ...
              let authority = installer.range(of: "AUTHORITY_GIT_DIR=\"$ROOT/.git\"", range: normalize.upperBound..<installer.endIndex),
              ...
              let cleanTree = installer.range(of: "verify_accepted_checkout_source \"Current checkout is not the exact accepted Capture source.\"", range: equality.upperBound..<installer.endIndex),
              let requestedMessage = installer.range(of: "say \"Exact requested Capture source matched under isolated Git + raw-byte authority: $SOURCE_SHA\"", range: cleanTree.upperBound..<installer.endIndex),
        ...
        #expect(normalize.lowerBound < authority.lowerBound)
    ...
    func sourceCustodyEnclosesBuildAndReadback() throws {
        ...
        guard let initialEquality = installer.range(of: "[[ \"$SOURCE_SHA\" == \"$EXPECTED_SOURCE_SHA\" ]]"),
              ...
              let postBootstrapTree = installer.range(of: "verify_accepted_checkout_source \"Private workspace bootstrap changed accepted-source inputs.\"", range: bootstrap.upperBound..<installer.endIndex),
              ...
              let postBuildTree = installer.range(of: "verify_accepted_checkout_source \"Accepted-source inputs changed while the field build was compiling. Discard this candidate and restart.\"", range: buildStage.upperBound..<installer.endIndex),
        ...
        #expect(bootstrap.lowerBound < postBootstrapTree.lowerBound)
        #expect(postBootstrapTree.lowerBound < baseline.lowerBound)
        #expect(baseline.lowerBound < buildStage.lowerBound)
        #expect(buildStage.lowerBound < postBuildTree.lowerBound)
