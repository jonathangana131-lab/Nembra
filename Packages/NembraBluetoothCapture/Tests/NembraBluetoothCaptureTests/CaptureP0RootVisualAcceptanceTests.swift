...
struct CaptureP0RootVisualAcceptanceTests {
    ...
    func publicRootShowsFieldAuthorityBeforeAccountSetup() throws {
        ...
        #expect(root.contains("rootHero\n                        buildAuthorityStatus\n                        accountSetupPanel"))
    }

    @Test("premium root retires card soup and strengthens iPhone 12 contrast")
    func premiumRootUsesPrecisionSections() throws {
        let source = try readRepositoryFile("NembraApp/App/NembraCaptureEntrypoint.swift")
        let root = String(try section(
            in: source,
            from: "private struct CaptureP0Root: View",
        ...
        #expect(root.contains("Color.white.opacity(0.78)"))
        #expect(root.contains("Color.white.opacity(0.76)"))
        #expect(root.contains("Color.white.opacity(0.74)"))
        #expect(root.contains(".scrollDismissesKeyboard(.interactively)"))
        #expect(root.contains(".tint(.cyan)"))
    }

    @Test("Accessibility XXXL promotes the primary action before verbose status")
