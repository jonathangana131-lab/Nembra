...
struct TuyaAcceptedExportArtifactImmutabilitySourceTests {
    @Test("acceptance cut freezes the complete export before another post-seal suspension")
    func cutFreezesWholeAcceptedEnvelopeBeforeAcceptedPhase() throws {
        ...
        guard let cutPrefix = body.range(of: "let acceptedEventPrefixAtCut = self.events"),
            ...
            Issue.record("Accepted path must freeze the cut prefix, seal the package, re-check source authority, then freeze the complete export before accepted UI.")
