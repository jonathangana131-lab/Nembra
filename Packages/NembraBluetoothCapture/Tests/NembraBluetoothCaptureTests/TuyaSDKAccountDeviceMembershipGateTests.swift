...
struct TuyaSDKAccountDeviceMembershipGateTests {
    ...
    @Test("partial home loading cannot authorize even when one loaded home contains the exact scooter")
    func exactMembershipCannotOverrideIncompleteEnumeration() {
        let snapshot = TuyaSDKAccountDeviceMembershipGate.Snapshot(
            isLoggedIn: true,
            homeEnumerationCompleted: true,
            loadedHomeCount: 1,
