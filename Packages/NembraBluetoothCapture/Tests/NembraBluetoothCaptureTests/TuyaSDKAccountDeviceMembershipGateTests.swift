...
struct TuyaSDKAccountDeviceMembershipGateTests {
    ...
    @Test("negative home-load failures cannot be normalized into authorization")
    func negativeHomeLoadFailureCountBlocks() {
        let snapshot = TuyaSDKAccountDeviceMembershipGate.Snapshot(
            isLoggedIn: true,
            homeEnumerationCompleted: true,
            loadedHomeCount: 1,
    ...
    @Test("negative loaded-home count remains invalid evidence")
    func negativeLoadedHomeCountBlocks() {
        let snapshot = TuyaSDKAccountDeviceMembershipGate.Snapshot(
            isLoggedIn: true,
            homeEnumerationCompleted: true,
            loadedHomeCount: -1,
