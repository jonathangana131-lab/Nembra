...
struct TuyaInternalLifecycleFailureTerminalTests {
    @Test("clock-regressed auth promotion can retire the exact generation without another clock sample")
    func regressedAuthenticationPromotionRetiresWithoutClockRecovery() async throws {
        ...
        // Cleanup must not depend on the broken clock recovering. It retires callback authority
        // while preserving the last legitimate liveness horizon and without inventing auth proof.
    ...
    @Test("post-auth internal failure preserves earned evidence only as non-authorizing diagnostics")
    func postAuthenticationFailurePreservesEarnedChronology() async throws {
        ...
        // A terminal internal failure is evidence invalidation, not a new liveness observation.
        clock.set(1)
        try await ledger.markInternalLifecycleFailure(for: token)
        let failed = await ledger.currentPreflightSnapshot()
