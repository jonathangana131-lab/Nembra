...
struct ES80CaptureVoiceOverAlertAcceptanceTests {
    ...
    func announcementsStayOffTheDisplayClock() throws {
        ...
        #expect(timeline.contains("presentAnalysisReadySemanticsIfNeeded()"))
    ...
    func statusAnnouncementsStaySparseAndActionable() throws {
        let source = try Self.shellSource()
        let status = try Self.span(
            in: source,
            from: "private func announceAccessibilityStatus",
            to: "private func announceAccessibilityAlert"
        )
    ...
    @Test("analysis readiness is announced once without warning-prefix semantics")
    func analysisReadyTransitionIsNotLostOrDuplicated() throws {
        let source = try Self.shellSource()
        let success = try Self.span(
            in: source,
            from: "private func presentAnalysisReadySemanticsIfNeeded",
            to: "private func isBlockingHapticPhase"
        )
        #expect(success.contains("presentationAnalysisReady"))
        #expect(success.contains("phase(status: coordinator.status) == .complete"))
        #expect(success.contains("!didPresentAnalysisReadySemantics"))
        #expect(success.contains("didPresentAnalysisReadySemantics = true"))
        #expect(success.contains("Capture complete. Ready for analysis and sharing."))
        #expect(success.contains("UIAccessibility.post"))
        #expect(!success.contains("Capture alert"))
        ...
        #expect(status.contains("announcement = presentationAnalysisReady ? nil"))
        #expect(!status.contains("Capture complete. Ready for analysis and sharing."))
    }

    @Test("async warning banners are coherent accessibility elements and use alert semantics only for warnings")
    func warningBannerSemanticsStayCoherent() throws {
        let source = try Self.shellSource()
        let banner = try Self.span(
            in: source,
            from: "private func diagnosticBanner",
            to: "private func announceAccessibilityStatus"
        )
        let alert = try Self.span(
            in: source,
            from: "private func announceAccessibilityAlert",
            to: "private func handleSemanticPhaseTransition"
        )
        ...
        #expect(!alert.contains("Capture complete. Ready for analysis and sharing."))
    }
