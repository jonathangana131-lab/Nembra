...
/// Controller integration contract for foreground integrity while an admitted
/// observation Horizon is being finalized. The current product already consumes
/// exact pre-recorder H abandonment; this suite pins the remaining suspension-window
/// promotion gaps without racing the high-contention controller file.
struct ForegroundCoreBluetoothCaptureControllerForegroundFinalizationIntegrityTests {
    ...
    @Test("pre-recorder finalization failure keeps exact H abandonment integration")
    ...
    @Test("foreground loss during recorder or JSON awaits is checked before promotion")
