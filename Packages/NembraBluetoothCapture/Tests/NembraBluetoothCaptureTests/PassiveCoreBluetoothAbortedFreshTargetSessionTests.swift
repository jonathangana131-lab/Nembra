...
struct PassiveCoreBluetoothAbortedFreshTargetSessionTests {
    ...
    func createsExactNextRecorderProof() async throws {
        ...
        let fresh = try PassiveCoreBluetoothAbortedFreshTargetSession.create(
            after: resolution,
            id: sessionID,
    ...
    func detachedReceiptRetainsExactRecorder() async throws {
        ...
        #expect(ObjectIdentifier(receipt.recorder) == ObjectIdentifier(fresh.recorder))
    ...
    func independentProvisioningCannotAliasRecorderProof() async throws {
        let resolution = try await resolvedAbort(previousGeneration: 7)
        let first = try PassiveCoreBluetoothAbortedFreshTargetSession.create(
            after: resolution,
            vehicleIdentity: es80
        )
        let second = try PassiveCoreBluetoothAbortedFreshTargetSession.create(
            after: resolution,
            vehicleIdentity: es80
        )
    ...
    func generationExhaustionFailsClosed() async throws {
        ...
        #expect(throws: PassiveCoreBluetoothAbortedFreshTargetSession.StateError.targetSessionGenerationExhausted) {
            _ = try PassiveCoreBluetoothAbortedFreshTargetSession.create(
                after: resolution,
                vehicleIdentity: es80
            )
        }
