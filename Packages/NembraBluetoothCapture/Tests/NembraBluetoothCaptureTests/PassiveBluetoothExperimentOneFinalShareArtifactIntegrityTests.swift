...
struct PassiveBluetoothExperimentOneFinalShareArtifactIntegrityTests {
    ...
    @Test
    func reportNamesExactOuterShareAndExactNestedEvidence() throws {
        ...
        let verified = try PassiveBluetoothExperimentOneFinalShareArtifactCodec.decodeAndVerify(artifact.json)
        ...
        #expect(report.softwareExport.capture.sha256.count == 64)
        #expect(report.softwareExport.capture.rawValueRecordCount == 1)
    }

    @Test
    func semanticallyEquivalentTrailingWhitespaceStillGetsDifferentOuterIdentity() throws {
        let artifact = try makeArtifact()
        var changedExactBytes = artifact.json
        changedExactBytes.append(contentsOf: [0x0A, 0x20])

        let original = try PassiveBluetoothExperimentOneFinalShareArtifactIntegrity.inspect(artifact.json)
        let changed = try PassiveBluetoothExperimentOneFinalShareArtifactIntegrity.inspect(changedExactBytes)

        #expect(changed.experimentID == original.experimentID)
        #expect(changed.softwareExportSHA256 == original.softwareExportSHA256)
        #expect(changed.finalShareByteCount == original.finalShareByteCount + 2)
        #expect(changed.finalShareSHA256 != original.finalShareSHA256)
    }

    @Test
    func nestedTamperCannotEarnAnalysisIntegrityEvenWithValidOuterJSON() throws {
        let artifact = try makeArtifact()
        ...
        #expect(throws: PassiveBluetoothExperimentOneFinalShareArtifactError.softwareExportDigestMismatch) {
