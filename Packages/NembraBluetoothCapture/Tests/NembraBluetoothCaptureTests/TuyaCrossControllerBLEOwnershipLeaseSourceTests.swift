...
struct TuyaCrossControllerBLEOwnershipLeaseSourceTests {
    ...
    func leaseSerializesPackageCorrelationAgainstTuyaHandoff() throws {
        ...
        let retiredProcessFence = make.range(of: "guard !packageCorrelationRetiredForProcess,")
        ...
        #expect(retiredProcessFence != nil)
        #expect(noPackageOwner != nil)
        #expect(retirePackageForever != nil)
        #expect(driver != nil)
        if let retiredProcessFence, let noPackageOwner, let retirePackageForever, let driver {
            #expect(retiredProcessFence.lowerBound < noPackageOwner.lowerBound)
