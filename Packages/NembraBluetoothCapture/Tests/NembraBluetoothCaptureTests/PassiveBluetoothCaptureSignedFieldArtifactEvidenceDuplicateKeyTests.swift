...
struct PassiveBluetoothCaptureSignedFieldArtifactEvidenceDuplicateKeyTests {
    @Test("exact duplicate root field fails closed before JSON decoder precedence")
    func exactDuplicateRootFieldFailsClosed() throws {
        let data = try evidenceJSON(
            extraMember: #", "sourceCommitSHA":"2222222222222222222222222222222222222222""#
        )
    ...
    @Test("escaped spelling of duplicate semantic key fails closed")
    ...
    private func evidenceJSON(extraMember: String) throws -> Data {
        ...
        let json = """
        {
          ...
          "procedureVersion":"V14"\(extraMember)
        }
        """
