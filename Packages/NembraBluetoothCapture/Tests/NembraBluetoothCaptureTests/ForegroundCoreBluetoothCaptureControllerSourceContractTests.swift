...
struct ForegroundCoreBluetoothCaptureControllerSourceContractTests {
    ...
    @Test
    func cancellationCallGraphKeepsPreHorizonEvidenceBoundaryBeforeRetirement() throws {
        let source = try Self.controllerSource()
        let body = try Self.section(
            in: source,
        ...
        // Horizon-owned teardown deliberately retires transport without appending evidence.
        // The ordering contract below is therefore scoped to the normal pre-Horizon evidence path.
    ...
    func finalizedTeardownRequiresFrozenCurrentAuthorityAndAuthorityChangesRevokeIt() throws {
        ...
        let synchronousPublication = authorityAdvance[
            authorityAdvance.index(authorityAdvance.startIndex, offsetBy: transitionOffset)..<authorityAdvance.index(authorityAdvance.startIndex, offsetBy: revokeOffset)
        ]
    ...
    func finalizedTeardownQuarantineRejectsMarkersBeforeTerminalCallback() throws {
        ...
        let normal = cancellation[normalStart...]
        let pendingOffset = try Self.offset(
            of: "selectedTargetCancellationPending = true",
            in: normal
        )
