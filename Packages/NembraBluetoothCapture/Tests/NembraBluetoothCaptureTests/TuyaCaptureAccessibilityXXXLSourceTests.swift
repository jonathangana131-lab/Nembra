...
struct TuyaCaptureAccessibilityXXXLSourceTests {
    ...
    func correlationHeaderRecomposesAtAccessibilitySizes() throws {
        ...
        #expect(panel.contains("HStack(alignment: .firstTextBaseline"))
    }
    ...
    func observationProgressRecomposesAtAccessibilitySizes() throws {
        let app = try readRepositoryFile("NembraApp/App/NembraCaptureEntrypoint.swift")
        let panel = String(try section(in: app, from: "private var secureObservationPanel: some View", to: "private var failurePanel: some View"))
