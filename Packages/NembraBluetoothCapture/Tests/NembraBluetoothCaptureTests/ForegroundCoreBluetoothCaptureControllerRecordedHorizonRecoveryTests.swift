...
struct ForegroundCoreBluetoothCaptureControllerRecordedHorizonRecoveryTests {
    ...
    func recordedHorizonCommitFailureUsesProducerQuarantine() throws {
        ...
        #expect(section.distance(from: section.startIndex, to: record) < section.distance(from: section.startIndex, to: recorded))
        #expect(section.distance(from: section.startIndex, to: recorded) < section.distance(from: section.startIndex, to: commit))
