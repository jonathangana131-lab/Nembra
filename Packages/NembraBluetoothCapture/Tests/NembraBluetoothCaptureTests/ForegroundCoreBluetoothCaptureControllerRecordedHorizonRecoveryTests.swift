...
/// Controller integration contract for the producer-issued recorded-H quarantine.
...
struct ForegroundCoreBluetoothCaptureControllerRecordedHorizonRecoveryTests {
    ...
    @Test("durable Horizon commit failure quarantines exact recorded H before artifact freeze")
    func recordedHorizonCommitFailureUsesProducerQuarantine() throws {
        ...
        let recordedCase = try #require(section.range(of: "case let .recorded(boundary)")?.lowerBound)
        ...
        #expect(section.distance(from: section.startIndex, to: record) < section.distance(from: section.startIndex, to: recordedCase))
