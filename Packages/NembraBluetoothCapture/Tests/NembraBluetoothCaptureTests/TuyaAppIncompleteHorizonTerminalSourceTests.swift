...
struct TuyaAppIncompleteHorizonTerminalSourceTests {
    ...
    func appDoesNotAttemptSecondIncompleteHorizonTerminal() throws {
        ...
        for terminal in [applicationTerminal, watchdogTerminal] {
            ...
            #expect(terminal.contains("Scooter data did not become sufficient before the bounded observation deadline."))
            #expect(!terminal.contains("within 60 seconds"))
            #expect(!terminal.contains("60-second"))
