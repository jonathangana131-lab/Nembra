...
struct TuyaMetadataSecretRedactionSourceTests {
    ...
    @Test("final metadata export re-sanitizes every retained opaque payload at custody")
    func finalExportReSanitizesOpaquePayloads() throws {
        ...
        // The current truth contract deliberately removed the separately fetched
        // `/status` response that had been mislabeled as a local strategy. Do not
        // make the redaction regression resurrect that false provenance or field.
