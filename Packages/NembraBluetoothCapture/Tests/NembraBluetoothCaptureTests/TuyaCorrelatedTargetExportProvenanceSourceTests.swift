...
struct TuyaCorrelatedTargetExportProvenanceSourceTests {
    ...
    func exportAndResetPreserveSessionBoundary() throws {
        let app = try readRepositoryFile("NembraApp/App/NembraCaptureEntrypoint.swift")
        let makeExport = try section(in: app, from: "private func makeExport", to: "func prepareExport()")
        #expect(makeExport.contains("selectedPeripheralID: selectedID?.uuidString"))
        ...
        let prepare = try section(in: app, from: "func prepareExport()", to: "private func abandonPackageCorrelation")
        #expect(prepare.contains("envelope = sealedAcceptedExport"))
        #expect(prepare.contains("envelope = makeExport("))
        #expect(prepare.contains("exportData = try encoder.encode(envelope)"))
