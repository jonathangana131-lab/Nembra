...
struct TuyaPendingCorrelatedTargetAuthoritySourceTests {
    ...
    @Test("confirmation rechecks live source authority and consumes pending authority")
    ...
    @Test("attempt and authority reset boundaries retire an unconfirmed target")
    func stalePendingCandidateCannotCrossBoundary() throws {
        let app = try readRepositoryFile("NembraApp/App/NembraCaptureEntrypoint.swift")
    ...
    @Test("starting a new OFF1 series necessarily crosses reset")
    ...
    private func clearsPendingCorrelation(in source: Substring) -> Bool {
        source.contains("pendingCorrelatedTargetID = nil") || source.contains("clearPendingCorrelatedTarget")
    }
