...
/// Provenance contract for the Experiment One handoff.
...
struct ForegroundCoreBluetoothCaptureControllerExperimentOnePostAdmissionRediscoveryTests {
    ...
    @Test("sealed admission carries producer-issued monotonic handoff boundary")
    func admissionCarriesIssuedUptime() throws {
        let source = try Self.runSource()
        #expect(source.contains("let issuedAtUptimeNanoseconds: UInt64"))
        #expect(source.contains("DispatchTime.now().uptimeNanoseconds"))
        #expect(source.contains("issuedAtUptimeNanoseconds: issuedAtUptimeNanoseconds"))
    }

    @Test("controller requires post-admission advertisement before installing run recorder")
    func currentCatalogMustBePostAdmissionEvidence() throws {
        ...
        let freshness = try #require(
            body.range(of: "latestAdvertisement.receivedAtUptimeNanoseconds >= payload.issuedAtUptimeNanoseconds")
        )
    ...
    @Test("accepted post-admission advertisement retains its original callback receipt")
    func acceptedAdvertisementKeepsOriginalSoftwareReceipt() throws {
        let body = try Self.consumerBody(try Self.controllerSource())
        #expect(body.contains("receivedAtUptimeNanoseconds: latestAdvertisement.receivedAtUptimeNanoseconds"))
        #expect(body.contains("receivedAtDate: latestAdvertisement.receivedAtDate"))
    }
}
