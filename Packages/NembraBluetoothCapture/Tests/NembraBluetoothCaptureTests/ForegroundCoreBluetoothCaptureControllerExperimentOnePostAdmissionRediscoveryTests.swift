...
struct ForegroundCoreBluetoothCaptureControllerExperimentOnePostAdmissionRediscoveryTests {
    ...
    func currentCatalogMustBePostAdmissionEvidence() throws {
        ...
        let payloadMatch = try #require(body.range(of: "payload.peripheralIdentifier == preview.peripheralIdentifier"))
        ...
        #expect(consume.lowerBound < payloadMatch.lowerBound)
        #expect(payloadMatch.lowerBound < selectTarget.lowerBound)
        #expect(payloadMatch.lowerBound < installRecorder.lowerBound)
    }
}
