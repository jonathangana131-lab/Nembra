...
struct TuyaSecureLinkProductRecoveryTruthSourceTests {
    ...
    @Test("accepted export retry remains bound to the already-frozen envelope")
    func acceptedRetryCannotRebuildFromMutableState() throws {
        let app = try readRepositoryFile("NembraApp/App/NembraCaptureEntrypoint.swift")
        let controller = try section(
            in: app,
            from: "private final class SecureLinkController: NSObject, ObservableObject",
        ...
        #expect(body.contains("var acceptedArtifactCanPrepareExport: Bool"))
        #expect(body.contains("phase == .accepted && sealedAcceptedExport != nil"))
