...
struct TuyaFieldAppPreflightIntegrationSourceTests {
    ...
    func suspensionGapFailsClosedBeforeObservationAdvance() throws {
        ...
        guard let gapCheck = source.range(of: "maximumObservationPollGapNanoseconds"),
              let observationAdvance = source.range(of: "sessionLedger.observeCurrentConnection(for: token)", range: gapCheck.upperBound..<source.endIndex) else {
            Issue.record("Field source must reject a long monotonic observation gap before advancing ledger liveness.")
            return
        }
        #expect(gapCheck.lowerBound < observationAdvance.lowerBound)
    }
