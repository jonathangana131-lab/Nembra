...
struct TuyaFieldAppPreflightIntegrationSourceTests {
    ...
    @Test("field authority carries current-generation post-auth payload chronology")
    func fieldAuthorityCarriesCurrentGenerationPayloadChronology() throws {
        let entrypoint = try readRepositoryFile("NembraApp/App/NembraCaptureEntrypoint.swift")

        #expect(entrypoint.contains("connectionGeneration"))
        #expect(entrypoint.contains("latestApplicationPayloadUptimeNanoseconds"))
        #expect(entrypoint.contains("generation == connectionGeneration"))
        #expect(entrypoint.contains("latestApplicationPayloadUptime = receipt"))
        #expect(
            entrypoint.contains("maximumObservationPollGapNanoseconds"),
            "A long app/task observation gap must not silently count toward authenticated continuity."
        )
    }
