...
struct ForegroundCoreBluetoothCaptureControllerExperimentOneAdmissionContractTests {
    ...
    @Test("Experiment One target comes from sealed preview, requires post-handoff rediscovery, then matches consumed full UUID")
    func consumedTargetMustBeFreshlyDiscoveredByThisController() throws {
        ...
        #expect(source.contains("payload.issuedAtUptimeNanoseconds == preview.issuedAtUptimeNanoseconds"))
