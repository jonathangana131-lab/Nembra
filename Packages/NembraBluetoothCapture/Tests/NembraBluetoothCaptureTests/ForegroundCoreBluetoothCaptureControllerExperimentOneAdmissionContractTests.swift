...
struct ForegroundCoreBluetoothCaptureControllerExperimentOneAdmissionContractTests {
    ...
    @Test("Experiment One target is staged from the consumed full UUID and must exist in the current controller catalog")
    func consumedTargetMustBeFreshlyDiscoveredByThisController() throws {
        let source = Self.codeOnly(try Self.controllerSource())

        // Staging deliberately validates the current controller catalog before irreversible
        // one-shot consumption. The preview exposes only producer-bound identity/UUID/uptime.
