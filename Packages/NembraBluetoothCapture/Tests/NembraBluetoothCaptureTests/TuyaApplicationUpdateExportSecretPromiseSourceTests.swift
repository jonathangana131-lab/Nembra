...
struct TuyaApplicationUpdateExportSecretPromiseSourceTests {
    @Test("SmartLife application admission uses the canonical sanitizer before controller custody")
    ...
    @Test("secret custody does not introduce protocol or physical authority")
    func sanitizerIntegrationRemainsReadOnly() throws {
        let app = try readRepositoryFile("NembraApp/App/NembraCaptureEntrypoint.swift")
        let driver = String(try section(
            in: app,
        ...
        for forbidden in ["publishDps", "queryDps", "writeValue", "disconnectBLE"] {
            #expect(!driver.contains(forbidden), "Application secret custody must not add command authority: \(forbidden)")
        }
    }
