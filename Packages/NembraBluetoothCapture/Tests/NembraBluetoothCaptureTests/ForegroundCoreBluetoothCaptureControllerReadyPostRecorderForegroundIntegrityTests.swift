...
struct ForegroundCoreBluetoothCaptureControllerReadyPostRecorderForegroundIntegrityTests {
    ...
    func recordedReadyCannotCommitAfterForegroundLoss() throws {
        ...
        #expect(
            section.contains("requireForegroundEvidenceIntegrity()"),
            "Ready must recheck durable foreground integrity after the recorder actor returns and before promoting recorded Ready into queue-committed authority."
        )
    }
    ...
    func recordedReadyPostRecorderFailureUsesExplicitRecoveryPrecedence() throws {
        ...
        #expect(
            !section.contains("try? self.observationBoundaryQueueGate.abortRecordedReadyBeforeGateCommit"),
            "Recorded-Ready quarantine failure is stronger lifecycle evidence and must never be suppressed."
        )
    }
}
