...
struct TuyaCaptureFailedDiagnosticsProductSourceTests {
    ...
    func failedStatesExposeDiagnostics() throws {
        ...
        #expect(diagnostics.contains("test.diagnosticExportStatus"))
    }

    @Test("failed export remains mutable diagnostics and preserves the original failure message")
    func failedDiagnosticsUseMutableExportPathOnly() throws {
        ...
        #expect(prepareExport.contains("diagnosticExportStatus"))
        #expect(prepareExport.contains("if phase == .accepted"))
    }
