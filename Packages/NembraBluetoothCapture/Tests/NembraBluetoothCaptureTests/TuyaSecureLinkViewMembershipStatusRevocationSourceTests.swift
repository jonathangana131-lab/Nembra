...
struct TuyaSecureLinkViewMembershipStatusRevocationSourceTests {
    @Test("view exit resets operator membership copy with the revoked proof")
    func viewExitRevokesMembershipStatusWithProof() throws {
        ...
        try expectStatusRevocationOrdering(in: cleanup)
    ...
    @Test("foreground loss resets operator membership copy with the revoked proof")
    ...
    private func expectStatusRevocationOrdering(in cleanup: String) throws {
        let clearVerified = try requiredOffset(containing: "sdkDeviceMembershipVerified = false", in: cleanup)
        let clearAccount = try requiredOffset(containing: "membershipAccountUID = nil", in: cleanup)
        let clearDevice = try requiredOffset(containing: "membershipDeviceID = nil", in: cleanup)
