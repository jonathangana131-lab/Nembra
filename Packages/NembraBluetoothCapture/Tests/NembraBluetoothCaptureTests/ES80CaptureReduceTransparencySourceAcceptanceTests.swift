...
struct ES80CaptureReduceTransparencySourceAcceptanceTests {
    ...
    func reducedTransparencyStrengthensSurfaceSeparation() throws {
        ...
        #expect(
            ...
            "The shell needs an explicit reduced-transparency branch or token."
        )
        ...
        #expect(
            ...
            "Reduce Transparency should strengthen panel/card separation through a centralized fill token or an explicit conditional treatment, rather than leaving every low-alpha white/orange surface unchanged."
        )
    }
    ...
    func presentationAccessibilityDoesNotAlterAuthority() throws {
        ...
        #expect(
            !source.contains("accessibilityReduceTransparency && coordinator"),
            "Accessibility presentation state must never become part of Capture authority or coordinator progression."
        )
    }
}
