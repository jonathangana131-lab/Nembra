...
struct PassiveCoreBluetoothTerminalResolutionAppliedFrontierTests {
    ...
    @Test("terminal resolution receipt cannot reopen before controller applies resolved chronology")
    @MainActor
    func unappliedResolutionKeepsTerminalClosed() async throws {
        ...
        let controllerResolvedThrough: UInt64 = 12
        let resolution = try Resolution.resolve(
            currentResolvedThroughQueueSequence: controllerResolvedThrough,
            currentLastEnqueuedEventSequence: 14,
