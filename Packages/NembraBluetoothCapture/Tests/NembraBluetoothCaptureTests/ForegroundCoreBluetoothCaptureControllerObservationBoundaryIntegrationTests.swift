...
struct ForegroundCoreBluetoothCaptureControllerObservationBoundaryIntegrationTests {
    ...
    func targetSessionResetFailsClosedAfterReadyBegins() throws {
        ...
        let fenceOffset = try Self.offset(of: "try artifactAuthorityFence.transition(", in: session)
        ...
        #expect(!session[session.index(session.startIndex, offsetBy: fenceOffset)..<session.index(session.startIndex, offsetBy: targetOffset)].contains("await"))
    }
}
