...
struct ForegroundCoreBluetoothCaptureControllerObservationBoundaryIntegrationTests {
    ...
    func targetSessionResetFailsClosedAfterReadyBegins() throws {
        ...
        let authorityOffset = try Self.offset(of: "try artifactAuthorityFence.transition(", in: session)
        let generationOffset = try Self.offset(of: "targetSessionGeneration = freshAuthority.targetSessionGeneration", in: session)
        let targetOffset = try Self.offset(of: "targetState.selectTarget(identifier)", in: session)
        #expect(resetOffset < authorityOffset)
        #expect(authorityOffset < generationOffset)
