...
struct ForegroundCoreBluetoothCaptureControllerObservationBoundaryIntegrationTests {
    ...
    func readyDecisionPrecedesActorHop() throws {
        ...
        let authorityFenceOffset = try Self.offset(of: "authorityFence: artifactAuthorityFence", in: pipeline)
        ...
        let recordOffset = try Self.offset(of: "let outcome = try await admission.recordBoundaryWithMutationOutcome(on: recorder)", in: pipeline)
        ...
        #expect(frontierOffset < authorityFenceOffset)
        #expect(authorityFenceOffset < gateOffset)
    ...
    func targetSessionResetFailsClosedAfterReadyBegins() throws {
        ...
        let freshAuthorityOffset = try Self.offset(
            of: "let freshAuthority = PassiveCoreBluetoothArtifactAuthorityContext(",
            in: session
        )
        ...
        let targetGenerationOffset = try Self.offset(
            of: "targetSessionGeneration = freshAuthority.targetSessionGeneration",
            in: session
        )
        let authorityGenerationOffset = try Self.offset(
            of: "artifactAuthorityGeneration = freshAuthority.authorityGeneration",
            in: session
        )
        let targetOffset = try Self.offset(of: "targetState.selectTarget(identifier)", in: session)
        let recorderOffset = try Self.offset(of: "recorder = newRecorder", in: session)

        #expect(resetOffset < freshAuthorityOffset)
        #expect(freshAuthorityOffset < transitionOffset)
        #expect(transitionOffset < targetGenerationOffset)
        #expect(targetGenerationOffset < authorityGenerationOffset)
        #expect(authorityGenerationOffset < targetOffset)
        #expect(targetOffset < recorderOffset)
    }
}
