...
struct ForegroundCoreBluetoothCaptureControllerObservationBoundaryIntegrationTests {
    ...
    @Test("Ready cutoff, frontier, authority fence, and admission are bound before the first await")
    ...
    func targetSessionResetFailsClosedAfterReadyBegins() throws {
        ...
        let generationOffset = try Self.offset(
            of: "let nextTargetSessionGeneration = targetSessionGeneration + 1",
            in: session
        )
        ...
        #expect(resetOffset < generationOffset)
        #expect(generationOffset < fenceOffset)
        #expect(fenceOffset < targetOffset)
    }
}
