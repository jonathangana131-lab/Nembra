...
struct TuyaAuthenticationFailureSourceRaceTests {
    ...
    func authenticationFailureRechecksSourceAuthorityBeforeChoosingTerminal() throws {
        ...
        // Token identity is necessary but not sufficient. Callback scheduling must not decide
        // whether already-observed account/membership/UID drift is recorded as a source failure
        // or overwritten as a generic SDK authentication failure.
        ...
        // A failure callback while the exact same-account source lease is still authoritative
        // must retain the ordinary authentication-establishment terminal.
