...
struct ForegroundCoreBluetoothCaptureControllerRecoveryErrorPrecedenceTests {
    ...
    func recordedHorizonRecoveryPreservesErrorPrecedence() throws {
        ...
        let recoveryEnd = try #require(section.range(
            of: "\n            let data: Data",
            range: recoveryStart..<section.endIndex
        )?.lowerBound)
    ...
    func recordedReadyRecoveryPreservesErrorPrecedence() throws {
        ...
        let recordedCase = try #require(section.range(of: "case let .recorded(recordedReady):")?.lowerBound)
        let outerCatch = try #require(section.range(
            of: "\n                } catch {\n                    self.failCapture(error)",
            range: recordedCase..<section.endIndex
        )?.lowerBound)
