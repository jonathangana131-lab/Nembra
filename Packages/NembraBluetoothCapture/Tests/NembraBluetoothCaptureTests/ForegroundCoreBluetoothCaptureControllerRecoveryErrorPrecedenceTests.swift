...
/// Expected-red source contracts for the two recorded-boundary recovery seams where
/// durable recorder evidence already exists but the typed MainActor queue commit fails.
/// Exact quarantine failure is stronger lifecycle evidence and must never be hidden by
/// `try?` while the controller reports only the triggering commit error.
struct ForegroundCoreBluetoothCaptureControllerRecoveryErrorPrecedenceTests {
    ...
    func recordedHorizonRecoveryPreservesErrorPrecedence() throws {
        ...
        // Recovery itself must be allowed to throw. Suppressing that failure leaves
        // durable H with an unproven quarantine state while reporting a weaker error.
