...
struct TuyaApplicationUpdateSecretSanitizerTests {
    ...
    @Test("verified account UID is redacted by exact value without generic uid-key inference")
    ...
    @Test("empty exact sensitive value is a no-op rather than matching every string boundary")
    func emptyExactSensitiveValueIsNoOp() {
    ...
    func ordinaryEvidenceStaysVisible() {
        for key in ["battery", "speed", "mode", "deviceID", "timestamp", "uid"] {
