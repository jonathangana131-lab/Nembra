...
struct TuyaApplicationEventMetadataPrecedenceSourceTests {
    @Test("trusted generation is stamped after lease-bound untrusted event redaction")
    func trustedGenerationWinsReservedCollision() throws {
        ...
        let redaction = try #require(receiver.range(of: "var eventDetails = redactedApplicationEventDetails("))
        let admittedUID = try #require(receiver.range(of: "accountUID: admittedAccountUID", range: redaction.lowerBound..<receiver.endIndex))
        ...
        #expect(redaction.lowerBound < admittedUID.lowerBound)
        #expect(admittedUID.lowerBound < generation.lowerBound)
