...
struct TuyaCaptureForegroundIntegrityCurrentSourceTests {
    ...
    @Test("view exit also revokes completed correlation authority")
    func completedTargetCannotCrossViewBoundary() throws {
        let source = try entrypointSource()
        let controller = String(try section(
            in: source,
            from: "private final class SecureLinkController",
        ...
        #expect(cleanup.contains("phase == .correlated || phase == .selected"))
        #expect(cleanup.contains("resetDiscoverySessionOnly()"))
        #expect(cleanup.contains("target_correlation_authority_revoked_on_view_exit"))
    }
