...
struct ES80CaptureAccessibilitySourceAcceptanceTests {
    ...
    func captureHasNoCustomMotionOrTransparencyDependency() throws {
        ...
        let forbiddenPresentationDependencies = [
            "withAnimation", ".animation(", ".blur(", ".ultraThinMaterial", ".thinMaterial",
            ".regularMaterial", ".thickMaterial", ".ultraThickMaterial", ".glassEffect("
        ]
        for dependency in forbiddenPresentationDependencies {
            #expect(!source.contains(dependency), "Capture gained a custom motion/transparency dependency that needs explicit accessibility treatment: \(dependency)")
        }
    ...
    func pollingPresentationDoesNotExposeEveryChildAsAnAccessibilityElement() throws {
        ...
        let progressEnd = try #require(source.range(of: "private func primaryContent(", range: progressStart.lowerBound..<source.endIndex))
        ...
        let primaryEnd = try #require(source.range(of: "private func correlationReadyPanel(", range: primaryStart.lowerBound..<source.endIndex))
        ...
        for term in ["bounded Bluetooth observation window", "Bluetooth observation", "CoreBluetooth", "finite acquisition", "package-owned"] {
            #expect(!primarySource.contains(term), "Primary VoiceOver copy still exposes implementation vocabulary: \(term)")
        }
    }
