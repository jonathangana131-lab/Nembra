...
struct TuyaApplicationSecretValueCustodySourceTests {
    ...
    func privateApplicationIdentityIsValueBoundBeforeEventCustody() throws {
        ...
        #expect(sanitizer.contains("redactExactSecretValues"))
        #expect(sanitizer.contains("redactedApplicationDescription"))

        // Exact private credential values must already be scrubbed by the recursive sanitizer
        // before the callback projects opaque evidence into String form for immutable custody.
    ...
    @Test("public builds have no synthetic secret values to redact")
    func exactSecretValuesRemainPrivateBuildBound() throws {
        let source = try readRepositoryFile("NembraApp/App/NembraCaptureEntrypoint.swift")
        let driver = String(try section(
            in: source,
            from: "@MainActor\nprivate final class SmartLifeDriver",
        ...
        #expect(driver.contains("#if canImport(NembraTuyaPrivateConfig)"))
        #expect(driver.contains("return []"))
