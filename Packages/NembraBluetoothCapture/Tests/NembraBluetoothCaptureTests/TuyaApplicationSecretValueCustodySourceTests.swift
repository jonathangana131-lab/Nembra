...
struct TuyaApplicationSecretValueCustodySourceTests {
    ...
    func privateApplicationIdentityIsValueBoundBeforeEventCustody() throws {
        ...
        // Key-name classification is necessary but not sufficient: an SDK payload may place an
        // exact private credential value under an opaque/numeric/otherwise innocuous key. The
        // exact compiled Tuya application identity therefore has to participate in recursive
        // value redaction before String(describing:) crosses into immutable event custody.
