...
struct TuyaFieldInstallerProcedureSourceTests {
    ...
    @Test("installer keeps exact source and physical stop conditions")
    func installerFailsClosedAtPhysicalBoundary() throws {
        let installer = try readRepositoryFile("scripts/field/install_one_time_capture.command")

        #expect(installer.contains("Pass the exact software-accepted Capture source SHA"))
        #expect(installer.contains("does not match accepted Capture source"))
