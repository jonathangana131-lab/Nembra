...
struct TuyaApplicationAccountUIDExportCustodySourceTests {
    ...
    func applicationEvidenceCannotExportVerifiedAccountUID() throws {
        ...
        #expect(updateAdmission.contains("guard let verifiedAccountUID = membershipAccountUID"))
    ...
    func accountUIDCannotEscapeThroughMalformedKeyOrValue() throws {
        let source = try readRepositoryFile("NembraApp/App/NembraCaptureEntrypoint.swift")
        let updateAdmission = String(try section(
            in: source,
            from: "private func receivedApplicationUpdate(",
        ...
        #expect(updateAdmission.contains("entry.key.replacingOccurrences"))
        #expect(updateAdmission.contains("entry.value.replacingOccurrences"))
        #expect(updateAdmission.components(separatedBy: "of: verifiedAccountUID").count - 1 >= 2)
        #expect(updateAdmission.components(separatedBy: "with: \"<redacted-account-uid>\"").count - 1 >= 2)
    }
