...
struct TuyaApplicationAccountUIDExportCustodySourceTests {
    ...
    func applicationEvidenceCannotExportVerifiedAccountUID() throws {
        ...
        #expect(custody.contains("membershipAccountUID"))
        #expect(custody.contains("<redacted-account-uid>"))
        #expect(custody.contains("key.replacingOccurrences("))
        #expect(custody.contains("value.replacingOccurrences("))
        #expect(custody.contains("while redacted[uniqueKey] != nil"))
        #expect(updateAdmission.contains("applicationUpdateForEventCustody(update)"))
