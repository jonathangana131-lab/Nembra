...
struct TuyaApplicationAccountUIDExportCustodySourceTests {
    ...
    func applicationEvidenceCannotExportVerifiedAccountUID() throws {
        ...
        let scrubber = String(try section(
            in: source,
            from: "private func redactVerifiedAccountUIDFromApplicationUpdate(",
        ...
        #expect(updateAdmission.contains("redactVerifiedAccountUIDFromApplicationUpdate(update)"))
        #expect(updateAdmission.contains("sanitizedUpdate.merging(["))
        #expect(!updateAdmission.contains("log(\"tuya_application_update\", update.merging(["))

        // The scrubber is bound to the already-earned current-account lease and covers both
        // dictionary keys and values so malformed application data cannot evade the export promise.
        #expect(scrubber.contains("membershipAccountUID"))
        #expect(scrubber.contains("trimmingCharacters(in: .whitespacesAndNewlines)"))
        #expect(scrubber.contains(".caseInsensitive"))
        #expect(scrubber.contains("<redacted-account-uid-key-"))
