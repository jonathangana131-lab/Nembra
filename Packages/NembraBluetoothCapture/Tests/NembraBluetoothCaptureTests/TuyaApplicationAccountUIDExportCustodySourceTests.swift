...
struct TuyaApplicationAccountUIDExportCustodySourceTests {
    ...
    @Test("exact verified account UID is scrubbed from application keys and values without blanket uid-key erasure")
    func accountUIDCustodyCoversKeysAndValues() throws {
        let source = try readRepositoryFile("NembraApp/App/NembraCaptureEntrypoint.swift")
        ...
        #expect(controller.contains("of: accountUID"))
        #expect(controller.contains("with: \"<redacted-account-uid>\""))
        #expect(!driver.contains("\"uid\","))
        #expect(!driver.contains("\"uid\"\n"))
    }

    @Test("secret classifier contains one session-key entry after custody simplification")
    func sessionKeyClassifierIsNotDuplicated() throws {
        let source = try readRepositoryFile("NembraApp/App/NembraCaptureEntrypoint.swift")
        let driver = String(try section(
            in: source,
            from: "@MainActor\nprivate final class SmartLifeDriver",
