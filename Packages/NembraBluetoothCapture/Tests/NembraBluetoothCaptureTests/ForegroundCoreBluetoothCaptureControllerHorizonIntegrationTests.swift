...
struct ForegroundCoreBluetoothCaptureControllerHorizonIntegrationTests {
    ...
    @Test("Horizon admission requires a durably committed Ready epoch under current authority")
    ...
    @Test("terminal Horizon permit and gate transaction are synchronous before first await")
    func horizonDecisionPrecedesFirstAwait() throws {
        ...
        #expect(permitOffset < transactionOffset)
        #expect(transactionOffset < cutoffOffset)
        #expect(cutoffOffset < frontierOffset)
        #expect(frontierOffset < gateOffset)
        #expect(gateOffset < firstAwaitOffset)
    }

    @Test("Horizon drains exact prefix, records and commits H, freezes JSON, then resolves post-cut FIFO")
