...
struct AuthenticatedStationaryCaptureSignedBuildEvidenceTests {
    ...
    @Test func canonicalEvidenceDecodesAndReconstructsAuthorizationBindings() throws {
    ...
    @Test func exactPythonStyleCanonicalBytesAreRequired() throws {
    ...
    @Test func duplicateAndUnexpectedFieldsFailClosed() throws {
        ...
        let duplicate = Data(
            canonical.replacingOccurrences(
                ...
                options: [],
                range: canonical.startIndex..<canonical.endIndex
            ).utf8
        )
    ...
    @Test func zeroDigestAndNonV4BuildInstanceAreRejected() throws {
    ...
    @Test func evidenceMustMatchTheRunningBuildTupleBeforeAttemptCreation() throws {
        let data = try canonicalEvidence()
    ...
    private func canonicalEvidence(overrides: [String: String] = [:]) throws -> Data {
        ...
        let lines = keys.enumerated().map { index, key in
            "  \(try! jsonString(key)): \(values[key]!)\(index + 1 == keys.count ? "" : ",")"
        }
