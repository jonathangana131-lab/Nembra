...
struct ES80CaptureSimulatorPreflightSnapshotHandoffTests {
    ...
    @Test("canonical acceptance comes from exact-ledger receipted evidence and an immutable seal")
    func acceptedCaptureCannotBeMintedFromSyntheticSnapshotState() throws {
        ...
        #expect(source.contains("sessionLedger.captureApplicationReceipt("))
        ...
        #expect(!source.contains("TuyaReadOnlyApplicationReceipt.capture(for:"))
        #expect(!source.contains("simulatorQASnapshot"))
    }
}
