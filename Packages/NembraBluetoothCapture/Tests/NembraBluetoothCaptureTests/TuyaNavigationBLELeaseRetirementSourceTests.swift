...
struct TuyaNavigationBLELeaseRetirementSourceTests {
    ...
    @Test("view-exit cleanup retires transport through the existing ordered package path")
    func viewExitCleanupDoesNotReleaseLeaseOrMutateScooterDirectly() throws {
        let source = try readRepositoryFile("NembraApp/App/NembraCaptureEntrypoint.swift")
