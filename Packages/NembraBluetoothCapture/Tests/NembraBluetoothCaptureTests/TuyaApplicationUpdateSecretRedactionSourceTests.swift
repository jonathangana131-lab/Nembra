...
struct TuyaApplicationUpdateSecretRedactionSourceTests {
    ...
    @Test("export still declares redaction only while application events use the redacted driver boundary")
    func exportRedactionClaimIsCoupledToApplicationBoundary() throws {
        let source = try readRepositoryFile("NembraApp/App/NembraCaptureEntrypoint.swift")
