...
///
/// Foundation's `JSONSerialization` and `JSONDecoder` intentionally expose keyed object models,
/// so duplicate JSON members can collapse with different precedence between the two parsers. Any
/// signed or externally accepted object that is validated with one parser and decoded with the
/// other must reject duplicate semantic keys before authority is interpreted.
///
