...
/// Nembra-owned event metadata.
///
/// The SDK payload remains application evidence. It must not be able to impersonate
/// Nembra-owned provenance fields, and the exact Tuya account UID used as source
/// authority must not cross into an exported Capture event.
///
/// This primitive does not assign any DP meaning, authenticate a device, create a
...
public enum TuyaAuthenticatedApplicationEventCustody {
    ...
    /// Returns flat event details that preserve every application field while reserving
    /// `generation` for Nembra's trusted connection-token provenance.
    ///
    /// If the application payload itself contains `generation`, that value is retained
    /// under a deterministic `application.` namespace rather than being allowed to
    /// overwrite trusted provenance. Namespace collisions are resolved by repeatedly
    /// prefixing `application.` so no application field is discarded.
    ///
    /// The exact account UID is redacted by value from both application keys and values.
    /// A generic key such as `uid` is intentionally not suppressed because it may be
    /// legitimate device/application evidence unrelated to the authenticated account.
