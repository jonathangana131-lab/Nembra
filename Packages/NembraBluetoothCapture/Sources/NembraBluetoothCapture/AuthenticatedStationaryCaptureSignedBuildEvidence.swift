...
///
/// The producer uses Python `json.dumps(..., ensure_ascii=False, indent=2, sort_keys=True) + "\n"`.
/// Current field build identifiers are repository-generated printable ASCII, so this decoder accepts
/// that bounded production subset and reconstructs the exact producer bytes. The exact-byte SHA is
/// what the later independently signed authorization payload binds.
public enum AuthenticatedStationaryCaptureSignedBuildEvidenceVerifier {
    ...
    private static func canonicalBytes(for wire: Wire) throws -> Data {
        ...
        for (index, pair) in values.enumerated() {
            let encodedKey = try jsonString(pair.0)
            lines.append("  \(encodedKey): \(pair.1)\(index + 1 == values.count ? "" : ",")")
        }
