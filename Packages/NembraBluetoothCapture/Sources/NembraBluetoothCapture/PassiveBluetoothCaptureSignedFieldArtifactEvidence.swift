...
public enum PassiveBluetoothCaptureSignedFieldArtifactEvidenceJSON {
    ...
    /// `JSONSerialization` and `JSONDecoder` collapse duplicate object members into keyed storage.
    /// Scan the exact UTF-8 evidence bytes first so parser precedence can never decide which of two
    /// conflicting field declarations becomes the mechanical rendezvous subject. Escaped and
    /// unescaped spellings of the same semantic key are treated as duplicates.
