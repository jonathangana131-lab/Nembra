...
///
/// Schema v2 preserves the current scoped receipt-sequence authority and explicit
/// packet-zero restart boundary. Raw encrypted bytes remain only in the source
/// artifact; this report records byte counts and exact source references instead
/// of creating a second evidence object.
public struct PassiveBluetoothTuyaCaptureReport: Equatable, Codable, Sendable {
    ...
    /// observation. Receipt sequence/scope is preserved separately from clock
    /// metadata so downstream tooling never has to reconstruct ordering.
    ...
    /// use millisecond epoch precision and sorted keys keep reports reviewable.
...
public enum PassiveBluetoothTuyaCaptureReportBuilder {
    /// Returns exact peripheral identifiers with target-attributable connection,
    /// GATT, subscription, or value evidence in first-observed order. Broad-scan
    /// advertisement alone never becomes target authority.
    ...
    /// Resource limits are caller-owned offline safety bounds, not physical ES80
    /// protocol limits or learned device properties.
    ...
    private static func makeStreamReport(
        ...
        return .init(
            ...
            fragments: transcript.fragments.enumerated().map { index, fragment in
                sourceReference(fragment, observationIndex: index)
            },
            events: try analysis.events.map {
                try makeEventReport($0, transcript: transcript, streamIndex: streamIndex)
            }
        )
    }
    ...
    private static func makeEventReport(
        ...
        switch event {
        case let .completed(start, end, message):
            return .init(
                kind: .completed,
                startObservationIndex: start,
                endObservationIndex: end,
                lastAcceptedObservationIndex: end,
                failingObservationIndex: nil,
                nextObservationIndex: nil,
                boundary: nil,
                error: nil,
                startSource: try requiredSource(transcript, index: start, streamIndex: streamIndex),
                endSource: try requiredSource(transcript, index: end, streamIndex: streamIndex),
                lastAcceptedSource: try requiredSource(transcript, index: end, streamIndex: streamIndex),
                failingSource: nil,
                nextSource: nil,
        ...
        case let .rejectedCandidate(start, lastAccepted, failing, error):
            return .init(
                kind: .rejectedCandidate,
                startObservationIndex: start,
                endObservationIndex: nil,
                lastAcceptedObservationIndex: lastAccepted,
                failingObservationIndex: failing,
                nextObservationIndex: nil,
                boundary: nil,
                error: describe(error),
                startSource: try requiredSource(transcript, index: start, streamIndex: streamIndex),
                endSource: nil,
                lastAcceptedSource: try lastAccepted.map {
                    try requiredSource(transcript, index: $0, streamIndex: streamIndex)
                },
                failingSource: try requiredSource(transcript, index: failing, streamIndex: streamIndex),
                nextSource: nil,
                completedMessage: nil
            )

        case let .incompleteAtBoundary(start, lastAccepted, next, boundary):
            return .init(
                ...
                lastAcceptedObservationIndex: lastAccepted,
                failingObservationIndex: nil,
                nextObservationIndex: next,
                boundary: describe(boundary),
                error: nil,
                startSource: try requiredSource(transcript, index: start, streamIndex: streamIndex),
                endSource: nil,
                lastAcceptedSource: try requiredSource(transcript, index: lastAccepted, streamIndex: streamIndex),
                failingSource: nil,
                nextSource: try requiredSource(transcript, index: next, streamIndex: streamIndex),
                completedMessage: nil
            )

        case let .incompleteAtEnd(start, lastAccepted):
            return .init(
        ...
        case let .unexpectedAnalyzerFailure(failing):
            return .init(
    ...
    private static func requiredSource(
        ...
        guard let fragment = transcript.sourceFragment(atAnalysisObservationIndex: index) else {
            throw PassiveBluetoothTuyaCaptureReportError.sourceFragmentUnavailable(
                streamIndex: streamIndex,
                observationIndex: index
            )
        }
        return sourceReference(fragment, observationIndex: index)
    }
