...
/// terminal acceptance/failure horizons. Payload bytes are inspected only for non-emptiness and
/// are never retained by this type.
public actor TuyaAuthenticatedReadOnlySessionLedger: TuyaReadOnlyAuthenticationSessionProvider {
    ...
    public func markApplicationObservationTimedOut(
        ...
        authenticationState = .failed(
            reason: "Authenticated session produced no application payload before the observation deadline."
        )
        latestObservedUptimeNanoseconds = now
        currentToken = nil
    }
