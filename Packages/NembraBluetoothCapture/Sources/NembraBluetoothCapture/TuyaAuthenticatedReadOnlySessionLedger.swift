...
/// Owns the non-secret chronology used by `TuyaAuthenticatedReadOnlyPreflight`.
///
/// The eventual official Tuya adapter should report lifecycle events here rather than assemble
/// `TuyaAuthenticatedReadOnlyPreflightSnapshot` values itself. The ledger samples the monotonic
/// clock at the mutation boundary, resets payload evidence on every new connection, and rejects
/// callbacks attributed to an older connection token. Payload bytes are inspected only for
...
public actor TuyaAuthenticatedReadOnlySessionLedger: TuyaReadOnlyAuthenticationSessionProvider {
    ...
    public func markAuthenticated(for token: TuyaReadOnlyConnectionToken) throws {
        try requireCurrent(token)
        ...
        latestObservedUptimeNanoseconds = now
        // Evidence from before the accepted authentication boundary can never satisfy the gate.
        applicationPayloadCount = 0
        latestApplicationPayloadUptimeNanoseconds = nil
    }
