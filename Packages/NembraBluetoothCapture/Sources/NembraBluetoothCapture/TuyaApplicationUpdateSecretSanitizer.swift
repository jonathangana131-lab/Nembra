...
public enum TuyaApplicationUpdateSecretSanitizer {
    ...
    // Keep this classifier at least as strong as Capture's explicit accepted-export promise:
    // no AppKey/AppSecret, password, account token, local_key, session key, or SDK auth tokens.
    ...
    /// recursively replaced. Non-secret values remain opaque application evidence; these
    /// strings are not raw BLE bytes or decoded DP semantics.
