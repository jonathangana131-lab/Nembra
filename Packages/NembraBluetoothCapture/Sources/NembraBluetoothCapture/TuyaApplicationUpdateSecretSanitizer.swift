...
public enum TuyaApplicationUpdateSecretSanitizer: Sendable {
    ...
    ///
    /// The caller must supply the exact value from its own authority boundary. This intentionally
    /// does not infer sensitive values from generic keys such as `uid`, because those keys may be
    /// legitimate device evidence unrelated to an account identity.
