/// Converts a producer-issued pre-H abandoned-session retirement receipt into
...
/// old Ready epoch is quarantined and its drain is idle, the complete still-pending
/// suffix for that abandoned target session was validated and removed. Those queue
/// positions are therefore settled by an accepted lifecycle decision, but they are
/// not recorder evidence.
///
