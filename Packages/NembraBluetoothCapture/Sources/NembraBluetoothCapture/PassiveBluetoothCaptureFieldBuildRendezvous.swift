...
/// the exact field-build evidence bytes, the exact external build record, and the executable plus
/// raw Info.plist bytes that Nembra is actually running.
///
...
public struct PassiveBluetoothCaptureFieldBuildRendezvous: Equatable, Sendable {
    ...
    /// rendezvous itself must remain available to any future field-acceptance layer because the
    /// older SoftwareExport reference does not carry the Info.plist digest.
