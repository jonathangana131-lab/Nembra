...
/// File presence is never authority: it contains only the fresh challenge rendezvous facts and the
...
public struct AuthenticatedStationaryCaptureSignerRendezvousOutbox: Sendable {
    ...
    public func publish(
        ...
        // `renameat` atomically replaces any old process's stale complete document. A reader can
        // observe either the old full rendezvous or this new full rendezvous, never partial bytes.
        // An envelope produced from an old document still fails the current process-local challenge.
