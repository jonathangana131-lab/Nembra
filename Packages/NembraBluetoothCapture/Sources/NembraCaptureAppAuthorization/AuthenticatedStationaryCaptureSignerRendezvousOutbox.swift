...
public struct AuthenticatedStationaryCaptureSignerRendezvousOutbox: Sendable {
    ...
    /// Opens the Foundation-provided Application Support location without assuming it has already
    /// been materialized in a fresh app container. When creation is allowed, only the final
    /// Application Support component is created descriptor-relatively beneath its already-existing
    /// parent, and both parent/base custody are checked before any child handoff directory exists.
