...
Attempted follow-up `b16cec4c437b848339bd5b154ed43edf66220b2b` repinned the TODAY custody subject to a changed trusted-workflow blob. Exact hosted Final-GO run `32326183236` correctly rejected it because `es80_today_trusted_capture_xcode_subject.py` no longer matched the independently accepted executable bundle (`3a6e6609…` versus accepted `94d19c0c…`). It is retained in history as rejected evidence and is superseded by the corrective checkpoint below; do not use it as accepted authority.
