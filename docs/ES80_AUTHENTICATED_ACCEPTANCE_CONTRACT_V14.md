...
The old approximately-29.93-second unauthenticated disconnect pattern is not closed by generic Bluetooth liveness alone. The final physical artifact must prove that the authenticated **application notification path itself** survived beyond that region.
...
1. At least **two** genuine, non-empty authenticated application payloads have been admitted.
2. The latest accepted application payload arrived at least **30 seconds after authentication**.
3. The authenticated generation remained continuously accepted for at least **45 seconds after authentication**.
4. The accepted authentication provenance is the current SmartLife SDK-authenticated BLE generation.
5. Chronology is valid and belongs to the same current generation.
...
`docs/ES80_PHYSICAL_TRUTH_C7D09A22.md` is the physical predecessor ledger and now states the same 2 / 30 s late-payload / 45 s continuity boundary.
