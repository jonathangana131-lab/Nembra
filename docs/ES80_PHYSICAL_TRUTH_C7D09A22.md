...
5. Observe only through the accepted single-owner authenticated application path and preserve admitted evidence before interpreting it. Structured SDK application updates remain application-level evidence unless a separately accepted byte-exact FD50 path is proven.

The authenticated gate is accepted only when **all** canonical conditions are demonstrated in the same real physical session and current authenticated generation:

- at least **two genuine, non-empty application payloads** are admitted;
- the **latest accepted application payload occurs at least 30 seconds after authentication**, proving the application path itself survived beyond the historical rejection region; and
- the authenticated generation reaches at least **45 seconds of canonical authenticated continuity measured from authentication**.

One bootstrap callback plus generic transport liveness is not enough. A connection that survives 45 seconds without repeated application evidence, or repeated early callbacks whose latest accepted payload never reaches the 30-second post-authentication boundary, does not close the gate. Simulator/test payloads do not become physical evidence.

The canonical operational procedure is `docs/CAPTURE_P0_SECURE_LINK_NEXT_TEST.md`; when supporting historical material conflicts with that procedure or the compiled `TuyaAuthenticatedReadOnlyPreflight` acceptance constants, the stricter current canonical contract wins.

## After gate closure — stationary DP mapping first

Once authenticated application evidence closes the gate, move immediately into physical DP mapping using stationary scenarios before any repeat outdoor ride. Preserve admitted source data and timestamps, change one observable condition at a time, and distinguish observed correlation from accepted semantics.
