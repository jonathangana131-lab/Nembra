...
This lane depends on the distance disclosure contract from PR #346 and composes its exact additive blobs with current main. It does not modify #346-owned files.

The new composite source/tests are additive NembraCore package work. They do not wire Home, Dashboard, Ride Details/AppRoot, persistence, Bluetooth/Tuya, navigation, battery/range, or Xcode project files. App integration should consume this product-safe boundary once the dependent distance presentation is accepted and the relevant UI lane is free.
