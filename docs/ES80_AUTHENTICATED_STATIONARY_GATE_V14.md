...
## Canonical authenticated acceptance contract

The machine authority on current main is `TuyaAuthenticatedReadOnlyPreflight`. Its acceptance conditions are conjunctive and must not be weakened by product copy, tests, runbooks, or physical-truth prose:

- authenticated continuity: at least **45.0 seconds** after accepted SmartLife SDK authentication;
- genuine application evidence: at least **two** admitted non-empty application payloads in the same authenticated generation; and
- post-authentication application survival: the **latest admitted application payload must arrive at least 30.0 seconds after authentication**.

The two-payload rule prevents a single bootstrap/state-replay callback from being treated as an ongoing authenticated notification path. The post-30-second payload requirement proves application-path survival rather than generic transport liveness. The 45-second continuity requirement preserves margin beyond the approximately-29.930-second unauthenticated rejection observed in C7D09A22.

A connection that satisfies only one or two of these conditions is still blocked.
...
- live `main`: `990cbff33b70904aa6af261f86b77d3de8a6cef1`;
- the canonical machine gate on main already encodes `45 s` authenticated continuity, `2` application payloads, and a latest payload at least `30 s` after authentication;
- this document and `docs/ES80_PHYSICAL_TRUTH_C7D09A22.md` previously described a weaker one-payload / `>30 s` gate and are therefore non-authoritative until aligned with the machine contract;
- an existing terminal-horizon fixture also attempted to seal after only one early application update and must be aligned before the package test suite can represent the canonical contract truthfully;
- currently open product PRs are outside these Capture gate paths, so this truth-convergence lane does not claim their Dashboard/Rides ownership.

These facts remain coordination checkpoints, not physical acceptance. A package green, source review, queued/skipped workflow, ancestor result, or documentation edit cannot authorize the physical session. Future workers must refresh live GitHub; newer accepted composition wins over this snapshot.
...
11. **Genuine authenticated application evidence.** The physical artifact must not relabel a structured SDK `dpsUpdate` string projection as byte-exact FD50 notification data. The accepted gate requires genuine current-generation non-empty application evidence with chronology/provenance. If final physical acceptance additionally requires raw FD50 notification bytes, the composed build must prove an accepted legitimate raw-byte source under the one-owner rule and feed it through a credential-free admission ledger. Structured SDK application updates may be retained as application-level evidence only when labeled accurately.
12. **No silent weakening of raw-vs-structured evidence.** If the supported official SDK surface cannot lawfully expose byte-exact notifications without violating the one-owner/session contract, make that limitation explicit. Do not call `String(describing:)` DP values “raw payloads.” A reviewed successor may define the smallest lawful evidence gate, but terminology cannot be weakened to manufacture a pass.
13. **Opaque payloads do not mint telemetry semantics.** Even repeated genuine application payloads prove only application-layer receipt until a separately accepted repeatable decoding/correlation contract exists. They do not authorize speed, battery, voltage, current, power, mode, odometer, command acknowledgement, or any other field meaning.
14. **Canonical evidence seal/export.** Accepted application/raw evidence and target-correlation provenance must be admitted through the accepted chronology/provenance model, frozen immutably, and exported without credentials/secrets. Delayed callbacks after seal cannot mutate the accepted prefix.
...
19. **App-visible gate copy must match machine authority.** The observing UI must not present `applicationUpdateCount > 0` as though one payload were sufficient. It must make repeated application evidence and the 30-second application-survival boundary truthful and understandable without exposing raw engineering jargon.
...
5. Preserve every admitted application evidence item with exact current connection generation, source identity, callback/receipt order, monotonic timing, and build/procedure provenance. Where byte-exact FD50 notification evidence is lawfully accepted, preserve the raw bytes and exact notification characteristic identity; otherwise preserve structured SDK evidence under its accurate application-level label.
6. Require at least **two** genuine non-empty application evidence items in the same authenticated generation.
7. Require the **latest** admitted application payload to arrive at least **30.0 seconds after authentication**.
8. Keep accepted authenticated continuity alive for at least **45.0 seconds after authentication**.
9. Seal through the canonical evidence path and use the app's normal Share/export flow only after all canonical acceptance conditions are met.
...
- at least **two genuine non-empty application payloads** are admitted from the accepted physical authenticated application path;
- the **latest admitted application payload is at least 30.0 seconds after authentication**;
- the authenticated connection remained continuously accepted for at least **45.0 seconds after authentication**;
- byte-exact evidence is preserved where the final accepted one-owner source requires and lawfully provides it, while structured SDK evidence remains distinctly labeled;
...
A transport callback, write completion, notification subscription success, timer UI, one bootstrap application callback, two callbacks that both precede the 30-second application-survival boundary, structured `dpsUpdate` text mislabeled as raw bytes, or 45 seconds without the required repeated late application evidence is **not** a physical PASS.
...
- the session disconnects before 45.0 seconds of accepted authenticated continuity;
- fewer than two genuine application payloads are admitted;
- the latest application payload does not reach the 30.0-second post-authentication survival boundary;
- required byte-exact evidence is unavailable where the final accepted raw-evidence contract requires it;
