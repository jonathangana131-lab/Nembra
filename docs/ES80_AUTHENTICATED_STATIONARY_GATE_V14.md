...
The physical acceptance wording in this document must remain aligned with `TuyaAuthenticatedReadOnlyPreflight`; prose must never weaken the compiled gate. The current canonical minimums are:

- `minimumAuthenticatedApplicationPayloadCount = 2`;
- `minimumPostAuthenticationPayloadSurvivalNanoseconds = 30_000_000_000` — the latest qualifying application payload must be at least 30.0 seconds after authentication;
- `minimumAuthenticatedConnectionNanoseconds = 45_000_000_000` — authenticated continuity must reach at least 45.0 seconds after authentication.

A one-payload / merely-`>30 s` description is obsolete and is not authorized to classify a physical PASS.
...
- compiled canonical auth preflight on main already requires 2 application payloads, latest-payload survival >=30 s after authentication, and authenticated continuity >=45 s;
- the immediate defect being repaired by this revision is stale/weaker physical-gate prose, not the compiled `TuyaAuthenticatedReadOnlyPreflight` authority;
- open non-Capture product work remains independently non-authorizing for this physical gate.

These are coordination facts, not accepted composition. A child PR, donor branch, package green, source review, queued/skipped workflow, or ancestor Xcode success cannot authorize the physical session. Future workers must refresh live GitHub; newer accepted composition wins over this snapshot.
...
7. Keep authenticated continuity accepted for at least **45.0 seconds after authentication**. The app may show guidance, but only accepted monotonic evidence can satisfy this boundary.
...
- the latest qualifying admitted application payload is at least **30.0 seconds after authentication**;
...
- the latest qualifying application payload is earlier than 30.0 seconds after authentication;
