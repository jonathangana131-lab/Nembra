...
- at least **two** genuine, non-empty application payloads;
- the latest accepted application payload at least **30 seconds after authentication**; and
- at least **45 seconds of accepted authenticated continuity after authentication**.

A weaker prose rule such as “one payload + >30 seconds connected” is not an alternate acceptance path and must not be used to authorize field execution or stationary DP mapping.
...
The older PR/head list below is retained only as archaeology for the path that produced the current foundation. It is **not live acceptance state** and must not be used without a fresh GitHub inspection:
...
6. Keep the authenticated generation continuously accepted for at least **45 seconds after authentication**.
7. Require at least **two** genuine, non-empty application payloads on that generation, with the latest accepted payload arriving at least **30 seconds after authentication**.
...
- at least **two genuine non-empty application notification payloads** are admitted from the accepted physical application/notify source; byte-exact evidence is preserved where the final GO contract requires it;
- the **latest accepted application payload is at least 30 seconds after authentication**;
- the authenticated connection maintained at least **45 seconds of accepted authenticated continuity after authentication**;
...
A transport callback, write completion, notification subscription success, timer UI, structured `dpsUpdate` string projection mislabeled as raw bytes, one/bootstrap-only application payload, two early payloads followed only by generic BLE liveness, or 45 seconds without the required sustained application evidence is **not** a physical PASS.
...
- authenticated continuity ends before 45 seconds after authentication;
- fewer than two genuine application payloads are admitted, or the latest accepted application payload does not reach 30 seconds after authentication;
