...
Historical runbooks and retained field artifacts from the pre-C7D09A22 flow do not authorize this authenticated-session experiment. Do not reuse an old Final GO subject, signed IPA, retained-artifact identity, or historical target UUID as authorization for this gate.
...
Fresh live state at the start of this reconciliation:

- `main`: `990cbff33b70904aa6af261f86b77d3de8a6cef1`;
- compiled canonical preflight requires two application payloads, latest accepted payload at least 30 seconds after authentication, and at least 45 seconds of authenticated continuity;
- the prior canonical procedure file was absent from `main` even though current source tests and the Capture authority pointer referenced it;
- the supporting C7D09A22 physical-truth document had regressed to a weaker one-payload / `>30 s` description;
- no open Capture PR existed when this reconciliation branch was claimed; unrelated open Dashboard/Rides work remains outside this lane.

These are coordination facts, not physical acceptance. Future workers must refresh live GitHub; newer accepted composition wins over this snapshot.
...
7. Keep the accepted authenticated session alive for at least **45 seconds of canonical authenticated continuity measured from authentication**.
...
- at least **two genuine non-empty application payloads** are admitted from the accepted authenticated application source;
- the **latest accepted application payload is at least 30 seconds after authentication**, proving the application path itself survived beyond the old rejection window;
- authenticated continuity reaches at least **45 seconds measured from authentication**;
...
One callback plus 45 seconds of generic connection liveness is **not** a physical PASS. Two early callbacks whose latest payload is before the 30-second post-authentication boundary are also **not** a physical PASS. A transport callback, write completion, notification subscription success, timer UI, structured `dpsUpdate` projection mislabeled as raw bytes, or 45 seconds without accepted repeated application evidence does not close the gate.
...
- authenticated continuity does not reach at least 45 seconds from authentication;
...
Do not repair a failed physical attempt by relabeling missing evidence, substituting a different build, weakening application-evidence timing, weakening raw-vs-structured evidence terminology, or inferring protocol semantics from timing/GPS/UI behavior.
