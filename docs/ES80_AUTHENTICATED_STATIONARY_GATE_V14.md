...
## Canonical compiled acceptance contract

This physical gate must remain no weaker than shipping `TuyaAuthenticatedReadOnlyPreflight`. The current canonical minimums are:

- `minimumAuthenticatedApplicationPayloadCount = 2`;
- `minimumPostAuthenticationPayloadSurvivalNanoseconds = 30_000_000_000` — the latest qualifying application payload must arrive at least 30.0 seconds after authentication;
- `minimumAuthenticatedConnectionNanoseconds = 45_000_000_000` — accepted authenticated continuity must reach at least 45.0 seconds after authentication.

A one-payload / merely-`>30 s` description is obsolete and cannot classify a physical PASS or unlock stationary mapping.
...
## Historical software convergence snapshot

The old PR/head list below is retained only as archaeology. It is not current acceptance state:

- obsolete app-authority parent: PR `#2094` at `9931f9e3d54f563bd37c5f983b49e27b6f13b5b4`;
...
- opaque-payload telemetry-authority hardening: merged via PR `#2099` from `783b2b19762a32d7d3a288056f38226972c575cc`.

Live GitHub and the final exact composed head always win over this archaeology.
...
4. **Preserve correlation provenance.** Do not collapse the four-window result to only a winning UUID. The accepted artifact must preserve enough non-secret deterministic evidence to audit why the target earned current-attempt authority.
5. **App-visible exact build authority.** The primary preflight UI must consume compiled build authority directly and fail closed when exact field provenance is unavailable.
...
8. **No-resample chronology terminal.** A regressed/invalid monotonic clock or authentication-promotion chronology rejection must terminally retire the exact current generation without inventing new authority.
9. **Transport-success lifecycle closure.** Enforce one bounded local-BLE settlement owner per current generation; duplicate/stale callbacks are idempotently classified and all terminal/reset paths clear settlement ownership.
...
11. **Genuine authenticated application evidence.** Structured SDK application updates and byte-exact raw FD50 evidence are distinct provenance. Do not relabel `String(describing:)` SDK values as raw bytes.
12. **No silent weakening of the raw-evidence gate.** If the supported official SDK cannot lawfully expose byte-exact notifications under the one-owner rule, record that limitation explicitly rather than fabricating raw evidence.
13. **Opaque payloads do not mint telemetry semantics.** Application receipt alone cannot authorize speed, battery, voltage, current, power, mode, odometer, command acknowledgement, or any other field meaning.
14. **Canonical evidence seal/export.** Accepted evidence and correlation provenance must freeze immutably and export without credentials/secrets; delayed callbacks after seal cannot mutate the prefix.
15. **No semantic query or control expansion.** Do not add DP queries, unknown scooter commands, random writes, control toggles, or telemetry interpretation merely to provoke traffic.
16. **Exact-head app acceptance.** Required focused/source tests and exact-head Xcode 27 app/Capture runtime acceptance must be terminal green on the final unchanged candidate.
17. **Private intended-device acceptance.** The final accepted build identity and exact signed install must be bound to the intended iPhone 12 / iOS 27 and intended private Tuya workspace/dependency provenance.
...
6. Require at least **two** genuine non-empty qualifying application payloads from the accepted application source; the latest qualifying payload must be at least **30.0 seconds after authentication**.
7. Keep accepted authenticated continuity for at least **45.0 seconds after authentication**. The UI may show guidance, but only accepted monotonic evidence can satisfy this boundary.
...
The authenticated physical experiment may be classified `PASS` only if the sealed accepted artifact proves all of the following for the same current authenticated generation:
...
- at least **two genuine non-empty application payloads** are admitted from the accepted physical application source; byte-exact evidence is preserved where the final GO contract requires it;
- the latest qualifying admitted application payload arrives at least **30.0 seconds after authentication**;
- accepted authenticated continuity reaches at least **45.0 seconds after authentication**;
...
A transport callback, write completion, notification subscription success, timer UI, one bootstrap callback, two callbacks that both occur before the 30-second survival boundary, structured `dpsUpdate` string projection mislabeled as raw bytes, or 45 seconds without accepted genuine application evidence is **not** a physical PASS.
...
- fewer than two qualifying genuine application payloads are received;
- the latest qualifying application payload arrives before 30.0 seconds after authentication;
...
Future workers must refresh live GitHub before using any software-head snapshot in this file. Live exact heads and accepted composition win. Preserve the invariant requirements in this document while updating coordination state.
