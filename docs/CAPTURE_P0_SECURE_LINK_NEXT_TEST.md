...
18. One bootstrap callback plus transport liveness is not sufficient. The latest accepted application evidence itself must cross the 30-second post-authentication boundary, and canonical authenticated continuity must reach 45 seconds.
19. The app must seal the canonical ready prefix before presenting success. Delayed callbacks after seal or failure cannot mutate the accepted artifact.
20. Prepare/share the sanitized diagnostic JSON. The artifact must carry exact build/source provenance plus correlation provenance and must explicitly retain `rawFD50BytesCaptured=false`, `dpQueriesSent=false`, and `dpCommandsSent=false` for this supported structured-SDK path.
...
- fewer than two genuine same-generation application callbacks are admitted;
- the latest accepted application evidence does not occur at least 30 seconds after authentication;
- canonical authenticated continuity does not reach 45 seconds after authentication;
