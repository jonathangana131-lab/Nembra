...
16. Acceptance requires one uninterrupted current generation with all of the following: supported SmartLife SDK authentication provenance, current same-account exact-device authority, Tuya local BLE observably online, no disqualifying continuity/clock gap, at least **two genuine non-empty same-generation `ThingSmartDeviceDelegate.dpsUpdate` application payloads**, the **latest accepted application payload at least 30 seconds after authentication**, and at least **45 seconds of canonical authenticated observation measured from authentication**. One bootstrap callback plus transport liveness is not sufficient.
