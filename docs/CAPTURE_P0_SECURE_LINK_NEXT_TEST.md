...
7. `NembraCaptureBuildIdentity.isAuthoritativeFieldBuild` is a mandatory fail-closed build-provenance prerequisite: shipping `startBaseline()` rejects OFF1 before correlation when it is false. It is necessary but not sufficient field authority. OFF1 also cannot begin until the package-owned one-time signed authorization session is freshly `.armed` for this live app attempt and the same-current-account exact-device authority is current. Complete build metadata by itself never arms Capture or substitutes for that signed authorization.
...
1. Connect/unlock the intended iPhone 12, install the exact accepted signed Capture build, verify the app shows complete exact build metadata and that `NembraCaptureBuildIdentity.isAuthoritativeFieldBuild` is satisfied for that installed candidate, while treating that predicate only as a necessary build-provenance prerequisite rather than sufficient field authority, and verify `Procedure` is exactly `ES80-AUTHENTICATED-STATIONARY-v1`.
