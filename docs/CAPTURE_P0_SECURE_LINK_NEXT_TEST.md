...
- The first run repeatedly disconnected around 30 seconds while no application characteristic-value frames were observed; C7D09A22 does not establish the cause of that disconnect cadence.
...
5. The official Tuya SDK itself has an authenticated session for the same account that owns the scooter. **For this Apple-backed Smart Life account, use Sign in with Apple through Tuya's documented Apple OAuth transport.** Email/phone verification remains available only when that is genuinely the owning account's login method, and a different Tuya account must never be substituted merely because it can log in.
...
7. `NembraCaptureBuildIdentity.isAuthoritativeFieldBuild` is a **necessary OFF1 build-provenance prerequisite, but never sufficient field authority by itself**. OFF1 discovery cannot begin unless that predicate is true, the package-owned one-time signed authorization session is freshly `.armed` for this live app attempt, and the same-current-account exact-device authority is current.
...
The field utility must use the official SDK account transport that corresponds to the same Smart Life account that owns the scooter. For this Apple-backed Smart Life account, use **Sign in with Apple**. Email/phone verification remains available only when it is genuinely the owning account's login method. A successful login still does not authorize Bluetooth correlation or authentication until exact scooter membership is established and bound to the same current SDK account, and a different Tuya account must never be substituted.
...
1. Connect/unlock the intended iPhone 12, install the exact accepted signed Capture build, verify the app shows complete exact build metadata for that installed candidate, require `NembraCaptureBuildIdentity.isAuthoritativeFieldBuild == true` as a necessary build-provenance prerequisite without treating it as sufficient OFF1 authority, and verify `Procedure` is exactly `ES80-AUTHENTICATED-STATIONARY-v1`.
...
4. In Capture, use the same official Tuya SDK account that owns the scooter. **For this Apple-backed Smart Life account, use Sign in with Apple.** If Sign in with Apple is unavailable, the SDK does not enter the expected account, or the required Apple capability/provisioning is absent, **STOP**; do not fall back to a different account.
...
16. Keep the scooter stationary, keep its charger physically disconnected, keep Capture in the foreground, and do not change mode/light/brake/throttle/charger state during this preflight.
...
- exact build metadata/runtime identity becomes incomplete or mismatched, `NembraCaptureBuildIdentity.isAuthoritativeFieldBuild` becomes false, or the one-time signed authorization session is missing, revoked, or invalid;
