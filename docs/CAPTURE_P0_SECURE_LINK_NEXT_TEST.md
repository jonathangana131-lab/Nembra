...
The package must end the four-window series with exactly one repeatable full CoreBluetooth UUID. No repeatable candidate or more than one repeatable candidate is a terminal **STOP / restart from OFF1** result. There is no hint-based override.
...
5. The official Tuya SDK itself has an authenticated session for the **same account method that owns the scooter**. Use the account method that genuinely owns the scooter; do not substitute a different account merely because it can log in.
...
Provision the SmartLife App SDK app for the exact Capture bundle identifier and install the matching iOS security component under the ignored local provisioning path expected by the repo.

Use the repository provisioning/bootstrap/field installer so the physical candidate is built from `NembraCapture.xcworkspace`, not the bare `.xcodeproj` or the normal Nembra app target. Preserve the accepted `Podfile.lock`; do not run an ad-hoc dependency update before a physical evidence run.
...
A successful account login still does not authorize any Bluetooth correlation or authentication until exact scooter membership is established and bound to the same current SDK account.
