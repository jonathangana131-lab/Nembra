...
Its post-merge `Capture TODAY Independent Candidate Crosscheck QA` run `31294291838` completed successfully. This exact revision requires the closed-world private TODAY Research compile tuple and rejects unknown retained-environment keys so injected or downgraded authority-looking fields cannot silently widen the producer contract. Do not silently substitute a newer verifier merely because `main` moved.
...
- external cross-check does not verify the exact private TODAY Research compile tuple;
