...
- the exact retained IPA installed on the intended iPhone 12 / iOS 27 without rebuild/substitution; before scan, the operator-visible recipe/build/source/build-instance tuple must match the retained accepted candidate and package-owned ResearchAdmission must succeed for that exact running build. Executable SHA-256 and raw processed Info.plist SHA-256 remain independently inspected retained-candidate evidence; the current TODAY UI does not externally compare those hashes;
...
- an **external, post-build TODAY Final GO Record** retained outside the repository source tree that records decision `GO` and binds the exact accepted source/build/installable artifact, recipe, procedure version, intended-device install evidence, executable/raw-Info.plist candidate evidence, truthful runtime rendezvous, expected capture artifact, stop conditions, and package research-admission state. That record must not mutate the source commit it authorizes.
...
After the exact retained signed Research Field Build is independently accepted, installed without substitution on the intended device, and its truthful pre-scan runtime rendezvous succeeds, the field handoff may retain an external TODAY Final GO Record for those exact immutable subjects.
...
- evidence that, before any Bluetooth scan, the operator-visible recipe/build/source/build-instance tuple matched the retained accepted candidate and package-owned ResearchAdmission was live for that exact running build;
- expected Capture Share artifact contract;
- exact stop/failure conditions from this accepted procedure.

The executable and raw processed Info.plist hashes above remain independently inspected retained-candidate evidence. The current TODAY app does not display or externally compare those hashes before scan. The package internally computes the running executable and raw processed Info.plist identity before it can mint ResearchAdmission, so successful package-owned admission plus the visible recipe/build/source/build-instance rendezvous is the executable runtime gate the accepted product actually provides. Do not invent an operator-visible hash comparison that does not exist.
...
- the independently accepted signed-device/installable build authority is the same retained candidate whose visible recipe/build/source/build-instance tuple is confirmed during the pre-scan rendezvous;
...
- the operator-visible recipe/build/source/build-instance tuple no longer matches the retained accepted Research Field Build, or package-owned ResearchAdmission is no longer valid for the exact running build;
...
At this source checkpoint: **NO-GO.** This tracked runbook is intentionally immutable procedure source, not the post-build authorization record. The first physical session may occur only after the exact final source has terminal acceptance, the exact retained signed Research Field Build is independently accepted and installed without substitution, the visible recipe/build/source/build-instance runtime rendezvous succeeds, package-owned exact-recipe ResearchAdmission is live for that exact running build while ordinary builds remain NO-GO, all safety/preflight gates pass, and an external TODAY Final GO Record retains decision `GO` for those exact immutable subjects. Executable/raw-Info.plist hashes remain independently inspected retained-candidate evidence rather than a fabricated operator-visible runtime comparison. No post-build edit to this tracked file is required or permitted to create that authority. Full P-256 release authorization remains POST-CAPTURE unless explicitly promoted by a demonstrated TODAY blocker.
