...
This slice is an explicit child of PR #262 (`es80-tuya-dp-marker-correlation`), itself dependent on PR #238 (`es80-tuya-dp-candidate-analysis`) and the now-merged PR #219 (`es80-tuya-offline-framing`).
...
- deterministic tie-breaking when caller labels collide;
- overflow-resistant means across repeated extreme-but-finite errors.
