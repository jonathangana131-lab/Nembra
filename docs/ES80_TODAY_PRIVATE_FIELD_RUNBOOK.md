...
3. **PENDING PRIVATE SIGNING SURFACE:** run the TODAY-only `scripts/ci/xcode27_today_research_field_candidate.sh` wrapper on exact frozen source. It must compile the dedicated `NEMBRA_ES80_TODAY_RESEARCH` capability and delegate signing, exact-source, intended-device, recipe, hashing, and retained-evidence production to the canonical `scripts/ci/xcode27_signed_field_candidate.sh` producer. Invoking the ordinary producer directly is intentionally NO-GO because it must not compile research admission.
...
1. Check out exact frozen source `a0f4a33451f61411d6e0541f2e70edea5438342d` in a clean invocation checkout.
2. Provide the accepted private Apple Team/export-options inputs and the intended-device UDID only through the producer's required private mode-0600 file path. Never place the raw intended-device UDID in GitHub comments, shell command arguments, artifact names, screenshots, or public durable notes.
3. Run `scripts/ci/xcode27_today_research_field_candidate.sh` so the dedicated private TODAY compile mode is selected by the package-owned wrapper.
4. Preserve the one immutable retained candidate and its `inspection/build-evidence/NembraField.ipa` subject.
5. Require canonical signing/provisioning/intended-device inspection to pass on those exact retained bytes.
6. Complete the pinned external retained-candidate cross-check from a separate clean tooling checkout as required by `docs/ES80_TODAY_EXACT_RETAINED_IPA_INSTALL.md`; require exact `PASS_NOT_FINAL_GO` semantics and exact frozen-source producer/inspector blob reconciliation.
7. Hash the retained IPA immediately before installation.
8. Install that exact retained IPA through Xcode device management without rebuild, re-export, substitution, Product → Run, or DerivedData app installation.
9. Launch Nembra from the iPhone Home Screen and verify exact source/build/build-instance/recipe rendezvous plus `PRIVATE RESEARCH BUILD / Runtime provenance ready`.
10. Hash the original retained IPA again and require exact equality with the accepted pre-install/cross-check digest.
11. Perform the fresh operator observations required by `docs/ES80_TODAY_FINAL_GO_OPERATOR_ATTESTATION.md` and issue the hardened Final GO record through `scripts/ci/es80_today_final_go_hardened.py`.
12. Only an accepted hardened Final GO record may transition this runbook to Experiment One.
