...
The accepted external private intended-device input helper is non-authorizing operator tooling:

- helper source commit: `5006b99133bf26b8ac40a29106beee6c5ab2a6a6`
- helper path: `scripts/ci/es80_today_private_device_input.py`
- helper blob: `9a9f7f724ceaf895e52d6d443d326043f97645c8`
- exact focused QA run: `31348618461` — terminal success
- exact focused QA job: `93335149235` — terminal success on exact helper lineage head
- physical authorization: never granted by this helper

The accepted external pre-signing helper is also non-authorizing software tooling:
...
Both helpers exist only to prevent known operator-side dead ends or custody failures before the frozen producer is invoked. The frozen `a0f4…` producer independently revalidates all authoritative signing/private-input conditions.
...
- the intended iPhone's UDID, acquired into one absolute mode-`0600` regular non-symlink file through the accepted descriptor-bound helper below;
- Xcode 27 and the intended iPhone 12 / iOS 27;
- signing/provisioning credentials needed by Xcode.

Do not place the raw intended-device UDID in GitHub, PR comments, command arguments, environment variables, screenshots, artifact names, or public durable notes.
...
## 2. Create the private intended-device verification file through the accepted helper

The exact frozen `a0f4…` source intentionally does not contain this later operator helper. Do not copy a moving `main` helper into `FIELD_SOURCE`, do not alter the frozen source checkout, and do not substitute a newer app source SHA merely to gain tooling.

Materialize the exact accepted helper bytes from a separate local Nembra tooling repository. The helper opens the private path component-by-component with descriptor-relative no-follow semantics, creates the final input with exclusive no-follow authority, verifies directory/file identity after creation, and fails closed on parent-path retargeting. The raw UDID is acquired by `getpass` inside the helper and is never placed in shell argv/environment/output.

```bash
umask 077

PRIVATE_INPUT_COMMIT='5006b99133bf26b8ac40a29106beee6c5ab2a6a6'
PRIVATE_INPUT_BLOB='9a9f7f724ceaf895e52d6d443d326043f97645c8'
...
cd "$TOOL_REPO"
/usr/bin/git cat-file -e "$PRIVATE_INPUT_COMMIT^{commit}"
test "$(/usr/bin/git rev-parse --verify "$PRIVATE_INPUT_COMMIT:scripts/ci/es80_today_private_device_input.py")" = "$PRIVATE_INPUT_BLOB"
/usr/bin/git show "$PRIVATE_INPUT_COMMIT:scripts/ci/es80_today_private_device_input.py" > "$PRIVATE_INPUT_HELPER"
test "$(/usr/bin/git hash-object --no-filters -- "$PRIVATE_INPUT_HELPER")" = "$PRIVATE_INPUT_BLOB"
...
If the final path already exists, the helper fails without clobbering it. Preserve the existing file and choose a fresh private filename/path; never delete, overwrite, or fall back to a raw shell redirection merely to make the handoff pass.

Keep the private input helper material outside both `FIELD_SOURCE` and `ARTIFACTS_DIR`. Keep the resulting UDID file private and do not commit it or copy it into the retained candidate directory.
...
Do not run a moving `main` copy of the preflight helper and do not copy it into the frozen source checkout. Materialize the exact accepted helper bytes from the separate local Nembra tooling repository that contains commit `9b5bde849e6b8f6b76e2a15abb52d643e3616a7a`, then run those bytes against the exact frozen `FIELD_SOURCE`.

The preflight deliberately reads the private signing values from the environment and the intended-device value only from its mode-`0600` file. Its JSON report omits the TeamIdentifier, raw UDID, private input paths, export-options contents, and dirty-checkout text.
...
- the accepted private-input helper commit/blob cannot be materialized exactly, the helper exits nonzero, the private directory/path is retargeted or invalid, the final private path already exists, the retained verification file is not mode-`0600` regular non-symlink input, or the helper reports any other fail-closed custody error;
- anyone proposes acquiring or writing the raw intended-device UDID through shell redirection, argv, environment variables, GitHub, screenshots, or retained candidate artifacts instead of the accepted helper path;
- the intended-device verification value contains leading/trailing whitespace/newline or other bytes rejected by the accepted helper/frozen producer contract;
...
The dedicated outer worktree and temporary operator helpers can be removed after the retained candidate and required private evidence are safely preserved:
...
/bin/rm -rf "$PRIVATE_INPUT_HELPER_DIR"
```

Removing the source worktree or temporary helper material does not authorize or invalidate the retained candidate. The retained candidate's own exact provenance/evidence remains authoritative for the next gate.
