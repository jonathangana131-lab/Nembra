...
The accepted helper identity is fixed below. The current merged helper additionally fails closed rather than accepting a potentially echoed `getpass` fallback, removes the exact fresh inode if a write/fsync failure occurs without unlinking a replacement pathname subject, and refuses an already-occupied final name before asking the operator for the private UDID while retaining `O_EXCL` as the post-precheck race authority. These helper bytes are operator-custody tooling only; they do not alter the frozen `a0f4…` app subject and do not authorize signing acceptance or Bluetooth activity.

Accepted current helper evidence:

- merged helper commit: `fbd9e375292f0d5bfce48339e9c838db593d9e0f`;
- helper path: `scripts/ci/es80_today_private_device_input.py`;
- helper blob: `e9aa14d5c36afa906b27b6eb4cf76168cd3a40ca`;
- exact tested predecessor head carrying the same helper blob: `f42a005d2da13a33383cb4ef8247ef02d2983ea6`;
- focused `Capture TODAY Field Candidate Preflight QA` run `31349318657` — terminal success on that exact predecessor head.
...
PRIVATE_INPUT_HELPER_COMMIT='fbd9e375292f0d5bfce48339e9c838db593d9e0f'
PRIVATE_INPUT_HELPER_BLOB='e9aa14d5c36afa906b27b6eb4cf76168cd3a40ca'
