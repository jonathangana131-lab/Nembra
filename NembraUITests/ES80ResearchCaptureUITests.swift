...
final class ES80ResearchCaptureUITests: XCTestCase {
    ...
    func testPassiveResearchLaunchShowsProductCaptureShellWithoutVehicleControls() {
        ...
        XCTAssertFalse(
            ...
            "Horizon finalization must not be offered before a real target session reaches package-owned durable Ready and the conservative presentation wait."
        )
        XCTAssertFalse(
            app.buttons["Advanced details"].exists,
            "The product shell must not expose the package research console because that console owns independent controls outside the shell lifecycle guard."
        )
        XCTAssertFalse(
        ...
        attachment.name = "Nembra ES80 Capture Shell V14"
