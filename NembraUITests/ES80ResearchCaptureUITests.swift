...
final class ES80ResearchCaptureUITests: XCTestCase {
    ...
    func testV14ResearchLaunchMechanicallyBlocksPhysicalExperimentWhilePackageIsNoGo() {
        ...
        XCTAssertTrue(
            app.staticTexts["Physical capture locked"].waitForExistence(timeout: 3),
            "The current package-owned NO-GO must be the primary product state in rider-facing language."
        )
        XCTAssertTrue(
            app.staticTexts["Physical capture unavailable"].waitForExistence(timeout: 3),
            "The primary blocker must explain the physical state without exposing internal authority terminology."
        )
        XCTAssertTrue(
        ...
        XCTAssertFalse(
            ...
            "Internal architecture vocabulary must not remain on the primary rider-facing NO-GO surface."
        )
