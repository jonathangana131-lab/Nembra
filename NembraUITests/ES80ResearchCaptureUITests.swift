...
final class ES80ResearchCaptureUITests: XCTestCase {
    ...
    func testV14ResearchLaunchMechanicallyBlocksPhysicalExperimentWhilePackageIsNoGo() {
        ...
        XCTAssertTrue(
            ...
            "The current package-owned lock must be the primary rider-facing product state."
        )
        XCTAssertTrue(
            app.staticTexts["Not ready for scooter capture yet"].waitForExistence(timeout: 3),
            "The primary blocker must explain the rider state without exposing internal architecture terminology."
        )
        XCTAssertTrue(
        ...
        XCTAssertFalse(
            ...
            "Internal architecture vocabulary must not return to the primary rider-facing lock screen."
        )
        XCTAssertFalse(
            app.staticTexts["Physical Experiment One locked"].exists,
            "The primary rider-facing lock screen must not regress to internal experiment terminology."
        )
