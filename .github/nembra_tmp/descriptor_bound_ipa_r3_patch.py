...
old_extract_start = "def extract_ipa_safely(ipa_path: Path, destination: Path) -> Path:\n"
read_info_marker = "\ndef read_info_plist(app_path: Path) -> tuple[dict, Path]:\n"
if text.count(old_extract_start) != 1 or text.count(read_info_marker) != 1:
    raise SystemExit("unexpected extract_ipa_safely anchors")
start = text.index(old_extract_start)
end = text.index(read_info_marker, start)
new_extract = '''def extract_ipa_safely(
    ipa_path: Path,
    destination: Path,
    *,
...
'''
text = text[:start] + new_extract + text[end + 1:]
...
if text.count(old_preamble) != 1:
    raise SystemExit("unexpected snapshotted IPA inspection preamble")
