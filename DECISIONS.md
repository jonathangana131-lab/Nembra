...
## D-032 — Dashboard mode personality follows confirmed ride mode and is visual-only
Phase 11 may let Walk, Eco, Drive, and Sport feel progressively more energetic in the landscape cockpit, but that personality is deliberately presentation state rather than vehicle-performance state. `DashboardModePersonality` is resolved from the scooter-confirmed `RideMode?`, not from the user's tap intent. Command confirmation therefore remains the authority for which personality is shown.

The accepted personality levers are restrained monochrome changes in ambient center emphasis, speed-instrument scale, mode readout/marker emphasis, and secondary status emphasis. They must not alter telemetry, speed, speed-limit semantics, ride evidence, distance, history, persistence, or command behavior. They must not imply unverified power/torque/throttle/range/efficiency differences and must not invent a mapping between MAXSHOT ride modes and DP101/DP102/DP103. Reduce Motion removes the spatial/snappy transition while preserving the confirmed final state.

Real Xcode 27 / iPhone 12 / iOS 27 Simulator captures for Walk, Eco, Drive, Sport, confirmed Sport, and moving Dashboard were visually accepted on run `31063560164`: the modes are distinguishable without RGB/gamer styling, the center speed remains dominant/fixed, side rails do not reflow, and moving-state safety remains intact.

## D-033 — Final visual acceptance requires a dedicated Production Visual Overhaul
Current Home/Dashboard layouts and passing Simulator screenshots are **functional intermediate baselines**, not final product-level visual acceptance. Clean constraints, safe areas, readable hierarchy, and regression-free screenshots remain required for every systems slice, but those gates do not prove that Nembra has reached its intended product/design quality.

Once the foundational UI inputs are sufficiently real—especially battery/SoC, ride state, navigation/maps, live trip data, persisted/completed rides, and relevant vehicle/charging/error state—Nembra must run an explicit future milestone named **Production Visual Overhaul / Final Product Design Pass**. This is a release gate and must not be dropped or postponed indefinitely.

The final quality bar is world-class native iOS 27 with premium modern EV instrumentation: huge beautiful rolling MPH, truthful high-resolution battery presentation where supported, elegant range/trip/duration, deeply integrated navigation that dynamically rearranges the cockpit, polished live ride state, restrained meaningful mode personalities, original scooter-aware graphics where useful, native materials/Liquid Glass with restraint, excellent typography/spacing/depth/motion/haptics/accessibility, and minimal wasted space. Stark/Tesla-level refinement may guide the quality bar, but the result must be original and must not copy either product.

The final design must reject developer-dashboard aesthetics, giant empty black regions, generic card mosaics, prototype rails, and placeholder-looking surfaces. Visual ambition never authorizes invented telemetry, battery precision, range, route safety, or vehicle capabilities.

Before declaring the app visually complete, each major surface must go through the loop: **current real Simulator screenshot → critique against the master vision and truthful data → redesign → implement → run/interact → screenshot → critique → repeat**. This must cover portrait Home; landscape Dashboard with and without navigation; live ride; supported battery/charging states; completed rides/maps; history/stats; leaderboard when implemented; controls/settings; and major connection/permission/unsupported-hardware/persistence/recovery error states.

A technically correct screen that looks mediocre is not accepted as final.

## D-034 — Automatic ride application ownership is root-scoped; persistent history is exact, local, and hardware-gated
Phase 12 wires the existing ride domain into one root-owned application coordinator rather than creating another detector, recovery journal, or history pipeline. `AppRuntime` owns the shared scooter service, `VehicleStore`, and `RideApplicationStore` outside individual SwiftUI view lifetime. `RideApplicationStore` drives the accepted `RideCheckpointCoordinator` and `RideHistoryCommitCoordinator`, so app navigation or screen recreation cannot define ride identity.

Freshness is a hard truth boundary at the application bridge. Only raw authoritative speed packets may populate `RideObservation.speedSample`; cached vehicle-state speed and state-only control acknowledgements cannot be promoted or replayed as measurements. State publications are ride evidence only for meaningful connection transitions or actual odometer advancement. Because state and raw-speed streams are independent, the bridge may temporarily retain only the newest **unconsumed** authoritative packet while state still says connecting/reconnecting, consume it exactly once when confirmed connected state catches up, clear it on disconnect, and still subject it to the ride engine's freshness limit. This prevents both packet loss from cross-stream ordering and multiple measurements being invented from one packet.

Completed history uses a local SwiftData actor implementing the existing `RideHistoryStore` contract. It persists the exact core `RideHistoryRecord`, verifies readback before recovery evidence is acknowledged, treats an equivalent duplicate as idempotent success, rejects the same UUID with conflicting evidence, and rejects stored payload identity that does not match the indexed session UUID. Simulation recovery/history is physically namespaced separately from future production records.

Automatic ride detection policy remains **disabled for ordinary unverified production launch**. The injected thresholds used in Phase 12 are Simulator QA fixtures only. They must not become MAXSHOT defaults until real hardware establishes notification cadence, speed latency/resolution, reconnect behavior, and suitable detection/freshness thresholds. Phase 12 proves application continuity and persistence architecture; it does not claim real MAXSHOT automatic-ride behavior is hardware-validated.
